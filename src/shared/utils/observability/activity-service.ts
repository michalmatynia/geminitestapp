...
export async function logActivity(data: CreateActivityLog): Promise<ActivityLog> {
  ...
  logSystemEvent({
    ...
    source: data.sourceService ?? data.entityType ?? 'activity-service',
    userId: data.userId ?? null,
    context: {
      applicationId: activity.applicationId ?? data.applicationId ?? null,
      applicationName: activity.applicationName ?? data.applicationName ?? null,
      environment: activity.environment ?? data.environment ?? null,
      sourceService: activity.sourceService ?? data.sourceService ?? null,
      originDatabase: activity.originDatabase ?? data.originDatabase ?? null,
      originCollection: activity.originCollection ?? data.originCollection ?? null,
      originLogId: activity.originLogId ?? data.originLogId ?? activity.id,
