...
const addPlaywrightNodeToCanvas = async (page: import('@playwright/test').Page): Promise<void> => {
...
test.describe('AI Paths Admin Page', () => {
  ...
  test('should open node configuration dialog', async ({ page }) => {
    ...
    // Wait for inspector
    await expect(page.getByRole('button', { name: 'Open Node Config' })).toBeVisible({
      timeout: 10000,
    });
  ...
  test('should configure Playwright templates, personas, overrides, and capture toggles', async ({
    ...
    const templateTrigger = page
      .locator('[data-doc-id="playwright_script_template_select"]')
      .first();
    ...
    await expect(page.getByTestId('playwright-persona-fidelity')).toContainText(
      'Device: iPhone 13'
    );
  ...
  test('should fire a trigger and see it in job queue', async ({ page }) => {
    ...
    const triggerNode = page
      .locator('div[style*="translate"]')
      .filter({ hasText: /Trigger/i })
      .first();

    if (await triggerNode.isVisible({ timeout: 5000 })) {
      // Add timeout as it might not be immediately visible
      ...
      await expect(page.locator('tbody tr').first()).toContainText(/queued|running/i, {
        timeout: 20000,
      }); // Expect job to be visible and in a pending state
    } else {
