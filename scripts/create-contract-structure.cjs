...
const baseDir = path.join(__dirname, '../src/shared/contracts');
...
try {
  ...
  directories.forEach(dir => {
    ...
    if (!fs.existsSync(indexPath)) {
      fs.writeFileSync(indexPath, indexContent[dir], 'utf8');
  ...
  console.log(`\n✅ Setup Complete!`);
  console.log(`Created ${createdDirs} directories and ${createdFiles} index.ts files`);
  process.exit(0);
} catch (error) {
