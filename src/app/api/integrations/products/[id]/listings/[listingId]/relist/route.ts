...
export const POST = apiHandlerWithParams<{ id: string; listingId: string }>(
  (req, ctx, params) => {
    recordLegacyCompatCounter('compat_route_hit', {
      source: 'api.compat.integrations.products.[id].listings.[listingId].relist.POST',
      context: { route: '/api/integrations/products/[id]/listings/[listingId]/relist' },
    });
    assertLegacyCompatRouteEnabled({
      route: '/api/integrations/products/[id]/listings/[listingId]/relist',
      method: 'POST',
      source: 'api.compat.integrations.products.[id].listings.[listingId].relist.POST',
    });
