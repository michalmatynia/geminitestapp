...
export function AdvancedDraggableSection(): React.JSX.Element {
  ...
  return (
    <FormSection
      ...
      actions={<Checkbox checked={draggableEnabledValue} onCheckedChange={(v) => onChange({ ...config, draggableEnabled: v === true })} />}
        ...
        <div className='mt-4 space-y-4'>
          ...
          <div className='grid gap-3 sm:grid-cols-2'>
            <FormField label='Snap (px)'>
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(val)) onChange({ ...config, draggableSnap: Math.max(0, Math.min(200, val)) });
                }}
                className='h-9'
              />
            </FormField>
            <FormField label='Momentum factor'>
              <Input
                ...
                onChange={(e) => {
                  const val = parseFloat(e.target.value);
                  if (!isNaN(val)) onChange({ ...config, draggableMomentumFactor: Math.max(0.1, Math.min(2, val)) });
                }}
                className='h-9'
              />
            </FormField>
          ...
          <div className='flex flex-wrap items-center gap-4'>
            <div className='flex items-center gap-2'>
              <Checkbox checked={draggableMomentumValue} onCheckedChange={(v) => onChange({ ...config, draggableMomentum: v === true })} />
              <span className='text-xs text-gray-300'>Enable momentum</span>
            </div>
            <div className='flex items-center gap-2'>
              <Checkbox checked={draggableCarouselValue} onCheckedChange={(v) => onChange({ ...config, draggableCarousel: v === true })} />
            ...
            <div className='space-y-4 pl-2 border-l border-border/40'>
              <FormField label='Track selector'>
                <Input
                  ...
                  onChange={(e) => onChange({ ...config, draggableCarouselSelector: e.target.value })}
                  placeholder='e.g. .track'
                  className='h-9'
                />
              </FormField>
              <div className='flex items-center gap-2'>
                <Checkbox checked={draggableCarouselSnapValue} onCheckedChange={(v) => onChange({ ...config, draggableCarouselSnap: v === true })} />
