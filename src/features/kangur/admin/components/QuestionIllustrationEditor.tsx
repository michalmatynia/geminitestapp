    ...
    <div className='rounded-2xl border border-border/60 bg-card/30 p-3 space-y-3'>
      <div className='flex items-center gap-2'>
        <Input
          ...
          maxLength={4}
          aria-label={copy.illustration.panelLabel}
          title={copy.illustration.panelLabel}
        />
        <div className='flex-1 text-xs text-muted-foreground'>
          {copy.illustration.panelName(panel.label)}
        </div>
        <Button
          type='button'
          size='sm'
        ...
        {canDelete ? (
          <Button
            ...
            onClick={remove}
            aria-label={copy.illustration.deletePanel}
            title={copy.illustration.deletePanel}
          >
      ...
      <FormField label={copy.illustration.description}>
        <Input
          value={panel.description ?? ''}
          onChange={(event): void => setDescription(event.target.value)}
          placeholder={copy.illustration.descriptionPlaceholder}
          className='h-7 text-xs'
          aria-label={copy.illustration.descriptionPlaceholder}
          title={copy.illustration.descriptionPlaceholder}
        />
      </FormField>
    </div>
  );
  ...
  return (
    <div className='rounded-xl border border-border/40 bg-white p-3 space-y-2'>
      <div className='text-[10px] font-semibold uppercase tracking-wide text-gray-400'>
        {copy.illustration.combinedPreview}
      </div>
      <div
        className={cn(
          'gap-3',
          ...
          <div key={panel.id} className='space-y-1'>
            ...
            {panel.svgContent.trim() ? (
              ...
              <div className='flex h-20 w-full items-center justify-center rounded-lg border border-dashed border-gray-200'>
                <span className='text-[9px] text-gray-300'>{copy.illustration.empty}</span>
              </div>
            )}
          </div>
        ))}
    ...
    <div className='space-y-4'>
      <div className='flex flex-wrap items-center gap-3'>
        <div className='w-52'>
          <SelectSimple
            ...
            options={copy.illustration.typeOptions}
        ...
        {illustration.type === 'panels' ? (
          <>
            <div className='flex items-center gap-2'>
              <span className='text-xs text-muted-foreground'>{copy.illustration.panelCount}</span>
              <SelectSimple
                size='sm'
            ...
            <div className='w-36'>
              <SelectSimple
                ...
                options={copy.illustration.layoutOptions}
            ...
            >
              {copy.illustration.syncLabelsToChoices}
            </Button>
          </>
        ) : null}
      </div>
      ...
      {illustration.type === 'none' ? (
        <div className='rounded-xl border border-dashed border-border/60 bg-card/20 p-4 text-sm text-muted-foreground'>
          {copy.illustration.noIllustration}
        </div>
      ) : null}
