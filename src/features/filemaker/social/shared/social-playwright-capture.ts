...
} from '@/features/playwright/engine/capture.public';
...
export const SOCIAL_PUBLISHING_PLAYWRIGHT_CAPTURE_TIMEOUT_MS = PLAYWRIGHT_CAPTURE_TIMEOUT_MS;
export const SOCIAL_PUBLISHING_DEFAULT_PLAYWRIGHT_CAPTURE_SCRIPT = PLAYWRIGHT_DEFAULT_CAPTURE_SCRIPT;

export const createEmptySocialPublishingProgrammableCaptureRoute = (
  index = 1
): SocialPublishingProgrammableCaptureRoute => createEmptyPlaywrightCaptureRoute(index);
...
export const buildSocialPublishingProgrammableCaptureUrl = (
  baseUrl: string,
  pathValue: string
): string => {
...
export const buildSocialPublishingProgrammableCaptureRoutesFromPresetIds = (
  presetIds?: string[]
): SocialPublishingProgrammableCaptureRoute[] => {
  const allowed = new Set((presetIds ?? []).map((id) => id.trim()).filter(Boolean));
  const presets =
    allowed.size > 0
      ? SOCIAL_PUBLISHING_CAPTURE_PRESETS.filter((preset) => allowed.has(preset.id))
...
export const resolveSocialPublishingProgrammableCaptureRoutePreview = (
  path: string,
  baseUrl: string
  ...
  if (isAbsoluteUrl) {
    return {
      resolvedUrl: buildSocialPublishingProgrammableCaptureUrl('', normalizedPath),
      issue: null,
    };
  }
  ...
  try {
    return {
      resolvedUrl: buildSocialPublishingProgrammableCaptureUrl(normalizedBaseUrl, normalizedPath),
      issue: null,
    };
  } catch {
    return {
...
export const validateSocialPublishingProgrammableCaptureRoutes = (
  ...
  const validatedRoutes = routes.map((route, index) => {
    const preview = resolveSocialPublishingProgrammableCaptureRoutePreview(route.path, baseUrl);
...
export const buildSocialPublishingProgrammableCaptureInputPreview = (
  routes: SocialPublishingProgrammableCaptureRoute[],
  baseUrl: string
): Array<{
  id: string;
...
export const buildSocialPublishingProgrammableCaptureRuntimeRequestPreview = ({
  appearanceMode,
  personaId,
  routes,
  baseUrl,
  ...
  input: {
    appearanceMode: SocialPublishingCaptureAppearanceMode;
    captures: ReturnType<typeof buildSocialPublishingProgrammableCaptureInputPreview>;
  };
} => ({
  browserEngine: 'chromium',
  ...
  input: {
    appearanceMode,
    captures: buildSocialPublishingProgrammableCaptureInputPreview(routes, baseUrl),
  },
});
