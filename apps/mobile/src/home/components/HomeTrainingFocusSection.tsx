...
function TrainingLoadingContent({ copy }: { copy: HomeTrainingFocusSectionProps['copy'] }): React.JSX.Element {
  return (
...
function TrainingNotReadyContent({ copy }: { copy: HomeTrainingFocusSectionProps['copy'] }): React.JSX.Element {
  return (
    <Text style={{ color: '#475569', lineHeight: 20 }}>
      {copy({
...
function TrainingCards({
  ...
  copy: HomeTrainingFocusSectionProps['copy'];
  trainingFocus: HomeTrainingFocusSectionProps['trainingFocus'];
}): React.JSX.Element {
  if (trainingFocus.weakestOperation === null && trainingFocus.strongestOperation === null) {
    return (
      <Text style={{ color: '#475569', lineHeight: 20 }}>
        {copy({
  ...
  return (
    <View style={{ gap: 12 }}>
      {trainingFocus.weakestOperation !== null && (
        <FocusCard
          actionHref={createKangurPracticeHref(trainingFocus.weakestOperation.operation)}
          actionLabel={copy({ de: 'Schwächsten Modus trainieren', en: 'Practice weakest mode', pl: 'Trenuj najsłabszy tryb' })}
          averageAccuracyPercent={trainingFocus.weakestOperation.averageAccuracyPercent}
          lessonHref={createKangurLessonHref(trainingFocus.weakestLessonFocus)}
      ...
      {trainingFocus.strongestOperation !== null && (
        <FocusCard
          actionHref={createKangurPracticeHref(trainingFocus.strongestOperation.operation)}
          actionLabel={copy({ de: 'Tempo halten', en: 'Keep the momentum', pl: 'Utrzymaj tempo' })}
          averageAccuracyPercent={trainingFocus.strongestOperation.averageAccuracyPercent}
          lessonHref={createKangurLessonHref(trainingFocus.strongestLessonFocus)}
...
export function HomeTrainingFocusSection({
  ...
  if (!areDeferredHomePanelsReady) return null;

  const getLoadingContent = (): React.JSX.Element => {
    if (trainingFocus.isRestoringAuth || trainingFocus.isLoading) return <TrainingLoadingContent copy={copy} />;
    return <DeferredTrainingFocusDetailsPlaceholder />;
  };

  const getActiveContent = (): React.JSX.Element => {
    if (!trainingFocus.isEnabled && trainingFocus.weakestOperation === null && trainingFocus.strongestOperation === null) {
      return <TrainingNotReadyContent copy={copy} />;
    }
    if (trainingFocus.error !== null && trainingFocus.error !== '') {
      return <Text style={{ color: '#b91c1c', lineHeight: 20 }}>{trainingFocus.error}</Text>;
    }
    return <TrainingCards copy={copy} trainingFocus={trainingFocus} />;
  };

  const renderContent = (): React.JSX.Element => {
    if (trainingFocus.isRestoringAuth || trainingFocus.isLoading || !areDeferredHomeTrainingFocusDetailsReady) {
