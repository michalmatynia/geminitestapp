...
import {
  useSocialPublishingImageAddons,
...
export function useSocialEditorSync(deps: SocialEditorSyncDeps) {
  ...
  const activePostQuery = useSocialPublishingPost(activePostId, {
  ...
  useEffect(() => {
    ...
    trackSocialPublishingClientEvent('social_publishing_page_view', {
      ...
      hasPublishingIntegration: Boolean(deps.publishingConnectionId),
  ...
  const handleSelectAddon = (addon: SocialPublishingImageAddon): void => {
  ...
  const handleSelectAddons = (addons: SocialPublishingImageAddon[]): void => {
