...
describe('CMS Domain Service', () => {
  ...
  describe('resolveCmsDomainScopeById', () => {
    it('handles circular aliases gracefully', async () => {
      ...
      mockCollection.findOne.mockResolvedValueOnce(d1).mockResolvedValueOnce(d2);
  ...
  describe('Management', () => {
    ...
    it('should delete a domain and its links', async () => {
      ...
      expect(mockCollection.updateMany).toHaveBeenCalledWith({ aliasOf: 'd1' }, expect.any(Object));
    });
