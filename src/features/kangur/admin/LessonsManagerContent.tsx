...
import { Badge, SelectSimple, useToast } from '@/features/kangur/shared/ui';
...
export function LessonsManagerContent({ standalone = true }: { standalone?: boolean }): React.JSX.Element {
  ...
  return (
      <KangurAdminContentShell
        ...
        headerActions={
          <div className='flex items-center gap-3'>
            ...
            <SelectSimple value={ageGroupFilter} onChange={(v) => setAgeGroupFilter(v as any)} options={[{ value: 'all', label: 'All Ages' }, ...KANGUR_AGE_GROUPS.map((g) => ({ value: g.id, label: g.label }))]} className='w-40' ariaLabel='Age group filter' title='Age group filter' />
            <SelectSimple value={authoringFilter} onChange={(v) => setAuthoringFilter(v as any)} options={authoringFilterCounts.map((f) => ({ value: f.id, label: `${f.label} (${f.count})` }))} className='w-52' ariaLabel='Editorial state filter' title='Editorial state filter' />
      ...
      >
        ...
        <ConfirmModal isOpen={Boolean(lessonToDelete)} onClose={() => setLessonToDelete(null)} onConfirm={() => void handleDelete()} title='Delete Lesson' message={`Are you sure you want to delete "${lessonToDelete?.title}"?`} confirmLabel='Delete' variant='danger' />
