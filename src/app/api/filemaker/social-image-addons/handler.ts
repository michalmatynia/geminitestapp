...
} from '@/features/filemaker/social/server/social-image-addons-repository';
import { createSocialPublishingImageAddonFromPlaywright } from '@/features/filemaker/social/server/social-image-addons-service';
import { ErrorSystem } from '@/shared/utils/observability/error-system';
import { socialPublishingCaptureAppearanceModeSchema } from '@/shared/contracts/social-publishing-image-addons';
...
export async function getSocialPublishingImageAddonsHandler(
  req: NextRequest,
  ...
  const [recentAddons, selectedAddons] = await Promise.all([
    listSocialPublishingImageAddons(limit),
    requestedIds.length > 0 ? findSocialPublishingImageAddonsByIds(requestedIds) : Promise.resolve([]),
  ]);
  ...
  void logSocialPublishingServerEvent({
    source: 'social-publishing.image-addons.list',
    message: 'Social publishing image add-ons listed',
    request: req,
    requestContext: ctx,
    actor,
    statusCode: 200,
...
export async function postSocialPublishingImageAddonsHandler(
  req: NextRequest,
  ctx: ApiHandlerContext
): Promise<Response> {
  const actor = await resolveSocialPublishingActor(req);
  ...
  try {
    ...
    const addon = await createSocialPublishingImageAddonFromPlaywright({
      title: parsed.title,
    ...
    void logSocialPublishingServerEvent({
      source: 'social-publishing.image-addons.create',
      message: 'Social publishing image add-on created',
      request: req,
      requestContext: ctx,
      actor,
      statusCode: 200,
