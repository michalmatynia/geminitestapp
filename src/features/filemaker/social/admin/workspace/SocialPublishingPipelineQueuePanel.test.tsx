...
describe('SocialPublishingPipelineQueuePanel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('defers the compact queue status bootstrap until after the initial mount', async () => {
    ...
    try {
      render(<SocialPublishingPipelineQueuePanel variant='compact' />);
      ...
      expect(apiGetMock).toHaveBeenCalledWith('/api/filemaker/social-pipeline/status', {
        timeout: 60_000,
      });
    } finally {
      vi.clearAllTimers();
  ...
  it('hides run now while the full queue panel already has active pipeline work', async () => {
    ...
    render(<SocialPublishingPipelineQueuePanel />);

    await waitFor(() => {
      expect(apiGetMock).toHaveBeenCalledWith('/api/filemaker/social-pipeline/status', {
        timeout: 60_000,
      });
      expect(apiGetMock).toHaveBeenCalledWith('/api/filemaker/social-pipeline/jobs', {
        timeout: 60_000,
      });
    });
  ...
  it('keeps run now visible in compact mode when the queue is only waiting', async () => {
    ...
    expect(apiGetMock).not.toHaveBeenCalledWith('/api/filemaker/social-pipeline/jobs', {
      timeout: 60_000,
    });
  ...
  it('shows delete for terminal jobs, hides it for active jobs, and refreshes after delete', async () => {
    ...
    await waitFor(() => {
      expect(apiDeleteMock).toHaveBeenCalledWith('/api/filemaker/social-pipeline/jobs', {
