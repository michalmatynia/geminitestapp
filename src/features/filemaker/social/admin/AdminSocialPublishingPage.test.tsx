...
const { useAdminSocialPublishingPageMock } = vi.hoisted(() => ({
  useAdminSocialPublishingPageMock: vi.fn(),
}));
...
const { useSocialPublishingImageAddonsBatchJobsMock } = vi.hoisted(() => ({
  useSocialPublishingImageAddonsBatchJobsMock: vi.fn(),
}));
...
vi.mock('@/features/kangur/admin/components/KangurAdminContentShell', () => ({
  KangurAdminContentShell: ({
    ...
    <div>
      <h1>{title}</h1>
      <div>{description}</div>
      <div data-testid='social-publishing-header-actions'>{headerActions}</div>
      <div>{children}</div>
    </div>
  ),
}));
...
vi.mock('./workspace/SocialPublishingPage.hooks', () => ({
  useAdminSocialPublishingPage: () => useAdminSocialPublishingPageMock(),
}));
...
vi.mock('@/features/filemaker/social/hooks/useSocialPublishingImageAddons', () => ({
  useSocialPublishingImageAddonsBatchJobs: (...args: unknown[]) =>
    useSocialPublishingImageAddonsBatchJobsMock(...args),
}));
...
vi.mock('./workspace/SocialPublishingPipelineQueuePanel', () => ({
  SocialPublishingPipelineQueuePanel: () => (
    <div data-testid='social-publishing-pipeline-queue'>social-publishing-pipeline-queue</div>
  ),
}));
...
import { AdminSocialPublishingPage } from './AdminSocialPublishingPage';

const buildPost = () => ({
  id: 'post-1',
  titlePl: 'Product Weekly Update',
  titleEn: 'Product Weekly Update',
  bodyPl: '',
  bodyEn: '',
  combinedBody: '',
...
const buildHookState = () => ({
  ...
  publishingConnectionId: 'linkedin-1',
  ...
  handlePublishingConnectionChange: vi.fn(),
...
describe('AdminSocialPublishingPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    useSocialPublishingImageAddonsBatchJobsMock.mockReturnValue({
      data: [],
      isLoading: false,
    });
    useAdminSocialPublishingPageMock.mockReturnValue(buildHookState());
  });

  it('keeps the posts workspace visible until the settings modal is opened', () => {
    render(<AdminSocialPublishingPage />);

    expect(screen.queryByRole('tablist', { name: 'Social Publishing sections' })).not.toBeInTheDocument();
    ...
    expect(screen.getByTestId('social-publishing-pipeline-queue')).toBeInTheDocument();
  ...
  it('keeps batch capture history disabled until settings are opened', () => {
    ...
    expect(useSocialPublishingImageAddonsBatchJobsMock).toHaveBeenLastCalledWith({
      limit: 5,
      enabled: false,
    });
  ...
  it('loads batch capture history when programmable capture is already open', () => {
    ...
    useAdminSocialPublishingPageMock.mockReturnValue(hookState);
  ...
  it('keeps the list visible while the active social workspace detail is still loading', () => {
    useAdminSocialPublishingPageMock.mockReturnValue({
      ...buildHookState(),
      posts: [],
      activePostId: null,
      activePost: null,
  ...
  it('opens the settings modal with models, project, documentation, publishing, and capture tabs', () => {
    ...
    expect(screen.getByLabelText('Default publishing connection')).toBeInTheDocument();
  ...
  it('shows a page-level Playwright capture counter while a standalone capture job is running', () => {
    ...
    expect(screen.getByTestId('social-publishing-header-actions')).toHaveTextContent(
  ...
  it('blocks unpublishing the active post while Social runtime jobs are still in flight', () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Unpublish' })).toBeInTheDocument();
    expect(
      screen.getByText('Wait for the current Social runtime job to finish before confirming this action.')
    ).toBeInTheDocument();
