...
import type { getProductValidationSemanticOperationUiMetadata } from '@/shared/lib/products/utils/validator-semantic-operations';
...
import type { SemanticTransitionNotice } from './ValidatorPatternModal.helpers';
import { ValidatorDocTooltip } from './ValidatorDocsTooltips';
import { useValidatorSettingsContext } from './ValidatorSettingsContext';

type SemanticUiMetadata = ReturnType<typeof getProductValidationSemanticOperationUiMetadata>;

export function ValidatorPatternModalContent({
  semanticUi,
  transitionNotice,
}: {
  semanticUi: SemanticUiMetadata;
  transitionNotice: SemanticTransitionNotice | null;
}): React.JSX.Element {
  return (
    <div className='space-y-4'>
      <SemanticMetadataNotice semanticUi={semanticUi} />
      <SemanticTransitionPanel transitionNotice={transitionNotice} />
      ...
      <RegexTimingFields />
      <MessageField semanticUi={semanticUi} />
    </div>
  );
}

function SemanticMetadataNotice({
  semanticUi,
}: {
  semanticUi: SemanticUiMetadata;
}): React.JSX.Element | null {
  if (semanticUi === null) return null;
...
function SemanticTransitionPanel({
  transitionNotice,
}: {
  transitionNotice: SemanticTransitionNotice | null;
}): React.JSX.Element | null {
  if (transitionNotice === null) return null;
  ...
  return (
    <div className={className}>
      <p className={titleClassName}>{transitionNotice.title}</p>
      <p className={bodyClassName}>{transitionNotice.body}</p>
    </div>
  );
}

function useSequenceGroupOptions(): Array<{
  description: string;
  label: string;
  value: string;
}> {
  const { sequenceGroups } = useValidatorSettingsContext();
...
function SequenceFields(): React.JSX.Element {
  const { formData, setFormData } = useValidatorSettingsContext();
  const sequenceGroupOptions = useSequenceGroupOptions();
  ...
  return (
    <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
      ...
      >
        <SelectSimple
          size='sm'
          value={formData.sequenceGroupId === '' ? '__none__' : formData.sequenceGroupId}
      ...
      <FormField label='Sequence'>
        <Input
          ...
          onChange={(event: React.ChangeEvent<HTMLInputElement>): void =>
            setFormData((prev: PatternFormData) => ({ ...prev, sequence: event.target.value }))
          }
          placeholder='10'
          aria-label='10'
          title='10'
...
function ChainFields(): React.JSX.Element {
  const { formData, setFormData } = useValidatorSettingsContext();
    ...
    <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
      ...
      <FormField label='Max Executions'>
        <Input
          ...
          onChange={(event: React.ChangeEvent<HTMLInputElement>): void =>
            setFormData((prev: PatternFormData) => ({ ...prev, maxExecutions: event.target.value }))
          }
          placeholder='1'
          aria-label='1'
          title='1'
...
function ReplacementFields(): React.JSX.Element {
  const { formData, setFormData, replacementFieldOptions, normalizeReplacementFields } =
...
function RegexTimingFields(): React.JSX.Element {
  const { formData, setFormData } = useValidatorSettingsContext();
...
function MessageField({ semanticUi }: { semanticUi: SemanticUiMetadata }): React.JSX.Element {
  const { formData, setFormData } = useValidatorSettingsContext();
  const placeholder = semanticUi?.messagePlaceholder ?? 'Remove duplicate spaces from product name.';
