...
describe('social pipeline jobs handler', () => {
  ...
  it('returns a compact manual job summary in list mode', async () => {
    ...
    const response = await getHandler(
      new NextRequest('http://localhost/api/filemaker/social-pipeline/jobs', {
        method: 'GET',
      }),
      createContext({})
    );
  ...
  it('returns the full manual job result when requesting a specific job id', async () => {
    ...
    const response = await getHandler(
      new NextRequest('http://localhost/api/filemaker/social-pipeline/jobs?id=job-1', {
        method: 'GET',
      }),
  ...
  it('fails when the job does not exist', async () => {
    ...
    await expect(
      deleteHandler(
        new NextRequest('http://localhost/api/filemaker/social-pipeline/jobs?id=missing-job', {
          method: 'DELETE',
        }),
