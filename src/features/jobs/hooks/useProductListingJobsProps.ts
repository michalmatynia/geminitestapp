...
export function useProductListingJobsProps() {
  ...
  const listingRows: ListingRow[] = useMemo<ListingRow[]>(
    () =>
      jobs.flatMap((job: ProductJob): ListingRow[] =>
        job.listings.map((listing: ListingJob): ListingRow => ({
          job,
          listing,
        }))
      ),
  ...
  const filteredRows: ListingRow[] = useMemo<ListingRow[]>(() => listingRows.filter(({ job, listing }: ListingRow) => {
  ...
  const tableData: JobRowData[] = useMemo<JobRowData[]>(() => pagedRows.map((row: ListingRow): JobRowData => {
