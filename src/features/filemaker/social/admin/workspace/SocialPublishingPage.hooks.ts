...
export function useAdminSocialPublishingPage(options?: UseAdminSocialPublishingPageOptions) {
  ...
  const hasSavedProgrammableCaptureDefaults =
    ...
    persistedProgrammableCaptureScript !== SOCIAL_PUBLISHING_DEFAULT_PLAYWRIGHT_CAPTURE_SCRIPT;
  ...
  const buildSocialContext = useCallback((overrides?: Record<string, unknown>): Record<string, unknown> => ({
    ...
    hasPublishingConnection: Boolean(settings.publishingConnectionId),
    brainModelId: resolvedBrainModelId,
    visionModelId: resolvedVisionModelId,
