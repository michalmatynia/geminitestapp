...
import {
  normalizeSocialPublishingImageAddon,
  type SocialPublishingCaptureAppearanceMode,
  type SocialPublishingImageAddonBatchCaptureResult,
...
import {
  buildSocialPublishingProgrammableCaptureUrl,
  SOCIAL_PUBLISHING_DEFAULT_PLAYWRIGHT_CAPTURE_SCRIPT,
  SOCIAL_PUBLISHING_PLAYWRIGHT_CAPTURE_TIMEOUT_MS,
...
import {
  ...
  upsertSocialPublishingImageAddon,
...
type BatchCaptureInput = Omit<SocialPublishingImageAddonsBatchPayload, 'baseUrl' | 'presetIds'> & {
  baseUrl: string;
  presetIds?: string[] | null;
  playwrightPersonaId?: string | null;
  playwrightScript?: string | null;
  playwrightRoutes?: SocialPublishingProgrammableCaptureRoute[] | null;
...
type BatchCapturePreset = (typeof SOCIAL_PUBLISHING_CAPTURE_PRESETS)[number];
...
type ResolvedBatchCaptureRequest = {
  baseUrl: string;
  appearanceMode: SocialPublishingCaptureAppearanceMode | null;
  createdBy: string | null;
...
const buildAddonPublicPath = (filename: string): string =>
  `/uploads/filemaker/social-addons/${filename}`;
...
const BATCH_CAPTURE_TEMP_ROOT = '/var/tmp/libapp-uploads/filemaker/social-addons';
...
const buildServeUrl = (filename: string): string =>
  `/api/filemaker/social-image-addons/serve?filename=${encodeURIComponent(filename)}`;
...
const resolvePlaywrightStorageState = (params: {
  ...
  if (resolved.droppedCookieNames.length > 0) {
    logger.warn('[social-publishing.image-addons.batch] dropped invalid Playwright cookies', {
...
const readCaptureResultStatus = (
  value: unknown
): SocialPublishingImageAddonBatchCaptureResult['status'] => {
  switch (value) {
    case 'ok':
    case 'failed':
    case 'skipped':
...
const readBatchCaptureResults = (
  value: unknown
): Map<string, SocialPublishingImageAddonBatchCaptureResult> => {
  ...
  return new Map(
    value.flatMap((entry) => {
      ...
      const parsed: SocialPublishingImageAddonBatchCaptureResult = {
        id,
...
const resolveBatchCaptureRequest = (
  ...
  const requestedPresets =
    presetIds.length > 0
      ? SOCIAL_PUBLISHING_CAPTURE_PRESETS.filter((preset) => presetIds.includes(preset.id))
      : SOCIAL_PUBLISHING_CAPTURE_PRESETS;
  ...
  const playwrightScript =
    input.playwrightScript?.trim() || SOCIAL_PUBLISHING_DEFAULT_PLAYWRIGHT_CAPTURE_SCRIPT;
...
export const startPlaywrightBatchCapture = async (
  ...
  const captures = resolved.targets.map((target) => ({
    id: target.id,
    title: target.title,
    url: buildSocialPublishingProgrammableCaptureUrl(resolved.baseUrl, target.path),
  ...
  const run = await startPlaywrightEngineTask({
    request: {
      ...
      timeoutMs: SOCIAL_PUBLISHING_PLAYWRIGHT_CAPTURE_TIMEOUT_MS,
...
export const finalizePlaywrightBatchCapture = async (
  ...
  const addons: SocialPublishingImageAddon[] = [];
  const failures: Array<{ id: string; reason: string }> = [];
  const captureResults: SocialPublishingImageAddonBatchCaptureResult[] = [];
  ...
  for (const target of targets) {
    ...
    const resolvedUrl =
      resultStatus?.resolvedUrl ?? buildSocialPublishingProgrammableCaptureUrl(baseUrl, target.path);
    if (!artifact) {
    ...
    const addon = normalizeSocialPublishingImageAddon({
      id: randomUUID(),
      title: target.title,
      description: target.description,
      sourceUrl: buildSocialPublishingProgrammableCaptureUrl(baseUrl, target.path),
    ...
    const saved = await upsertSocialPublishingImageAddon(addon);
    addons.push(saved);
    captureResults.push({
      id: target.id,
      title: resultStatus?.title ?? target.title,
  ...
  void ErrorSystem.logInfo('Social publishing image add-on batch capture completed', {
    service: 'social-publishing.image-addons',
    action: 'batch',
...
export async function createSocialPublishingImageAddonsBatch(
