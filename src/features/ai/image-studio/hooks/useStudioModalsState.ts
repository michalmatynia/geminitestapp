...
export function useStudioModalsState() {
  ...
  const { setExtractReviewOpen, setExtractDraftPrompt } = usePromptActions();
  ...
  const selectedGenerationPreview = useMemo(() => {
    ...
    return (
      linkedGeneratedVariants.find((v) => v.key === generationPreviewKey) ??
  ...
  const handleApplyLinkedVariantToCard = useCallback(
    async (variant: LinkedGeneratedVariant) => {
