        ...
        try {
          ...
          if (group.length > 1) {
            const ids = group.map(item => 
              item.queryKey[item.queryKey.length - 1]
            ...
            if (ids.length > 0) {
              ...
              const batchResult = await fetch('/api/batch', {
                ...
                body: JSON.stringify({ ids, type: group[0].queryKey[0] }),
              }).then(r => r.json());
...
export function useQueryDeduplication() {
  ...
  const deduplicatedQuery = useCallback(async (
    ...
    if (activeQueries.current.has(key)) {
      return activeQueries.current.get(key)!;
    }
