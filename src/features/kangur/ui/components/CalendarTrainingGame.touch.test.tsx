/**
 * @vitest-environment jsdom
 */

import { render, screen } from '@testing-library/react';
import { describe, expect, it, vi } from 'vitest';

vi.mock('@/features/kangur/ui/hooks/useKangurCoarsePointer', () => ({
  useKangurCoarsePointer: () => true,
}));

import CalendarTrainingGame from '@/features/kangur/ui/components/CalendarTrainingGame';

describe('CalendarTrainingGame touch mode', () => {
  it('shows a touch hint and larger answer cards on coarse pointers', () => {
    render(<CalendarTrainingGame onFinish={() => undefined} />);

    expect(screen.getByTestId('calendar-training-touch-hint')).toHaveTextContent(
      'Dotknij odpowiedź, a potem poczekaj na kolejne pytanie.'
    );

    const firstChoice = screen.getByTestId('calendar-training-choice-0');
    expect(firstChoice).toHaveClass('touch-manipulation', 'select-none', 'min-h-[4.25rem]');
  });
});
