...
describe('GeometryPerimeterLesson', () => {
  it('opens the game shell through the launchable default instance', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Finish geometry perimeter' }));

    await waitFor(() => {
      expect(screen.getByTestId('lesson-hub-section-game_draw')).toBeInTheDocument();
    });
  });
});
