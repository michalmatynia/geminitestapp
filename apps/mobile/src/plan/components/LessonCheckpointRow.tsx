...
function CheckpointHeader({ item }: { item: KangurMobileLessonCheckpointItem }) {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
      ...
      <Pill label={`${item.bestScorePercent}%`} tone={{ backgroundColor: '#eef2ff', borderColor: '#c7d2fe', textColor: '#4338ca' }} />
    </View>
  );
}
...
export function LessonCheckpointRow({
  ...
  return (
    <InsetPanel gap={10}>
      ...
      <CheckpointMetadata item={item} copy={copy} locale={locale} />
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
        ...
        {item.practiceHref !== null && (
          <LinkButton href={item.practiceHref} label={`${copy({ de: 'Danach trainieren', en: 'Practice after', pl: 'Potem trenuj' })}: ${item.title}`} />
        )}
      </View>
    </InsetPanel>
  );
