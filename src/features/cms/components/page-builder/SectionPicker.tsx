...
const SectionCard = ({ def, onClick }: { def: SectionDefinition; onClick: () => void }): React.ReactNode => (
  <button
    type='button'
    onClick={onClick}
...
const CategorySection = ({ title, items, onSelect }: CategorySectionProps): React.ReactNode =>
  items.length > 0 ? (
    <div>
      ...
      <div className='grid gap-3 md:grid-cols-2'>
        {items.map((def: SectionDefinition) => (
          <SectionCard key={def.type} def={def} onClick={() => onSelect(def.type)} />
        ))}
      </div>
    </div>
  ) : null;
    ...
    <>
      ...
      <AppModal open={isOpen} onClose={() => setIsOpen(false)} title='Add a section' size='lg' bodyClassName='h-[70vh]' header={
        ...
        <div className='space-y-6'>
          <CategorySection title='Primitives' items={primitives} onSelect={handleSelect} />
          <CategorySection title='Elements' items={elements} onSelect={handleSelect} />
          <CategorySection title='Templates' items={templates} onSelect={handleSelect} />
