...
2. `npm run canonical:check:sitewide` -> passed (`3847` runtime source files, `4` docs).
3. `npm run ai-paths:check:canonical` -> passed (`4245` source files under `src/`).
...
6. Consolidated stabilization gate rerun:
   - `npm run canonical:stabilization:check` -> passed (latest observability snapshot `generatedAt=2026-03-05T01:50:45.714Z`).
