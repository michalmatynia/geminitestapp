...
type SemanticAuditEntry = ReturnType<typeof getProductValidationSemanticAuditHistory>[number];
...
const hasFocusedAuditKey = (focusedAuditKey: string | null): focusedAuditKey is string =>
  focusedAuditKey !== null && focusedAuditKey !== '';
...
export function ValidatorPatternSemanticHistoryPanel({
  ...
  React.useEffect(() => {
    if (!hasFocusedAuditKey(focusedAuditKey)) return;
  ...
  return (
    <section className='mt-3 w-full min-w-0 overflow-hidden rounded-md border border-border/60 bg-background/20 p-3'>
      <SemanticHistoryHeader
        patternLabel={pattern.label}
        currentSemanticTitle={currentSemanticTitle}
        onClose={onClose}
      />
      <SemanticHistoryEntries
        historyEntries={historyEntries}
        focusedAuditKey={focusedAuditKey}
        entryRefs={entryRefs}
      />
    </section>
  );
...
function SemanticHistoryHeader({
  patternLabel,
  currentSemanticTitle,
  onClose,
}: {
  patternLabel: string;
  currentSemanticTitle: string;
  onClose?: (() => void) | undefined;
}): React.JSX.Element {
  return (
    <div className='flex flex-wrap items-start justify-between gap-3'>
      <div className='min-w-0 flex-1 space-y-1'>
        ...
        <p className='break-words text-xs text-muted-foreground'>
          Selected pattern: <span className='font-medium text-foreground break-words'>{patternLabel}</span>
        </p>
      </div>
      <div className='flex max-w-full flex-wrap items-center justify-end gap-2'>
        ...
        {onClose !== undefined ? (
          <Button type='button' variant='ghost' size='xs' onClick={onClose} className='h-7 shrink-0'>
            Close History
          </Button>
        ) : null}
      </div>
...
function SemanticHistoryEntries({
  historyEntries,
  focusedAuditKey,
  entryRefs,
}: {
  historyEntries: readonly SemanticAuditEntry[];
  focusedAuditKey: string | null;
...
function SemanticHistoryEntry({
  entry,
  focusedAuditKey,
  entryRefs,
}: {
  entry: SemanticAuditEntry;
  focusedAuditKey: string | null;
  entryRefs: React.MutableRefObject<Map<string, HTMLDivElement | null>>;
}): React.JSX.Element {
  const entryKey = getProductValidationSemanticAuditRecordKey(entry);

  return (
    <div
      ref={(element) => {
        entryRefs.current.set(entryKey, element);
      }}
      className={cn(
        'min-w-0 overflow-hidden rounded-md border border-border/50 bg-black/20 p-3 transition',
        focusedAuditKey === entryKey && 'border-sky-500/50 bg-sky-500/10 ring-1 ring-sky-500/40'
      )}
    >
      <SemanticHistoryEntryHeader entry={entry} />
      ...
      <SemanticHistoryEntryOperations entry={entry} />
    </div>
  );
}

function SemanticHistoryEntryHeader({ entry }: { entry: SemanticAuditEntry }): React.JSX.Element {
  return (
    <div className='flex min-w-0 flex-wrap items-center gap-2 text-[11px]'>
      ...
      <span className={`rounded border px-2 py-0.5 font-medium ${SEMANTIC_AUDIT_TRANSITION_TONE[entry.transition]}`}>
        {entry.transition}
      </span>
    </div>
  );
...
function SemanticHistoryEntryOperations({ entry }: { entry: SemanticAuditEntry }): React.JSX.Element {
  return (
