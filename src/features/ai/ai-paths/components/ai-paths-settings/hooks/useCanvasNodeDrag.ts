...
export function useCanvasNodeDrag(args: {
  ...
  const handlePointerDown = useCallback((event: React.PointerEvent<HTMLDivElement>, nodeId: string): void => {
    if (isPathLocked) {
      notifyLocked();
      return;
    }
  ...
  const handlePointerMove = useCallback((event: React.PointerEvent<HTMLDivElement>, nodeId: string): void => {
  ...
  const handlePointerUp = useCallback((event: React.PointerEvent<HTMLDivElement>, nodeId: string): void => {
    if (dragState?.nodeId !== nodeId) return;
  ...
  const handleDragStart = useCallback((event: React.DragEvent<HTMLDivElement>, node: NodeDefinition): void => {
    if (isPathLocked) {
