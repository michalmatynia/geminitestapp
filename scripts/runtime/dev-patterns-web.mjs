...
const stateFile = path.join(runtimeDir, 'dev-patterns-web.json');
...
const terminateDevProcess = async ({ force }) => {
  ...
  if (!state) {
    console.log('No tracked Patterns web dev process found.');
    return;
  }

  if (state.devPid && isPidRunning(state.devPid)) {
    const firstSignal = force ? 'SIGKILL' : 'SIGTERM';
    console.log(`Stopping Patterns web dev process group ${state.devPid} with ${firstSignal}.`);
    killProcessGroup(state.devPid, firstSignal);
    if (!force) {
      await sleep(stopWaitMs);
      if (isPidRunning(state.devPid)) {
        console.log(`Patterns web dev process ${state.devPid} is still running; forcing SIGKILL.`);
        killProcessGroup(state.devPid, 'SIGKILL');
      }
    }
  } else {
    console.log('Tracked Patterns web dev process is not running.');
  }
  ...
  if (
    ...
    console.log(`Stopping Patterns web dev supervisor ${state.supervisorPid} with ${signal}.`);
...
const stopMongo = () => {
  runNpmScript('mongo:patterns:down');
};

const startMongo = () => {
  runNpmScript('mongo:patterns:up');
};
...
const printStatus = async () => {
  ...
  runNpmScript('mongo:patterns:status');
};
...
const shutdownSupervisor = async (child, reason, exitCode = 0) => {
  ...
  shuttingDown = true;
  console.log(`\n[dev:patterns-web] Shutting down (${reason}).`);
...
const startDev = async () => {
  const state = await readState();
  if (state?.devPid && isPidRunning(state.devPid)) {
    console.log(`Patterns web dev app is already running (pid ${state.devPid}).`);
    startMongo();
    return;
  }
  await clearState();
  ...
  const child = spawn(npmCommand, ['run', 'dev', '-w', '@app/patterns-web'], {
    cwd: repoRoot,
    detached: true,
    env: process.env,
    stdio: 'inherit',
  ...
  await writeState({
    command: 'npm run dev -w @app/patterns-web',
    devPid: child.pid,
    supervisorPid: process.pid,
    startedAt: new Date().toISOString(),
  });

  console.log(`\n[dev:patterns-web] Patterns web dev app started (pid ${child.pid}).`);
  console.log('[dev:patterns-web] Press Ctrl+C or run npm run dev:patterns-web:kill to stop app + MongoDB.');
  ...
  child.on('exit', (code, signal) => {
    ...
    void shutdownSupervisor(child, `patterns-web dev exited${signal ? ` by ${signal}` : ''}`, exitCode);
  });
};
