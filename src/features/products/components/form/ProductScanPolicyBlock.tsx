...
export function ProductScanPolicyBlock({
  ...
  const urlsHref = summary.blockActions === true ? buildProductScan1688SectionId(scan.id, 'candidate-urls') : null;
  const matchHref = summary.blockActions === true ? buildProductScan1688SectionId(scan.id, 'match-evaluation') : null;

  return (
    <div className='space-y-1.5 rounded-md border border-amber-500/20 bg-amber-500/5 px-3 py-2'>
      <p className='text-xs font-medium text-amber-300'>
        Apply is blocked by policy: {summary.reason}
      </p>
      {isReviewed === true ? (
        <p className='text-[11px] text-muted-foreground'>
          Review bypass active (reviewed at {formatTimestamp(scan.updatedAt)})
        </p>
      ) : (
        <div className='flex flex-wrap gap-3'>
          ...
          {summary.blockActions === true ? (
            <button
              type='button'
              onClick={() => markBlockedScanReviewed(scan.id)}
      ...
      {isReviewed === true && summary.blockActions === true ? (
         <button
            type='button'
            onClick={() => clearBlockedScanReviewed(scan.id)}
            className='text-primary underline-offset-2 hover:underline text-[11px]'
         >
            Undo review
         </button>
