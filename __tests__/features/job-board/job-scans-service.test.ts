...
const mocks = vi.hoisted(() => ({
  ...
  upsertCompanyByMatchMock: vi.fn(),
  upsertJobListingMock: vi.fn(),
  upsertCompanyMock: vi.fn(),
  findCompanyEmailsMock: vi.fn(),
  upsertJobScanMock: vi.fn(),
  isVisionEmailFinderEnabledMock: vi.fn(),
}));
...
import { findCompanyEmails, isVisionEmailFinderEnabled } from '@/features/job-board/server/email-finding';
import { upsertCompany, upsertCompanyByMatch } from '@/features/job-board/server/companies-repository';
import { upsertJobListing } from '@/features/job-board/server/job-listings-repository';
import { upsertJobScan } from '@/features/job-board/server/job-scans-repository';
import { createJobScan, probeJobBoardOffer, synchronizeJobScan } from '@/features/job-board/server/job-scans-service';

// ... (update the usage in tests, replacing mockResolvedValue calls)
// Replaced vi.mocked usage with direct mock reference
    mocks.upsertCompanyByMatchMock.mockImplementation(async () => ({
      id: 'company-1',
      name: 'Acme',
    }));
...
describe('job scan email route selection', () => {
  beforeEach(() => {
    ...
    mocks.upsertJobListingMock.mockImplementation(async () => ({
      id: 'listing-1',
      companyId: 'company-1',
      title: 'Frontend Developer',
      description: null,
    ...
    mocks.upsertCompanyMock.mockImplementation(async (company) => ({
      id: company.id,
      name: company.name,
    ...
    mocks.findCompanyEmailsMock.mockImplementation(async () => ({
      durationMs: 100,
      emails: [],
      iterationsRun: 0,
      reasoning: null,
    ...
    mocks.upsertJobScanMock.mockImplementation(async (scan) => ({
      id: scan.id,
  ...
  it('uses the preselected route crawler when the queued scan requested it', async () => {
    mocks.isVisionEmailFinderEnabledMock.mockReturnValue(true);
