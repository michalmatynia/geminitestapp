...
import {
  listSocialPublishingImageAddons,
  getSocialPublishingImageAddonById,
  findSocialPublishingImageAddonsByIds,
  upsertSocialPublishingImageAddon,
  findLatestAddonByPresetId,
  updateSocialPublishingImageAddon,
...
describe('social-image-addons-repository (in-memory)', () => {
  ...
  it('upserts and retrieves an addon by id', async () => {
    ...
    const found = await getSocialPublishingImageAddonById('upsert-test-1');
    expect(found).not.toBeNull();
    expect(found?.title).toBe('Test Addon');
  });

  it('returns null for non-existent id', async () => {
    const found = await getSocialPublishingImageAddonById('does-not-exist');
    expect(found).toBeNull();
  });

  it('returns null for empty id', async () => {
    const found = await getSocialPublishingImageAddonById('');
    expect(found).toBeNull();
  });

  it('updates an existing addon on second upsert', async () => {
    const addon = makeAddon({ id: 'upsert-update-1', title: 'Original' });
    await upsertSocialPublishingImageAddon(addon);

    const updated = await upsertSocialPublishingImageAddon({
      ...addon,
      title: 'Updated',
    });
    ...
    const found = await getSocialPublishingImageAddonById('upsert-update-1');
  ...
  it('lists addons sorted by updatedAt descending', async () => {
    ...
    await upsertSocialPublishingImageAddon(addonA);
    // Small delay to ensure different timestamps
    await new Promise((r) => setTimeout(r, 5));
    await upsertSocialPublishingImageAddon(addonB);

    const list = await listSocialPublishingImageAddons(100);
  ...
  it('respects the limit parameter', async () => {
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'limit-1' }));
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'limit-2' }));
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'limit-3' }));

    const list = await listSocialPublishingImageAddons(2);
    expect(list.length).toBeLessThanOrEqual(2);
  });

  it('finds addons by multiple ids', async () => {
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'find-a' }));
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'find-b' }));
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'find-c' }));

    const found = await findSocialPublishingImageAddonsByIds(['find-a', 'find-c']);
  ...
  it('returns empty array for empty ids list', async () => {
    const found = await findSocialPublishingImageAddonsByIds([]);
    expect(found).toEqual([]);
  });

  it('finds latest addon by presetId', async () => {
    await upsertSocialPublishingImageAddon(
  ...
  it('updates an addon via updateSocialPublishingImageAddon', async () => {
    await upsertSocialPublishingImageAddon(makeAddon({ id: 'update-1', title: 'Before' }));

    const updated = await updateSocialPublishingImageAddon('update-1', { title: 'After' });
    expect(updated).not.toBeNull();
    expect(updated?.title).toBe('After');
  });

  it('returns null when updating a non-existent addon', async () => {
    const updated = await updateSocialPublishingImageAddon('ghost', { title: 'Nope' });
