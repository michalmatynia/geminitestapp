...
export function ProductImageManagerUIProvider({ 
  ...
  const externalBaseUrl = settingsStore.get(PRODUCT_IMAGES_EXTERNAL_BASE_URL_SETTING_KEY) ?? DEFAULT_PRODUCT_IMAGES_EXTERNAL_BASE_URL;

  // UI State
  const [debugInfo, setDebugInfo] = useState<DebugInfo | null>(null);
  const [showDebug, setShowDebug] = useState(false);
  const [slotViewModes, setSlotViewModes] = useState<SlotViewMode[]>(Array(imageSlots.length).fill('upload'));
  ...
  const triggerFileManager = useCallback((index: number) => {
  ...
  const handleSlotFileUpload = useCallback((slotIndex: number, files: File[]) => {
    if (files.length === 0) return;
    let nextSearchIndex = slotIndex + 1;
    files.forEach((file, fileIndex) => {
      const targetIndex = fileIndex === 0 ? slotIndex : (() => {
  ...
  const convertSlotToBase64 = useCallback(async (index: number) => {
  ...
  const convertLinkToFile = useCallback(async (index: number) => {
  ...
  const clearVisibleImage = useCallback(async (index: number) => {
    ...
    setSlotViewMode(index, 'upload');
  }, [isSlotImageLocked, imageSlots, handleSlotDisconnectImage, imageBase64s, imageLinks, setImageBase64At, setImageLinkAt, setSlotViewMode, pushDebug, slotImageLockedReason]);
