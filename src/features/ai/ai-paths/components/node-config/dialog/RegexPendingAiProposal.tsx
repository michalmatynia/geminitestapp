    ...
    <div className='rounded-md border border-purple-500/40 bg-purple-500/10 p-3'>
      ...
      {codeSnippets.length > 0 ? (
        <RegexSnippetDisplay
          ...
          selectedSnippetIndex={selectedSnippetIndex}
          onSelectSnippetIndex={onSelectSnippetIndex}
        />
      ) : null}

      {selectedSnippetIndex < 0 || codeSnippets.length === 0 ? (
        <RegexFullResponseDisplay pendingAiRegex={pendingAiRegex} />
      ) : null}
    </div>
  );
}

function RegexProposalHeader({
  snippetCount,
  onAccept,
  onReject,
  selectedSnippetIndex,
    ...
    <div className='flex items-center justify-between gap-2'>
      <div className='flex items-center gap-2'>
        ...
        {snippetCount > 0 ? (
          <span className='text-[10px] text-purple-300'>
            ({snippetCount} code snippet{snippetCount > 1 ? 's' : ''})
          </span>
        ) : null}
      </div>
      <div className='flex gap-2'>
        <Button
          ...
          onClick={onAccept}
        >
          {selectedSnippetIndex >= 0 && snippetCount > 0
...
function RegexFullResponseDisplay({ pendingAiRegex }: { pendingAiRegex: string }): React.JSX.Element {
  return (
