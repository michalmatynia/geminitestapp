...
export function useVectorCanvasInteractions({
  ...
  const toPoint = useCallback((clientX: number, clientY: number): VectorPoint | null => {
  ...
  const hitTestPoint = useCallback((clientX: number, clientY: number): { shapeId: string; pointIndex: number } | null => {
    const rect = getInteractionRect();
    if (!rect) return null;
  ...
  const hitTestShape = useCallback((clientX: number, clientY: number): { shapeId: string } | null => {
    const rect = getInteractionRect();
    if (!rect) return null;
