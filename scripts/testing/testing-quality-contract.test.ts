...
describe('testing quality contract', () => {
  it('keeps the testing quality and integration baseline scripts in package.json', () => {
    expect(packageJson.scripts).toMatchObject({
      ...
      'test:integration:prisma:baseline':
        'node scripts/testing/run-integration-prisma-baseline.mjs',
      'test:integration:prisma:baseline:strict':
        'node scripts/testing/run-integration-prisma-baseline.mjs --strict',
  ...
  it('keeps weekly quality reporting wired to integration baselines and the testing snapshot', () => {
    expect(weeklyWorkflow).toContain('Run Prisma integration baseline');
    expect(weeklyWorkflow).toContain('npm run test:integration:prisma:baseline');
    ...
    expect(weeklyWorkflow).toContain('docs/metrics/integration-prisma-latest.json');
  ...
  it('keeps the main test matrix publishing integration baseline artifacts and docs references', () => {
    expect(testMatrixWorkflow).toContain('Upload Prisma integration report');
    expect(testMatrixWorkflow).toContain('docs/metrics/integration-prisma-latest.json');
    ...
    expect(metricsDocs).toContain('[`integration-prisma-latest.md`](./integration-prisma-latest.md)');
