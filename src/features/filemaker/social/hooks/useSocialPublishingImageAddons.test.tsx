...
describe('useSocialPublishingImageAddons', () => {
  ...
  it('normalizes selected ids into the query key and request params', async () => {
    ...
    expect(apiGetMock).toHaveBeenCalledWith('/api/filemaker/social-image-addons', {
      params: {
        limit: 12,
        ids: 'addon-2,addon-1',
        scope: 'admin',
  ...
  it('loads recent batch capture jobs with the requested limit', async () => {
    ...
    expect(apiGetMock).toHaveBeenCalledWith('/api/filemaker/social-image-addons/batch', {
      params: {
        limit: 3,
      },
      timeout: 60_000,
