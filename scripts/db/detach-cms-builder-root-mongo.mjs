...
const expectedConfirm = 'detach-cms-builder-from-root';
...
const backupRootDir = getArgValue(
  '--backup-dir',
  path.join(repoRoot, 'apps', 'cms-builder-web', 'mongo', 'runtime', 'root-detach-backups')
);
...
const validateTargetCopy = async ({ targetDb, selections, sourceCounts }) => {
  ...
  if (!metadata) {
    fail('Refusing to detach root data before the CMS Builder target database has copy metadata.', {
      expectedMetadata: 'cms_builder_database_metadata/local-cms-builder-db',
      remedy: 'Run npm run mongo:copy-from-root -w @app/cms-builder-web first.',
    });
  }
  ...
  if (Object.keys(shortCollections).length > 0) {
    fail('Refusing to detach root data because the CMS Builder target has fewer matching documents.', {
      shortCollections,
      remedy:
        'Re-run npm run mongo:copy-from-root -w @app/cms-builder-web, then run this detach plan again.',
    });
  }
