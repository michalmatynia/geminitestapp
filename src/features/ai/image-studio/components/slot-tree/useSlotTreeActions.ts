...
export function useSlotTreeActions({
  ...
  const onSelectSlot = useCallback(
    (slot: ImageStudioSlotRecord): void => {
      setSelectedSlotId(slot.id);
    },
    [setSelectedSlotId]
  );

  const resolveOwnerCardSlotId = useCallback(
    (nodeId: MasterTreeId): string | null => {
  ...
  const onSelectCardNode = useCallback(
    (slot: ImageStudioSlotRecord, nodeId: MasterTreeId): void => {
        ...
        onConfirm: () => {
          deleteSlotMutation.mutate(slot.id, {
            onError: (error: unknown) => {
              toast(error instanceof Error ? error.message : 'Failed to delete card.', {
                variant: 'error',
              });
            },
          });
  ...
  const commitFolderRename = useCallback(
    (folderNodeId: MasterTreeId): void => {
      ...
      void onRenameFolder(normalizedSource, nextPath).catch((error: unknown) => {
        toast(error instanceof Error ? error.message : 'Failed to rename folder.', {
          variant: 'error',
        });
      });
    },
  ...
  const startCardRename = useCallback(
    (nodeId: MasterTreeId): void => {
  ...
  const commitCardRename = useCallback(
    (slot: ImageStudioSlotRecord): void => {
      const normalizedName = controller.renameDraft.replace(/[\\/]+/g, ' ').trim();
      if (!normalizedName) {
      ...
      void updateSlot({
        ...
        toast(error instanceof Error ? error.message : 'Failed to rename card.', {
          variant: 'error',
        });
      });
    },
