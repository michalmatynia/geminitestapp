...
import {
  useAmazonSelectorRegistry,
  useDeleteAmazonSelectorRegistryEntryMutation,
  useSaveAmazonSelectorRegistryEntryMutation,
  useSyncAmazonSelectorRegistryMutation,
} from '@/features/integrations/hooks/useAmazonSelectorRegistry';
import type { AmazonSelectorRegistryEntry } from '@/shared/contracts/integrations/amazon-selector-registry';
import { AMAZON_SELECTOR_REGISTRY_PROFILE } from '@/shared/lib/browser-execution/selectors/amazon';

const DEFAULT_PROFILE = AMAZON_SELECTOR_REGISTRY_PROFILE;
...
const collectProfiles = (
  entries: readonly AmazonSelectorRegistryEntry[],
...
export default function AmazonSelectorRegistryPage(): React.JSX.Element {
  const [profile, setProfile] = useState(DEFAULT_PROFILE);
  const [selectedKey, setSelectedKey] = useState<string | null>(null);
  const [draftValueJson, setDraftValueJson] = useState('');
  const [notice, setNotice] = useState<string | null>(null);

  const registryQuery = useAmazonSelectorRegistry({ profile });
  const syncMutation = useSyncAmazonSelectorRegistryMutation();
  const saveMutation = useSaveAmazonSelectorRegistryEntryMutation();
  const deleteMutation = useDeleteAmazonSelectorRegistryEntryMutation();
  ...
  const runHistoryHref = `/admin/playwright/step-sequencer/runs?${new URLSearchParams({
    selectorProfile: AMAZON_SELECTOR_REGISTRY_PROFILE,
  }).toString()}`;
      ...
      <header className='space-y-2'>
        <div className='flex flex-wrap items-center justify-between gap-3'>
          <div>
            ...
            <h1 className='text-2xl font-semibold tracking-tight'>Amazon Selector Registry</h1>
          </div>
        ...
        <p className='max-w-3xl text-sm text-muted-foreground'>
          Seeded Amazon and Google Lens selectors, hints, and extraction signals can be synced into
          MongoDB, overridden per profile, and connected from modular Playwright Step Sequencer
          steps.
        </p>
      </header>
      ...
      <section className='grid gap-4 rounded-lg border border-border bg-card/40 p-4 lg:grid-cols-[320px_minmax(0,1fr)]'>
        <aside className='space-y-3'>
          <div className='space-y-1'>
            ...
            <datalist id='amazon-selector-profiles'>
              {profiles.map((entry) => (
          ...
          {registryQuery.error ? (
            <div className='rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive'>
              {registryQuery.error instanceof Error
                ...
                : 'Failed to load Amazon selector registry.'}
            </div>
          ) : null}
        ...
        <section className='min-w-0 space-y-4'>
          {selectedEntry ? (
            ...
            <div className='rounded-md border border-border bg-background/70 p-8 text-center text-sm text-muted-foreground'>
              {registryQuery.isLoading ? 'Loading Amazon selector registry...' : 'No selector entries available.'}
            </div>
          )}
        </section>
      </section>
