...
import {
  socialPublishingManualGenerationProgressSchema,
  socialPublishingManualPipelineProgressSchema,
  socialPublishingManualVisualAnalysisProgressSchema,
...
const sanitizeJobProgress = (
  ...
  const manualPipeline = socialPublishingManualPipelineProgressSchema.safeParse(progress);
  ...
  const visualAnalysis = socialPublishingManualVisualAnalysisProgressSchema.safeParse(progress);
  if (visualAnalysis.success) {
    return visualAnalysis.data;
  }

  const manualGeneration = socialPublishingManualGenerationProgressSchema.safeParse(progress);
