...
const resolveImageAsset = async (
  asset: SocialPublishingPost['imageAssets'][number]
...
export async function publishLinkedInPersonalPost(
  ...
  try {
    const { repo, connection } = await resolveLinkedInConnection(post.publishingConnectionId);
    if (!connection) {
      throw configurationError(
    ...
    void ErrorSystem.logInfo('Social publishing LinkedIn post published', {
      ...baseContext,
...
export async function deleteLinkedInPersonalPost(post: SocialPublishingPost): Promise<void> {
  const startedAt = Date.now();
  const postReference = post.publishedPostId?.trim() ? post.publishedPostId : post.publishedUrl;
  ...
  try {
    const { connection } = await resolveLinkedInConnection(post.publishingConnectionId);
    if (!connection) {
      throw configurationError(
        'LinkedIn connection is missing. Connect LinkedIn in Admin > Integrations.'
      );
    ...
    void ErrorSystem.logInfo('Social publishing LinkedIn post deleted', {
      ...baseContext,
      durationMs: Date.now() - startedAt,
    });
