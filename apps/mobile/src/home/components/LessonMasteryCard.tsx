...
function CardHeader({ title, insight }: { title: string; insight: KangurMobileHomeLessonMasteryItem }): React.JSX.Element {
  return (
    <>
...
function CardBody({
  ...
  const lastLessonLabel = insight.lastCompletedAt !== null && insight.lastCompletedAt !== ''
...
export function LessonMasteryCard({
  ...
  return (
    ...
    >
      <CardHeader title={title} insight={insight} />
      <CardBody copy={copy} locale={locale} insight={insight} />
      <CardActions copy={copy} insight={insight} />
    </View>
  );
}