/**
 * Social Publishing Generated Draft Utilities
 * 
...
import type { SocialPublishingGeneratedDraft, SocialPublishingPost } from '@/shared/contracts/social-publishing-posts';
...
const extractLooseDraftFields = (text: string): Partial<SocialPublishingGeneratedDraft> => {
  const result: Partial<SocialPublishingGeneratedDraft> = {};
...
): Partial<SocialPublishingGeneratedDraft> => ({
...
const extractDraftFieldsFromText = (text: string): Partial<SocialPublishingGeneratedDraft> => {
...
export const looksLikeSerializedSocialPublishingDraft = (text: string): boolean => {
  const trimmed = text.trim();
  if (!trimmed) {
    return false;
  }
...
export const parseSocialPublishingGeneratedDraftText = (
  text: string
): Partial<SocialPublishingGeneratedDraft> => extractDraftFieldsFromText(text);

export const normalizeSocialPublishingDraftLike = (
  ...
  ].find((value) => looksLikeSerializedSocialPublishingDraft(value));
