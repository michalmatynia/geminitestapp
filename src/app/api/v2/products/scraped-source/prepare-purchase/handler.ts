...
import { prepareScrapedSourcePurchase } from '@/features/products/server/product-scraped-source';
import {
...
export async function postHandler(_req: NextRequest, ctx: ApiHandlerContext): Promise<Response> {
  ...
  const result = await prepareScrapedSourcePurchase(body.productId);
