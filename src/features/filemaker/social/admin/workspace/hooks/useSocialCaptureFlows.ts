...
export function useSocialCaptureFlows({
  ...
  const handleResetProgrammableCaptureScript = useCallback((): void => {
    setProgrammableCaptureScript(SOCIAL_PUBLISHING_DEFAULT_PLAYWRIGHT_CAPTURE_SCRIPT);
  }, []);
  ...
  const handleRetryFailedProgrammableCaptureJob = useCallback(
    async (job: SocialPublishingImageAddonsBatchJob): Promise<void> => {
      ...
      const failedRoutes = resolveFailedSocialPublishingProgrammableCaptureRoutes(
        job.result?.failures ?? [],
        storedRoutes
      );
