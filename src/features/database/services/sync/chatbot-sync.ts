...
import type { MongoChatbotSessionDoc, MongoChatbotJobDoc, MongoChatbotMessageDoc } from '../database-sync-types';
import type { SyncHandler } from './types';

export const syncChatbotSessions: SyncHandler = async ({ mongo, prisma, normalizeId, toDate }) => {
  const docs: MongoChatbotSessionDoc[] = (await mongo.collection('chatbot_sessions').find({}).toArray()) as unknown as MongoChatbotSessionDoc[];
  const sessions = docs
...
export const syncChatbotJobs: SyncHandler = async ({ mongo, prisma, normalizeId, toDate, toJsonValue }) => {
  ...
  const data = docs
    .map((doc): Prisma.ChatbotJobCreateManyInput | null => {
      ...
      return {
        ...
        payload: (toJsonValue ? toJsonValue(doc.payload ?? null) : (doc.payload ?? null)) as Prisma.InputJsonValue,
...
export const syncChatbotSessionsPrismaToMongo: SyncHandler = async ({ mongo, prisma, toObjectIdMaybe }) => {
...
export const syncChatbotJobsPrismaToMongo: SyncHandler = async ({ mongo, prisma, toObjectIdMaybe }) => {
