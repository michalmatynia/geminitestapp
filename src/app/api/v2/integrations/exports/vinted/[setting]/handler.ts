...
} from '@/app/api/v2/integrations/exports/vinted/default-connection/handler';
...
const EXPORT_VINTED_SETTING_HANDLERS: Record<string, RouteHandlers> = {
  'default-connection': {
...
const resolveHandlers = (setting: string): RouteHandlers => {
  const handlers = EXPORT_VINTED_SETTING_HANDLERS[setting];
  if (!handlers) {
    throw notFoundError(`Unknown exports/vinted setting: ${setting}`);
  }
  return handlers;
};
