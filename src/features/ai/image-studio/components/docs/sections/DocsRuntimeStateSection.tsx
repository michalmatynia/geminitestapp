  ...
  return (
    <Card variant='subtle' padding='lg' className='border-border/60 bg-card/40 space-y-3'>
      ...
      <div className='grid gap-2 sm:grid-cols-2 lg:grid-cols-4'>
        <MetadataItem label='Project' value={projectId || 'none'} hint={`${projectsQueryCount} total`} />
        <MetadataItem label='Slots' value={String(slotsLength)} hint={`selected: ${selectedSlotName}`} />
        <MetadataItem label='Working Slot' value={workingSlotName} hint={`folder: ${selectedFolder || 'root'}`} />
        <MetadataItem label='Preview Mode' value={previewMode} hint={`composite: ${compositeAssetsLength}`} />
        <MetadataItem label='Prompt Length' value={String(promptTextLength)} hint={`${promptIssueCount} validation issue(s)`} />
        <MetadataItem label='Params' value={String(paramsStateCount)} hint={`specs: ${paramSpecsCount}`} />
        <MetadataItem label='Mask Shapes' value={String(maskShapesLength)} hint={`${maskEligibleCount} eligible polygon/lasso`} />
        <MetadataItem label='Generation' value={runPending ? 'running' : 'idle'} hint={`${runOutputsLength} output(s), ${generationHistoryLength} history`} />
      </div>
      <div className='grid gap-2 sm:grid-cols-2 lg:grid-cols-4'>
      ...
      <div className='grid gap-2 sm:grid-cols-2 lg:grid-cols-4'>
        ...
        <MetadataItem label='Virtual Folders' value={String(virtualFoldersLength)} hint={`persisted: ${persistedTreeFoldersLength}`} />
        <MetadataItem label='Extraction Review' value={metricValue(extractReviewOpen)} hint={`draft length: ${extractDraftPromptLength}`} />
        <MetadataItem label='Slot UI Drafts' value={`url=${slotImageUrlDraftLength}`} hint={`base64=${slotBase64DraftLength}`} />
      </div>
    </Card>
  );
}
