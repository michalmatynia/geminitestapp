...
export function useCaseResolverPromptExploder({
  ...
  const [promptExploderApplyDiagnostics, setPromptExploderApplyDiagnostics] = useState<CaseResolverPromptExploderApplyUiDiagnostics | null>(null);
  ...
  const markPromptExploderTransferApplied = useCallback((transferId: string | null): void => {
    ...
    if (appliedPromptExploderTransferIdsRef.current.size > CASE_RESOLVER_APPLIED_PROMPT_TRANSFER_IDS_LIMIT) {
  ...
  const observedPromptExploderPayload = pendingPromptExploderPayload ?? expiredPromptExploderPayload;
  ...
  useEffect(() => {
    ...
    if (lastPromptExploderPayloadKeyRef.current === observedPromptExploderPayloadKey && observedPromptExploderPayloadKey !== null) {
      setPromptExploderApplyDiagnostics((current: CaseResolverPromptExploderApplyUiDiagnostics | null): CaseResolverPromptExploderApplyUiDiagnostics | null => (
        current ? { ...current, captureSettingsEnabled: caseResolverCaptureSettings.enabled } : current
      ));
      return;
    }
    lastPromptExploderPayloadKeyRef.current = observedPromptExploderPayloadKey;
    const payloadContextFileId = observedPromptExploderPayload.caseResolverContext?.fileId?.trim() || null;
    const precheckResolvedTargetFileId = resolveCaseResolverFileById(workspace.files, payloadContextFileId)?.id ?? null;
    ...
    const diagnostics: CaseResolverPromptExploderApplyUiDiagnostics = {
      ...
      payloadVersion: typeof observedPromptExploderPayload.payloadVersion === 'number' && Number.isFinite(observedPromptExploderPayload.payloadVersion) ? Math.trunc(observedPromptExploderPayload.payloadVersion) : null,
      ...
      proposalBuilt: false,
      proposalReason: caseResolverCaptureSettings.enabled ? ((observedPromptExploderPayload.caseResolverParties || observedPromptExploderPayload.caseResolverMetadata?.placeDate) ? 'proposal_builder_returned_null' : 'no_capture_payload') : 'capture_disabled',
  ...
  }, [caseResolverCaptureSettings.enabled, expiredPromptExploderPayload, observedPromptExploderPayload, observedPromptExploderPayloadKey, workspaceFileIdsSignature, workspace.files]);
  ...
  useEffect(() => {
    const refreshPayload = (): void => { refreshPendingPromptExploderPayload(); };
    const handleStorage = (event: StorageEvent): void => { if (event.key !== PROMPT_EXPLODER_APPLY_TO_STUDIO_KEY) return; refreshPayload(); };
    const handleBridgeStorageEvent = (): void => { refreshPayload(); };
    const handleFocus = (): void => { refreshPayload(); };
    const handlePageShow = (): void => { refreshPayload(); };
    const handleVisibilityChange = (): void => { if (document.visibilityState !== 'visible') return; refreshPayload(); };
    
    window.addEventListener('storage', handleStorage);
    window.addEventListener(PROMPT_EXPLODER_BRIDGE_STORAGE_EVENT, handleBridgeStorageEvent as EventListener);
    window.addEventListener('focus', handleFocus);
    window.addEventListener('pageshow', handlePageShow);
    document.addEventListener('visibilitychange', handleVisibilityChange);
    return (): void => {
      window.removeEventListener('storage', handleStorage);
      window.removeEventListener(PROMPT_EXPLODER_BRIDGE_STORAGE_EVENT, handleBridgeStorageEvent as EventListener);
  ...
  const handleDiscardPendingPromptExploderPayload = useCallback((): void => {
    ...
    const diagnostics: CaseResolverPromptExploderApplyUiDiagnostics = {
      ...
      payloadVersion: typeof discardedPayload.payloadVersion === 'number' && Number.isFinite(discardedPayload.payloadVersion) ? Math.trunc(discardedPayload.payloadVersion) : null,
      ...
      payloadKey: [discardedPayload.createdAt, discardedPayload.caseResolverContext?.fileId ?? '', discardedPayload.prompt.length].join('|'),
      requestedTargetFileId: payloadContextFileId,
      payloadContextFileId,
      fallbackTargetFileId: null,
      precheckResolvedTargetFileId,
      ...
      captureSettingsEnabled: caseResolverCaptureSettings.enabled,
      proposalBuilt: false,
      proposalReason: caseResolverCaptureSettings.enabled ? ((discardedPayload.caseResolverParties || discardedPayload.caseResolverMetadata?.placeDate) ? 'proposal_builder_returned_null' : 'no_capture_payload') : 'capture_disabled',
      mutationMissingAfterPrecheck: false,
      status: 'discarded',
      reason: null,
      updatedAt: new Date().toISOString(),
  ...
  const handleApplyPendingPromptExploderPayload = useCallback(
    async (): Promise<boolean> => {
      ...
      if (!payload) {
        if (expiredPromptExploderPayload) {
          transitionPromptExploderApplyDiagnostics({ nextStatus: 'expired', reason: 'payload_expired' });
          toast('Pending Prompt Exploder output expired before apply. Discard it and re-send from Prompt Exploder.', { variant: 'warning' });
          refreshPendingPromptExploderPayload();
          return false;
        }
      ...
      if (payloadTransferId.length > 0 && appliedPromptExploderTransferIdsRef.current.has(payloadTransferId)) {
        discardPendingCaseResolverPromptExploderPayload();
        setPromptExploderApplyDiagnostics({
          applyAttemptId: 'already_applied',
          transferId: payloadTransferId,
          payloadVersion: typeof payload.payloadVersion === 'number' && Number.isFinite(payload.payloadVersion) ? Math.trunc(payload.payloadVersion) : null,
          ...
          captureSettingsEnabled: caseResolverCaptureSettings.enabled,
          proposalBuilt: false,
          proposalReason: caseResolverCaptureSettings.enabled ? (payload.caseResolverParties || payload.caseResolverMetadata?.placeDate ? 'proposal_builder_returned_null' : 'no_capture_payload') : 'capture_disabled',
          mutationMissingAfterPrecheck: false,
          status: 'applied',
        ...
        refreshPendingPromptExploderPayload();
        toast('This Prompt Exploder transfer was already applied. Duplicate payload was discarded.', { variant: 'info' });
        return true;
      }
      const requestedContextFileId = requestedFileId?.trim() ?? '';
      const payloadContextFileId = payload.caseResolverContext?.fileId?.trim() ?? '';
      if (shouldOpenEditorFromQuery && requestedContextFileId.length > 0 && payloadContextFileId.length > 0 && payloadContextFileId !== requestedContextFileId) {
        logPromptExploderBindingGuardrailEvent({ mode: 'manual', reason: 'document_mismatch', payload, requestedContextFileId, requestedSessionId: requestedPromptExploderSessionId });
        toast('Pending Prompt Exploder output belongs to a different document. Reopen Prompt Exploder from this document and apply again.', { variant: 'warning' });
        transitionPromptExploderApplyDiagnostics({ nextStatus: 'blocked', reason: 'document_mismatch' });
        return false;
      }
      const payloadSessionId = payload.caseResolverContext?.sessionId?.trim() ?? '';
      const hasPromptSessionMismatch = requestedPromptExploderSessionId.length > 0 && payloadSessionId !== requestedPromptExploderSessionId;
      if (hasPromptSessionMismatch) {
        logPromptExploderBindingGuardrailEvent({ mode: 'manual', reason: 'session_mismatch', payload, requestedContextFileId, requestedSessionId: requestedPromptExploderSessionId });
        toast('Pending Prompt Exploder output belongs to a different editing session. Reopen Prompt Exploder from this document and apply again.', { variant: 'warning' });
        transitionPromptExploderApplyDiagnostics({ nextStatus: 'blocked', reason: 'session_mismatch' });
        return false;
      }
      ...
      setPromptExploderApplyDiagnostics({
        ...
        reason: result.applied ? (result.proposalState ? 'awaiting_capture_mapping' : null) : result.reason,
        updatedAt: new Date().toISOString(),
      });
      ...
      if (!result.applied) {
        const errorMessages: Record<string, string> = {
          ...
          'target_file_locked': 'Target document is locked. Unlock it in Case Resolver before applying output.',
          'missing_context_file_id': 'Prompt Exploder output is missing a target document context.',
          'target_missing_in_live_workspace_after_precheck': 'Target document was resolved in precheck but is unavailable in the live workspace snapshot.',
          'target_file_missing_precheck': 'Cannot resolve target document for Prompt Exploder output.',
          'target_file_missing_mutation_snapshot': 'Cannot resolve target document for Prompt Exploder output.'
        };
        toast(errorMessages[result.reason || ''] || 'No pending Prompt Exploder output to apply.', { variant: (result.reason ? 'warning' : 'info') });
        return false;
      }
      ...
      if (result.proposalState) {
        ...
        if (result.diagnostics.hasPartiesPayload || result.diagnostics.hasMetadataPayload) {
          if (!result.diagnostics.captureSettingsEnabled) {
            toast('Capture data exists, but Case Resolver Capture is disabled in settings.', { variant: 'warning' });
          } else {
            toast(`Capture data exists, but no mapping proposal was generated (${result.diagnostics.proposalReason}).`, { variant: 'warning' });
          }
        } else {
          toast('Applied output has no captured addresser, addressee, or document date.', { variant: 'info' });
        }
      }

      if (result.workspaceChanged) {
        window.setTimeout((): void => { flushWorkspacePersist(); }, 0);
    ...
    [caseResolverCaptureSettings, expiredPromptExploderPayload, filemakerDatabase, logPromptExploderBindingGuardrailEvent, markPromptExploderTransferApplied, pendingPromptExploderPayload, pendingPromptExploderPayloadKey, refreshPendingPromptExploderPayload, requestedFileId, requestedPromptExploderSessionId, setEditingDocumentDraft, shouldOpenEditorFromQuery, toast, transitionPromptExploderApplyDiagnostics, updateWorkspace, workspaceRef, flushWorkspacePersist, setIsApplyingPromptExploderPartyProposal]
  );
