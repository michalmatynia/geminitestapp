...
const { useSocialPostContextMock, useSocialPublishingPostsPageMock } = vi.hoisted(() => ({
  useSocialPostContextMock: vi.fn(),
  useSocialPublishingPostsPageMock: vi.fn(),
}));
...
vi.mock('@/features/filemaker/social/hooks/useSocialPublishingPosts', () => ({
  useSocialPublishingPostsPage: (options: unknown) => useSocialPublishingPostsPageMock(options),
}));
...
const buildPost = () => ({
  id: 'post-1',
  titlePl: 'Social Publishing Update',
  titleEn: 'Social Publishing Update',
  bodyPl: '',
  bodyEn: '',
  combinedBody: '',
...
const resolveListStatus = (post: ReturnType<typeof buildPost>): string =>
  post.status === 'published' || post.publishedAt || post.publishedPostId || post.publishedUrl
    ? 'published'
    : post.status;
...
describe('SocialPostList', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    useSocialPublishingPostsPageMock.mockImplementation(
      (options: {
        page: number;
        pageSize: number;
        search?: string;
  ...
  it('shows the runtime lock reason on published publication menu actions', () => {
    useSocialPostContextMock.mockReturnValue(
      buildSocialPostContextState({
        posts: [
          {
            ...
            publishedPostId: 'linkedin-post-1',
  ...
  it('opens the modal from the post name without row hover zoom treatment', () => {
    ...
    const nameButton = screen.getByRole('button', { name: 'Open social post Social Publishing Update' });
  ...
  it('keeps the Publication indicator and unpublish actions when a published post drifts back to local draft status', () => {
    useSocialPostContextMock.mockReturnValue(
      buildSocialPostContextState({
        posts: [
          {
  ...
  it('keeps unpublish actions enabled when the published post only has a published URL', () => {
    useSocialPostContextMock.mockReturnValue(
      buildSocialPostContextState({
        posts: [
          {
