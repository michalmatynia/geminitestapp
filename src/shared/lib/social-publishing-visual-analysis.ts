export const MAX_SOCIAL_PUBLISHING_VISUAL_SUMMARY_CHARS = 1200;
...
export const sanitizeSocialPublishingVisualSummary = (value: string | null | undefined): string => {
  let sanitized = (value ?? '')
    .replace(/\r\n?/g, '\n')
    .replace(/\*\*/g, '')
    .trim();
  ...
  return truncateText(sanitized, MAX_SOCIAL_PUBLISHING_VISUAL_SUMMARY_CHARS);
};

export const sanitizeSocialPublishingVisualHighlights = (
...
export const normalizeSocialPublishingVisualAnalysis = ({
  summary,
  highlights,
}: {
  summary?: string | null;
  ...
  summary: sanitizeSocialPublishingVisualSummary(summary),
  highlights: sanitizeSocialPublishingVisualHighlights(highlights),
});
