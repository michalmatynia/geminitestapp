...
const GEMINI_WRAPPER_ENV = process.env.GEMINI_WRAPPER || "gemini-iso";
const ISO_HOME =
  process.env.GEMINI_ISO_HOME || path.join(os.homedir(), ".gemini-home");
...
function spawnGemini() {
  ...
  console.error(
    `\n[gemini-preview] Launching model: ${model}` +
      ...
      `[gemini-preview] Wrapper: ${GEMINI_WRAPPER}\n` +
      `[gemini-preview] script passthrough: ${SCRIPT_PASSTHROUGH ? "ON (snappy)" : "OFF (automation, may lag)"}\n`
  );
