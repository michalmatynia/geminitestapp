...
describe('engine-core runtime-kernel telemetry propagation', () => {
  ...
  it('includes canonical runtime-kernel telemetry in blocked/missing-input profile events', async () => {
    ...
    expect(blockedEvent?.['runtimeStrategy']).toBe('code_object_v3');
    ...
    expect(blockedCall?.[0]?.['runtimeStrategy']).toBe('code_object_v3');
