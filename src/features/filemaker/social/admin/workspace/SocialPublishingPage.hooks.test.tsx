...
const createSettingsState = (overrides?: Record<string, unknown>) => ({
  ...
  handlePublishingConnectionChange: settingsHandleLinkedInConnectionChangeMock,
...
describe('useAdminSocialPublishingPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('captures images, patches the draft, and exposes the success message', async () => {
    ...
    const { result } = renderHook(() => useAdminSocialPublishingPage());
  ...
  it('delegates settings telemetry wrappers and reports blocked capture state', async () => {
    ...
    expect(result.current.socialDraftBlockedReason).toContain('Choose a Social Publishing post model');
    ...
    act(() => {
      ...
      result.current.handlePublishingConnectionChange('conn-2');
    });
    ...
    expect(trackSocialPublishingClientEventMock).toHaveBeenCalledWith(
      'social_publishing_post_model_select',
      expect.objectContaining({ nextModelId: 'brain-override' })
    );
    expect(trackSocialPublishingClientEventMock).toHaveBeenCalledWith(
      'social_publishing_post_vision_model_select',
      expect.objectContaining({ nextVisionModelId: 'vision-override' })
    );
    expect(trackSocialPublishingClientEventMock).toHaveBeenCalledWith(
      'social_publishing_post_connection_select',
  ...
  it('resets saved programmable defaults and restores the local advanced-capture draft state', async () => {
    const presetIds = SOCIAL_PUBLISHING_CAPTURE_PRESETS.slice(0, 2).map((preset) => preset.id);
    useSocialSettingsMock.mockReturnValue(
      createSettingsState({
        batchCaptureBaseUrl: 'https://capture.example.com',
    ...
    expect(result.current.programmableCaptureRoutes).toEqual(
      buildSocialPublishingProgrammableCaptureRoutesFromPresetIds(presetIds)
    );
