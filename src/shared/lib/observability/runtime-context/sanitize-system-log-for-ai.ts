...
const sanitizeRuntimeDocument = (value: unknown): ContextRuntimeDocument | null => {
  ...
  return {
    ...
    entityType: typeof record['entityType'] === 'string' ? record['entityType'] : 'runtime_document',
...
const sanitizeContextNode = (value: unknown): ContextNode | null => {
  ...
  return {
    ...
    source: {
      type: 'code',
      ref: typeof asRecord(record['source'])?.['ref'] === 'string' ? String(asRecord(record['source'])?.['ref']) : 'sanitized',
    },
  };
};
