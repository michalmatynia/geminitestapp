...
export function SocialSettingsCaptureTab() {
  ...
  const hasCustomProgrammableScript =
    programmableCaptureDefaultsScript !== SOCIAL_PUBLISHING_DEFAULT_PLAYWRIGHT_CAPTURE_SCRIPT;
  ...
  const lastBatchFailureSummary = batchCaptureResult
    ? buildSocialPublishingCaptureFailureSummary(batchCaptureResult.failures)
    : null;
  const lastBatchPrimaryIssueSummary = batchCaptureResult
    ? buildSocialPublishingCapturePrimaryIssueSummary(batchCaptureResult.captureResults)
    : null;
      ...
      <KangurAdminCard>
        <div className='space-y-3'>
          <div>
            ...
            <div className='text-sm text-muted-foreground'>Create reusable visuals for any Social Publishing post.</div>
          </div>
