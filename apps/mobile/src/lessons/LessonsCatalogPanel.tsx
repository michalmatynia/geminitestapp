...
import { type LessonsCatalogPanelProps } from './lessons-types';
          ...
          return (
            ...
            >
                ...
                <Pressable accessibilityRole='button' onPress={onOpenCatalogLesson} style={{ gap: 10 }}>
                  ...
                  {item.checkpointSummary ? (
                    <InsetPanel gap={6} padding={12} style={{ borderRadius: 18, borderColor: '#bfdbfe', backgroundColor: '#eff6ff' }}>
                      <Text style={{ color: '#1d4ed8', fontSize: 12, fontWeight: '700' }}>{copy({ de: 'Letzter Checkpoint', en: 'Latest checkpoint', pl: 'Ostatni checkpoint' })}</Text>
                      ...
                      <Text style={{ color: '#475569', fontSize: 13, lineHeight: 18 }}>
                        {copy({ de: `Ergebnis ${item.checkpointSummary.lastScorePercent}% • bestes ${item.checkpointSummary.bestScorePercent}%`, en: `Score ${item.checkpointSummary.lastScorePercent}% • best ${item.checkpointSummary.bestScorePercent}%`, pl: `Wynik ${item.checkpointSummary.lastScorePercent}% • najlepszy ${item.checkpointSummary.bestScorePercent}%` })}
                      </Text>
                    </InsetPanel>
                  ) : null}
                </Pressable>
              ...
              <View style={{ flexDirection: 'column', gap: 8 }}>
                ...
                {renderPracticeLink({ href: item.practiceHref, label: `${copy({ de: 'Training starten', en: 'Start practice', pl: 'Uruchom trening' })}: ${item.lesson.title}` })}
              </View>
            </InsetPanel>
          );
        })}
