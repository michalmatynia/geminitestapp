...
const JOB_STATUS_LABELS: Record<SocialPublishingImageAddonsBatchJob['status'], string> = {
  queued: 'Queued',
  running: 'Running',
  completed: 'Completed',
  failed: 'Failed',
...
const resolveJobTotalCount = (job: SocialPublishingImageAddonsBatchJob): number => {
        ...
        <div className='space-y-3'>
          {jobs.map((job) => {
            ...
            const failureSummary = job.result
              ? buildSocialPublishingCaptureFailureSummary(job.result.failures, {
                        ...
                        {progressTargets.map((target) => (
                          ...
                          >
                            ...
                            <div className='mt-1 text-muted-foreground'>
                              {resolveSocialPublishingCaptureTargetLabel(target.id, resolvedRoutes)}
                            </div>
                            {target.status ? (
                              <div className='mt-1 text-muted-foreground'>
                  ...
                  <div className='space-y-2'>
                    ...
                    <div className='space-y-2'>
                      {captureResults.map((result) => {
                        const targetLabel =
                          result.title?.trim() ||
                          resolveSocialPublishingCaptureTargetLabel(result.id, resolvedRoutes);
