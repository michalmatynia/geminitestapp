...
* Move Exploder into it's own TAB, and then in a sub tab Image Studio add rules to image Studio and tag them as Image Studio Rules

If iit's a logical operator I weant a dropdown (with all logical operator options)
...
END"
Prompt explosions and reassembly are still faulty. In Reassembly prompt "ROLE" Heading is exported twice, PARAMS Heading is exported twice. Adjust the Validation PAtterns in UI only (no hardcoding) to avoid such duplication. In "P0 — PRODUCT INTEGRITY (ABSOLUTE)
" Segment, the line"  Rule: if unsure whether a mark is real vs artifact => KEEP (unless remove_uncertain_marks=true)." is omitted, allow for additinoal rules in lists, Adjust Validation Patterns (in UI only) to account for that. In Segment "B) SHADOW (ONE ONLY, COMPACT UNDER-BASE)", "If add_new_ground_shadow=true: add ONE neutral gray soft shadow under product base." is added twice, to the subheading and to the list. It's part of the list, adjust Validation Patterns in UI only to account for that. Next, "C) COMPOSITION" segment has the following subheading "if enabled; if allow_intentional_offcenter=true, keep intentional premium off-center", which is actually a part of Compositoin list, second point of the list. 
Next "E) DETAILS" Has a subheading "if enabled; avoid logos/text/patterns/stitching/texture" whoich should be a part of its list point number one * Remove only obvious photo/handling artifacts (dust/lint/sensor spots/smudges clearly not part of product) if enabled; avoid logos/text/patterns/stitching/texture" adjust Validation Patterns in UI only to account for that.
In "=== STUDIO RELIGHTING EXTENSION (COMPACT, PROGRAMMABLE) ===" , "RELIGHTING RULES (ONLY ACTIVE WHEN apply_studio_relighting=true):" is a general Section rule, account for that. In "RL0 (Mandatory): Produce a new studio lighting look that is clearly different from the original capture lighting.
" , (Mandatory) is the name of the subsection, account for that in Validation Patterns. "Produce a new studio lighting look that is clearly different from the original capture lighting." is the section heading / guiding rule 

In "RL1 (Dramatic but real): Increase dimensionality using directional key + controlled fill.
" , (Dramatic but real) is the name of the subsection, account for that. ", "Increase dimensionality using directional key + controlled fill.
" , (Dramatic but real) is the name of the subsection, account for that. " is the section heading / guiding rule 

 In "RL2 (Color/material integrity): Relighting must NOT change:" , (Color/material integrity) is the name of the subsection, account for that. "Relighting must NOT change:" is the section heading / guiding rule .
  
 In "RL3 (Specular control): If the product is reflective:" , (Specular control) is the name of the subsection, account for that. "If the product is reflective:" is the section IF CONDITION, which is different that heading / guiding rule.
 
  In "RL4 (Shadow coherence): If new lighting direction is applied:" , (Shadow coherence) is the name of the subsection, account for that. "If new lighting direction is applied:" is the section IF CONDITION, which is different that heading / guiding rule.
  
  
  In "RL5 (Rim light use): If relight.add_rim_light=true:" , (Rim light use) is the name of the subsection, account for that.
"If relight.add_rim_light=true:" is the section IF CONDITION, which is different that heading / guiding rule. Also it is Referential , as it refers to the parameter and som the rule should have a logical operator dropdown inserted (IF) and the following parameter should be enclosed in a blocks that searchers parameters. 

PIPELINE is a mess, it should be HIERARCHICAL LIST list the Category LIST, use folder tree master component to show that. 
 The place where it says " RGB(255,255,255)" it should be turned into a color picker, where I can instantly pick different color if I want to. STEP 4 has been exploded into its own section for no reasonm it's a part of PIPELINE.

 IN QA, it's also HIERARCHICAL LIST and in  "QA_R1 Relighting Applied:
* PASS if lighting direction/contrast/highlight placement is noticeably different vs original AND remains photorealistic.
* FAIL if it looks like only global exposure/contrast was changed." , "* PASS if lighting direction/contrast/highlight placement is noticeably different vs original AND remains photorealistic.
* FAIL if it looks like only global exposure/contrast was changed."  are not separate list elements, but are children of "QA_R1 Relighting Applied:" Rule. Same in this one "QA_R2 Relighting Coherence:
* PASS if ground shadow direction/offset matches the new key light direction and looks grounded, while remaining compact under the base.
* FAIL if shadow conflicts with highlights, becomes a long cast trail, or if rim looks like a halo." and in this one "QA_R3 Material/Hue Preservation:
* PASS if finish class (matte/gloss), texture realism, and hue remain consistent with original product.
* FAIL if “CG/plastic” look, invented texture, or hue shift appears."
