...
import type { ProductCategory, ProductParameter } from '@/shared/types/domain/products';
...
describe('products ai-paths description-context handler', () => {
  ...
  it('returns parameters and categories by default for a catalog', async () => {
    ...
    const request = new Request(
      'http://localhost/api/products/ai-paths/description-context?catalogId=catalog-1&categoryId=cat-2'
    );

    const response = await GET_handler(
      request as Parameters<typeof GET_handler>[0],
      { query: { catalogId: 'catalog-1', categoryId: 'cat-2', includeCategories: true } } as Parameters<
        typeof GET_handler
      >[1]
    );
  ...
  it('keeps categoryName resolution when includeCategories is disabled', async () => {
    ...
    const request = new Request(
      'http://localhost/api/products/ai-paths/description-context?catalogId=catalog-1&categoryId=cat-1&includeCategories=false'
    );

    const response = await GET_handler(
      request as Parameters<typeof GET_handler>[0],
      { query: { catalogId: 'catalog-1', categoryId: 'cat-1', includeCategories: false } } as Parameters<
        typeof GET_handler
      >[1]
    );
  ...
  it('returns an empty payload when catalogId is missing', async () => {
    const request = new Request(
      'http://localhost/api/products/ai-paths/description-context?categoryId=cat-1'
    );

    const response = await GET_handler(
      request as Parameters<typeof GET_handler>[0],
      { query: { catalogId: undefined, categoryId: 'cat-1', includeCategories: true } } as Parameters<
        typeof GET_handler
      >[1]
    );
