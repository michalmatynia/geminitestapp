...
export function VintedQuickListButton(props: {
  ...
  const { resolveConnection } = useVintedQuickExportConnection(product.id);
  const {
    localFeedback,
    localFeedbackStatus,
    setFeedbackStatus,
  ...
  const handleClick = useCallback(async (): Promise<void> => {
    ...
    try {
      const resolvedContext: ResolvedVintedQuickListContext = await resolveConnection();
      const resolvedConnection = resolvedContext.vintedConnection;
      ...
      toast(
        response.queue?.jobId
          ? `Vinted listing queued (job ${response.queue.jobId}).`
  ...
  const resolvedLabel = isFailureState
    ? `Open Vinted recovery options (${resolvedButtonStatus}).`
  ...
  return (
    <Button
      ...
      title={`${resolvedLabel} (${resolvedButtonStatus}${vintedStatus ? ` / ${vintedStatus}` : ''})`}
      className={cn(
