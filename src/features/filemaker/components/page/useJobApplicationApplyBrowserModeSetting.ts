...
type JobApplicationApplyBrowserModeSetting = {
  action: JobApplicationApplyActionConnection | null;
...
export const useJobApplicationApplyBrowserModeSetting = (
  enabled: boolean
): JobApplicationApplyBrowserModeSetting => {
  ...
  const runtimeHeadless = useMemo(
    () => resolveJobApplicationApplyHeadless(actionsQuery.data),
    [actionsQuery.data]
  );
  const action = useMemo(
    () => resolveJobApplicationApplyActionConnection(actionsQuery.data),
    [actionsQuery.data]
  );
  ...
  const persist = useCallback(async (): Promise<void> => {
    ...
    await saveActions.mutateAsync({
      actions: updateJobApplicationApplyHeadlessAction({
        actions,
