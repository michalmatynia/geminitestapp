...
import { sanitizeSocialPublishingPromptText } from '@/features/filemaker/social/project-url';
...
export const buildKangurDocContext = async (
  ...
  const [excerptEntries, recentFeaturesContext] = await Promise.all([
    ...
    entries.length > 0 ? loadRecentFeaturesContext() : Promise.resolve(null),
  ]);
  ...
  const sanitizedContext = sanitizeSocialPublishingPromptText(combined);
