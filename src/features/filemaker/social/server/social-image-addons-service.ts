...
import { upsertSocialPublishingImageAddon } from './social-image-addons-repository';
...
const resolvePlaywrightStorageState = (params: {
  ...
  if (resolved.droppedCookieNames.length > 0) {
    logger.warn('[social-publishing.image-addons] dropped invalid Playwright cookies', {
...
export async function createSocialPublishingImageAddonFromPlaywright(
  ...
  try {
    ...
    void ErrorSystem.logInfo('Social publishing image add-on created', {
      service: 'social-publishing.image-addons',
      action: 'create',
