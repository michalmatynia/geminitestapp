...
Generated at: 2026-03-09T08:55:19.838Z
Node: v24.3.0

This is a retained historical weekly-quality snapshot. For current repo status,
use `docs/metrics/weekly-quality-latest.md`, `docs/metrics/weekly-quality-trend-latest.md`,
and `docs/metrics/trend-index-latest.md`.
...
| --- | --- | ---: | ---: | --- |
| Build | PASS | 2.4m | 0 | `npm run build` |
| Lint | SKIPPED | 0ms | - | `npm run lint` |
| Lint Domain Gate | PASS | 2.6m | 0 | `node scripts/quality/run-lint-domain-checks.mjs --include-test-tree --strict --ci --no-history` |
| Typecheck | PASS | 35.5s | 0 | `npm run typecheck` |
| Critical Flow Gate | PASS | 23.2s | 0 | `npm run test:critical-flows:strict -- --ci --no-history` |
...
| Architecture Guardrails | FAIL | 3.5s | 1 | `node scripts/architecture/check-guardrails.mjs` |
| UI Consolidation Guardrail | FAIL | 2.8s | 1 | `node scripts/architecture/check-ui-consolidation.mjs` |
| Observability Check | PASS | 1.3s | 0 | `npm run observability:check` |
| Unsafe Patterns | PASS | 970ms | 0 | `node scripts/quality/check-unsafe-patterns.mjs --ci --no-history` |
| Import Boundaries | PASS | 389ms | 0 | `node scripts/quality/check-import-boundaries.mjs --ci --no-history` |
| API Input Validation | PASS | 197ms | 0 | `node scripts/quality/check-api-input-validation.mjs --ci --no-history` |
| Context Health | PASS | 542ms | 0 | `node scripts/quality/check-context-health.mjs --ci --no-history` |
| Timer Cleanup | PASS | 296ms | 0 | `node scripts/quality/check-timer-cleanup.mjs --ci --no-history` |
| Test Distribution | PASS | 279ms | 0 | `node scripts/quality/check-test-distribution.mjs --ci --no-history` |
