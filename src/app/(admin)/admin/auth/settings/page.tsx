...
} from "@/lib/constants/auth-security";
...
export default function AuthSettingsPage() {
  ...
  useEffect(() => {
    ...
    const loadSecurity = async () => {
      try {
        const res = await fetch(`/api/auth/users/${session.user.id}/security`, {
          cache: "no-store",
        });
        if (!res.ok) return;
  ...
  const handleMfaSetup = async () => {
    try {
      ...
      const payload = (await res.json()) as { secret?: string; otpauthUrl?: string };
  ...
  const handleMfaDisable = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/auth/mfa/disable", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token: mfaDisableCode.trim(), recoveryCode: mfaDisableCode.trim() }),
      });
