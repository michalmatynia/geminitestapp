...
import {
  buildSocialPublishingProgrammableCaptureInputPreview,
...
describe('social Playwright capture presets', () => {
  it('includes direct-launch minigame presets for Social capture', () => {
    expect(SOCIAL_PUBLISHING_CAPTURE_PRESETS).toEqual(
  ...
  it('builds programmable capture routes and preview URLs for minigame presets', () => {
    const routes = buildSocialPublishingProgrammableCaptureRoutesFromPresetIds([
      'clock-quiz',
      'calendar-quiz',
      'geometry-quiz',
    ]);
    ...
    expect(
      buildSocialPublishingProgrammableCaptureInputPreview(routes, 'https://example.com')
    ).toEqual([
      expect.objectContaining({
        id: 'clock-quiz',
        title: 'Clock Quiz',
