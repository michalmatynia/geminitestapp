...
/**
 * Ensures the first session is selected if none are currently selected.
 */
function initializeFirstSession(currentSessionId: string | null, fetchedSessions: ChatSession[], setCurrentSessionId: (id: string | null) => void): void {
...
/**
 * Manages chatbot sessions, including fetching, creating, and deleting sessions.
 */
export function useChatbotSessions(params: ChatbotSessionsParams): ChatbotSessionsResult {
  ...
  const handleSessionsFetchResult = useCallback((fetchedSessions: ChatSession[]): void => {
    ...
    initializeFirstSession(currentSessionId, fetchedSessions, setCurrentSessionId);
  ...
  const fetchSessions = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      handleSessionsFetchResult(res.sessions ?? []);
    } catch (e: unknown) {
  ...
  }, [currentSessionId, fetchSessions, sessions, setCurrentSessionId, toast]);
