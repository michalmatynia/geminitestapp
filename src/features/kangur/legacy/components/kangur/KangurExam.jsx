...
import { getKangurQuestions } from './kangurQuestions';
import {
  Q1Illustration,
  Q2Illustration,
  Q3Illustration,
...
function ExamQuestion({ q, qIndex, total, selected, onSelect }) {
...
function ExamSummary({ questions, answers, onBack }) {
          ...
          {q.choices.map((choice, i) => {
            ...
            return (
              ...
              >
                ...
                {choice === userAnswer && choice !== q.answer && (
        ...
        {questions.map((q, i) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-[10px] font-bold ${skipped ? 'text-gray-400' : correct ? 'text-green-700' : 'text-red-600'}`}
              >
...
export default function KangurExam({ mode, onBack }) {
  ...
  const handleSelect = (choice) => {
    setAnswers((prev) => ({ ...prev, [questions[current].id]: choice }));
  };
  ...
  if (finished) {
    return <ExamSummary questions={questions} answers={answers} onBack={onBack} />;
  }
