...
export function FilemakerSummaryBadges(): React.JSX.Element {
  ...
  return (
    <div className='flex flex-wrap gap-2'>
      <Badge variant='outline' className='text-[10px]'>Persons: {database.persons.length}</Badge>
      <Badge variant='outline' className='text-[10px]'>Organizations: {database.organizations.length}</Badge>
      <Badge variant='outline' className='text-[10px]'>Events: {database.events.length}</Badge>
