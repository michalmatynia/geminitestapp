...
const AI_PATHS_RUNTIME_KERNEL_NODE_TYPES_KEY = 'ai_paths_runtime_kernel_node_types';
...
test.describe('AI Paths runtime kernel settings', () => {
  test('persists runtime-kernel node types and strict native registry from Canvas controls', async ({
    page,
  }) => {
    ...
    await page.route('**/api/ai-paths/settings**', async (route) => {
      ...
      if (method === 'GET') {
        ...
        const isRuntimeKernelKeysRequest =
          keys.includes(AI_PATHS_RUNTIME_KERNEL_NODE_TYPES_KEY) &&
    ...
    const strictSelect = page
      ...
      .first();
    const nodeTypesInput = page.locator('[data-doc-id="canvas_runtime_kernel_node_types"]').first();
    const applyButton = page.locator('[data-doc-id="canvas_runtime_kernel_apply"]').first();

    await expect(strictSelect).toBeVisible();
    await expect(nodeTypesInput).toBeVisible();
    await expect(applyButton).toBeVisible();
    await expect(nodeTypesInput).toHaveValue('constant, template');

    await nodeTypesInput.fill('constant, math, template');
    ...
    expect(getSettingValue(persistedUpdates[0]!, AI_PATHS_RUNTIME_KERNEL_NODE_TYPES_KEY)).toBe(
