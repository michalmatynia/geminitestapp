...
test.describe('Notes Advanced E2E', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    themes = [
      { id: 'theme-1', name: 'Dark Blue', backgroundColor: '#000033', textColor: '#ffffff' },
    ];
    ...
    await page.route('**/api/auth/session', (route) =>
      route.fulfill({
        json: {
          user: { id: 'u-1', name: 'Test User', role: 'super_admin' },
          expires: '2099-01-01T00:00:00.000Z',
        },
      })
    );
    await page.route('**/api/notes/notebooks', (route) => route.fulfill({ json: notebooks }));
    await page.route(/\/api\/notes\/categories\/tree.*/, (route) =>
      route.fulfill({ json: categories.map((c) => ({ ...c, children: [], notes: [] })) })
    );
