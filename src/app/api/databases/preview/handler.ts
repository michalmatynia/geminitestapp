...
export async function postDatabasesPreviewHandler(
  ...
  if (previewType === 'mongodb') {
    ...
    safePageSize = Math.min(200, Math.max(1, Number.isFinite(pageSize) ? Number(pageSize) : 20));
    ...
    try {
      ...
      const rowsResults = await Promise.all(
        collections.map(async (collectionName: string) => {
          ...
          const rows = await collection.find({}).skip(offset).limit(safePageSize).toArray();
  ...
  if (previewMode === 'backup') {
    ...
    let stderr: string;
    try {
      const result = await pgExecFileAsync(getPgRestoreCommand(), ['--list', backupPath]);
  ...
  if (previewMode === 'backup') {
    ...
    for (const line of lines) {
      const type = knownTypes.find((candidate: string) => line.includes(` ${candidate} `));
      if (!type) continue;
  ...
  const adminClient =
    previewMode === 'backup' ? new Client({ connectionString: adminUrl.toString() }) : null;
  ...
  try {
    ...
    if (tables.length > 0) {
      ...
      const rowMap = new Map(
        estimateRows.rows.map((row: { relname: string; reltuples: number }) => [
          row.relname,
    ...
    const uniqueMap = new Map(
      uniqueIndexResult.rows.map((r: { indexname: string; indisunique: boolean }) => [
        r.indexname,
        r.indisunique,
      ])
    );
    ...
    enumTypes = Array.from(enumMap.entries()).map(([name, values]: [string, string[]]) => ({
      name,
      values,
    }));
    ...
    const sizeMap = new Map(
      sizeResult.rows.map(
        (r: { tablename: string; size_bytes: string; size_formatted: string }) => [
          r.tablename,
    ...
    const rowEstimateMap = new Map(
      tableStats.map((t: { name: string; rowEstimate: number }) => [t.name, t.rowEstimate])
    );
    ...
    for (const table of tables) {
      ...
      const rowsResult = await previewClient.query(`SELECT * FROM "${table}" LIMIT $1 OFFSET $2`, [
        safePageSize,
        offset,
      ]);
