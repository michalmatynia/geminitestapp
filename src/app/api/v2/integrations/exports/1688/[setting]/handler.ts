...
const EXPORT_1688_SETTING_HANDLERS: Record<string, RouteHandlers> = {
  'default-connection': {
...
const resolveHandlers = (setting: string): RouteHandlers => {
  const handlers = EXPORT_1688_SETTING_HANDLERS[setting];
  if (!handlers) {
    throw notFoundError(`Unknown exports/1688 setting: ${setting}`);
  }
  return handlers;
};
