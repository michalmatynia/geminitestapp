...
export const syncAiPathRuns: SyncHandler = async ({ mongo, prisma, normalizeId, toDate, toJsonValue }) => {
  ...
  const data = docs
    .map((doc: Record<string, unknown>): Prisma.AiPathRunCreateManyInput | null => {
      ...
      return {
        ...
        triggerContext: toJsonValue((doc as { triggerContext?: unknown }).triggerContext ?? null),
        graph: toJsonValue((doc as { graph?: unknown }).graph ?? null),
        runtimeState: toJsonValue((doc as { runtimeState?: unknown }).runtimeState ?? null),
...
export const syncAiPathRunNodes: SyncHandler = async ({ mongo, prisma, normalizeId, toDate, toJsonValue }) => {
  ...
  const data = docs
    .map((doc: Record<string, unknown>): Prisma.AiPathRunNodeCreateManyInput | null => {
      ...
      return {
        ...
        inputs: toJsonValue((doc as { inputs?: unknown }).inputs ?? null),
        outputs: toJsonValue((doc as { outputs?: unknown }).outputs ?? null),
        errorMessage: (doc as { errorMessage?: string | null }).errorMessage ?? null,
        createdAt: (doc as { createdAt?: Date }).createdAt ?? new Date(),
        updatedAt: (doc as { updatedAt?: Date }).updatedAt ?? new Date(),
        startedAt: toDate((doc as { startedAt?: Date | string | null }).startedAt),
