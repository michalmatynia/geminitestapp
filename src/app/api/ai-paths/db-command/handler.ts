...
export async function postAiPathsDbActionHandler(
  ...
  const runActionWithProvider = async (provider: DbProvider): Promise<Record<string, unknown>> => {
    ...
    if (provider === 'prisma') {
      ...
      const where = coerceQuery(filter || query);
      ...
      if (action === 'updateOne') {
        const flatUpdates = extractFlatUpdates(update || updates);
      ...
      if (action === 'replaceOne') {
        const replacement = normalizeReplaceDoc(update || updates);
        if (!replacement) {
    ...
    const normalizedFilter = normalizeObjectId(coerceQuery(filter || query), idType);
    ...
    if (action === 'findOneAndUpdate') {
      const updateDoc = normalizeUpdateDoc(update || updates);
      if (!updateDoc) {
