...
const isProductValidationInstanceScope = (
  ...
  value === 'draft_template' || value === 'product_create' || value === 'product_edit';
...
export const normalizeProductValidationLaunchScopeBehavior = (
  value: unknown
): ProductValidationLaunchScopeBehavior => (value === 'condition_only' ? 'condition_only' : 'gate');

export const normalizeProductValidationSkipNoopReplacementProposal = (value: unknown): boolean =>
  value !== false;

export const normalizeProductValidationDenyBehavior = (
  value: unknown
): ProductValidationDenyBehavior => (value === 'ask_again' ? 'ask_again' : 'mute_session');
