...
const {
  getSocialPublishingPipelineQueueMock,
  getHealthStatusMock,
  getQueueMock,
  getJobsMock,
  isPausedMock,
  ...
  getSocialPublishingPipelineWorkerHeartbeatMock,
} = vi.hoisted(() => ({
  getSocialPublishingPipelineQueueMock: vi.fn(),
  ...
  getSocialPublishingPipelineWorkerHeartbeatMock: vi.fn(),
}));

vi.mock('@/features/filemaker/social/workers/socialPublishingPipelineQueue', () => ({
  getSocialPublishingPipelineQueue: (...args: unknown[]) =>
    getSocialPublishingPipelineQueueMock(...args),
  getSocialPublishingPipelineWorkerHeartbeat: (...args: unknown[]) =>
    getSocialPublishingPipelineWorkerHeartbeatMock(...args),
  SOCIAL_PUBLISHING_PIPELINE_REPEAT_EVERY_MS: 600000,
  SOCIAL_PUBLISHING_PIPELINE_WORKER_HEARTBEAT_TTL_MS: 120000,
}));
...
describe('social pipeline status handler', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getSocialPublishingPipelineQueueMock.mockReturnValue({
    ...
    getSocialPublishingPipelineWorkerHeartbeatMock.mockResolvedValue(null);
  });

  it('reports the queue as running while work is actively processing even if the local worker flag is false', async () => {
    const response = await getHandler(
      new NextRequest('http://localhost/api/filemaker/social-pipeline/status'),
      createContext()
    );
  ...
  it('upgrades offline worker state to idle when a fresh shared worker heartbeat exists', async () => {
    ...
    getSocialPublishingPipelineWorkerHeartbeatMock.mockResolvedValueOnce(heartbeatAt);
