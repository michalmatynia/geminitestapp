...
export function BrainCatalogNodeItem(props: BrainCatalogNodeItemProps): React.JSX.Element {
  ...
  const { onEdit, onRemove } = useBrainCatalogNodeItemRuntime();
...
function BrainCatalogNodeItemContent({
  node,
  entry,
  select,
}: {
  ...
  const poolLabel = BRAIN_CATALOG_POOL_LABELS[entry.pool];
  ...
  return (
    <div className='flex h-full w-full min-w-0 items-center gap-1.5 text-left'>
      ...
      <BrainCatalogNodeItemActions node={node} entry={entry} />
    </div>
  );
}

function BrainCatalogNodeItemActions({
  node,
  entry,
}: {
  node: MasterTreeNode;
