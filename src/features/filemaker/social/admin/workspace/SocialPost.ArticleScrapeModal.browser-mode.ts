...
import {
  type ArticleScrapeActionConnection,
  resolveArticleScrapeActionConnection,
  resolveArticleScrapeHeadless,
  updateArticleScrapeHeadlessAction,
} from './SocialPost.ArticleScrapeModal.runtime-settings';
...
export type ArticleScrapeBrowserModeSetting = {
  action: ArticleScrapeActionConnection | null;
...
export const useArticleScrapeBrowserModeSetting = (
  enabled: boolean
): ArticleScrapeBrowserModeSetting => {
  ...
  const runtimeHeadless = useMemo(
    () => resolveArticleScrapeHeadless(actionsQuery.data),
    [actionsQuery.data]
  );

  const action = useMemo(
    () => resolveArticleScrapeActionConnection(actionsQuery.data),
    [actionsQuery.data]
  );
  ...
  const persist = useCallback(async (): Promise<void> => {
    ...
    await saveActions.mutateAsync({
      actions: updateArticleScrapeHeadlessAction({ actions, headless: draft.value }),
    });
    setDraft((current) => ({
