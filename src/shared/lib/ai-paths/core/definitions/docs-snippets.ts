export const DOCS_WIRING_SNIPPET = [
  ...
  'Fetcher.context → ContextFilter.context',
  'ContextFilter.entityJson → Parser.entityJson',
  'Fetcher.context → ResultViewer.context',
  'Fetcher.meta → ResultViewer.meta',
...
export const DOCS_OVERVIEW_SNIPPET = [
  ...
  '  - Local: runs execute in the browser using the runtime engine.',
  '- Execution Flow (visual):',
  '  - Off / Low / Medium / High control wire animation intensity only.',
  '  - Flow does not change execution order, timing, or outputs.',
  '- Run Mode (local):',
  '  - Block: ignore new trigger clicks while a run is in progress.',
  '  - Queue: enqueue additional runs and execute them sequentially.',
  '  - If the active path changes, queued runs for the old path are skipped.',
  '',
...
export const DOCS_DESCRIPTION_SNIPPET = [
  'ContextFilter.entityJson → Parser.entityJson',
  'Parser.title → Prompt.title',
  'Parser.images → Prompt.images',
