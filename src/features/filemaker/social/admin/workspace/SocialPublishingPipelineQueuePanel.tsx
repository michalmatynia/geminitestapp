...
export function SocialPublishingPipelineQueuePanel({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (isCompactVariant) {
        const statusData = await api.get<PipelineStatus>('/api/filemaker/social-pipeline/status', {
          timeout: QUEUE_PANEL_REQUEST_TIMEOUT_MS,
        });
        ...
        const [statusData, jobsData] = await Promise.all([
          api.get<PipelineStatus>('/api/filemaker/social-pipeline/status', {
            timeout: QUEUE_PANEL_REQUEST_TIMEOUT_MS,
          }),
          api.get<PipelineJobRecord[]>('/api/filemaker/social-pipeline/jobs', {
            timeout: QUEUE_PANEL_REQUEST_TIMEOUT_MS,
          }),
        ]);
        setStatus(statusData);
  ...
  const handleDeleteJob = useCallback(
    async (jobId: string) => {
      if (
        !window.confirm(
          'Delete this Social Publishing pipeline job from the queue history? This cannot be undone.'
        )
      ) {
        return;
      }
        ...
        <div className='flex items-center justify-between'>
          <div>
            <div className='text-sm font-semibold text-foreground'>
              Social Publishing Runtime Queue
            </div>
            <div className='text-xs text-muted-foreground'>
              Redis-backed runtime for screenshot capture, image analysis, and post generation across Social Publishing.
            </div>
            {activeProcessSummary ? (
