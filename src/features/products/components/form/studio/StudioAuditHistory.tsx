...
export function StudioAuditHistory(): React.JSX.Element {
  ...
  return (
    <FormSection title='Run History & Audit' description='Route decisions, mode resolution, fallback reasons, and processing timings.'>
      <div className='flex flex-wrap items-center gap-2'>
        <Button size='xs' variant='outline' onClick={() => void refreshAudit()} disabled={auditLoading} loading={auditLoading}>Refresh Audit</Button>
      </div>

      {auditError && <Alert variant='error' className='mt-2 py-2 text-xs'>{auditError}</Alert>}
          ...
          {auditEntries.map((entry) => (
            <div key={entry.id} className='rounded border border-border/60 bg-card/20 p-2 text-xs text-gray-300'>
              <div className='flex flex-wrap items-center gap-x-3 gap-y-1'>
                ...
                <span className={entry.status === 'completed' ? 'text-emerald-300' : 'text-red-300'}>{entry.status.toUpperCase()}</span>
              ...
              <div className='mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-gray-400'>
                ...
                {entry.errorMessage && <span className='text-red-300'>error: {entry.errorMessage}</span>}
              </div>
            </div>
          ))}
        </div>
