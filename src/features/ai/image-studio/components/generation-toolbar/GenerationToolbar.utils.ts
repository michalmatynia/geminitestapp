...
export const formatLayoutPercent = (value: number): string => String(Number(value.toFixed(2)));
...
export const describeSchemaValidationIssue = (issues: unknown[]): string => {
  ...
  const path =
    firstIssue.path.length > 0 ? firstIssue.path.map((part) => String(part)).join('.') : 'payload';
