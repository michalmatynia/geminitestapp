...
export const syncProducts: DatabaseSyncHandler = async ({ mongo, prisma, normalizeId, toDate }) => {
  ...
  const deleted = (await prisma.product.deleteMany()) as BatchCountResult;

  const productData = data.map(
    ({ images: _i, catalogs: _cat, categories: _c, tags: _t, producers: _p, ...rest }) => rest
  );
  const created: BatchCountResult = productData.length
...
export const syncProductDrafts: DatabaseSyncHandler = async ({ mongo, prisma, normalizeId, toDate }) => {
  ...
  const data = docs
    .map((doc: Record<string, unknown>): ProductDraftSeed | null => {
      ...
      return {
        ...
        defaultPriceGroupId:
          (doc as { defaultPriceGroupId?: string | null }).defaultPriceGroupId ?? null,
        active: (doc as { active?: boolean | null }).active ?? true,
        icon: (doc as { icon?: string | null }).icon ?? null,
        iconColorMode:
          (doc as { iconColorMode?: string | null }).iconColorMode === 'custom'
            ? 'custom'
            : 'theme',
        iconColor:
          typeof (doc as { iconColor?: string | null }).iconColor === 'string' &&
          /^#[0-9a-fA-F]{6}$/.test(
            ((doc as { iconColor?: string | null }).iconColor as string).trim()
          )
  ...
  const deleted = (await prisma.productDraft.deleteMany()) as BatchCountResult;
