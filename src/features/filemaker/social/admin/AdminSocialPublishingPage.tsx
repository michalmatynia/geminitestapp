...
export type SocialPublishingAdminBreadcrumb = {
  href?: string;
  label: string;
};

export type SocialPublishingAdminPageCopy = {
  breadcrumbs: SocialPublishingAdminBreadcrumb[];
  settingsSubtitle: string;
  title: string;
  unpublishLocalScopeLabel: string;
};

const FILEMAKER_SOCIAL_PUBLISHING_COPY: SocialPublishingAdminPageCopy = {
  breadcrumbs: [
  ...
  settingsSubtitle:
    'Choose social publishing models from the AI Brain catalog and manage project references.',
...
function AdminSocialPublishingPageContent({
  copy,
}: {
  copy: SocialPublishingAdminPageCopy;
}): React.JSX.Element {
  const {
    activePost,
    isSettingsModalOpen,
    ...
    >
      ...
      >
        ...
        <div className='space-y-6'>
          ...
          <SocialPublishingPipelineQueuePanel variant='compact' />
      ...
      <ConfirmModal
        ...
        message={
          <div className='space-y-2'>
            <div>
              {`This will delete the published channel post and remove it from ${copy.unpublishLocalScopeLabel}. Continue?`}
            </div>
      ...
      />
      {isSettingsModalOpen ? (
        <SocialPublishingSettingsModal subtitle={copy.settingsSubtitle} />
      ) : null}
    </KangurAdminContentShell>
  );
}

export function AdminSocialPublishingPage({
  copy = FILEMAKER_SOCIAL_PUBLISHING_COPY,
}: {
  copy?: SocialPublishingAdminPageCopy;
}): React.JSX.Element {
  return (
    <SocialPostProvider>
      <AdminSocialPublishingPageContent copy={copy} />
    </SocialPostProvider>
  );
}
