...
const ACTION_ID = 'seed_canonical_starter_workflows' as const;

type CliOptions = {
  dryRun: boolean;
};

export type SeedCanonicalStarterWorkflowsSummary = {
  mode: 'dry-run' | 'write';
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  const summary: SeedCanonicalStarterWorkflowsSummary = {
...
if (isEntrypoint) {
  void main().catch((error) => {
    console.error('Failed to seed AI Paths canonical starter workflows:', error);
    process.exit(1);
  });
}
