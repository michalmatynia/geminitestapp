...
import {
  MAX_SOCIAL_PUBLISHING_VISUAL_SUMMARY_CHARS,
  sanitizeSocialPublishingVisualHighlights,
  sanitizeSocialPublishingVisualSummary,
} from '@/shared/lib/social-publishing-visual-analysis';
...
type BeforeAfterPair = {
  ...
  previousAddon: SocialPublishingImageAddon | null;
};

const resolvePreviousAddons = async (
  addons: SocialPublishingImageAddon[]
): Promise<Map<string, SocialPublishingImageAddon>> => {
  ...
  const previousAddons = await findSocialPublishingImageAddonsByIds(uniqueIds);
...
const resolveAddonSource = (addon: SocialPublishingImageAddon): string | null => {
...
const parseHighlights = (value: unknown): string[] => {
  ...
  return sanitizeSocialPublishingVisualHighlights(
...
const buildSystemPrompt = (basePrompt: string, hasBeforeAfter: boolean): string => {
  const lines = [
    basePrompt.trim(),
    'You analyze product or workflow screenshots for social publishing only.',
...
export async function analyzeSocialPublishingVisuals(
  input: VisualAnalysisInput
): Promise<SocialPublishingVisualAnalysis> {
  ...
  try {
    ...
    if (!modelId) {
      throw configurationError(
        'Social Publishing Visual Analysis model is missing. Configure it in AI Brain.'
      );
    }
    ...
    let parsed: Partial<SocialPublishingVisualAnalysis> & {
      highlights?: unknown;
    } = {};
    try {
      parsed = JSON.parse(res.text) as Partial<SocialPublishingVisualAnalysis>;
    } catch {
      // Try extracting JSON from markdown code fences (common with Ollama models)
      const jsonMatch = res.text.match(/```(?:json)?\s*([\s\S]*?)```/);
      if (jsonMatch?.[1]) {
        try {
          parsed = JSON.parse(jsonMatch[1].trim()) as Partial<SocialPublishingVisualAnalysis>;
        } catch {
          parsed = {};
        }
      }
      if (!parsed.summary) {
        ...
        if (raw.length > 0) {
          parsed = { summary: sanitizeSocialPublishingVisualSummary(raw) };
        }
      }
    }
    ...
    const summaryText =
      typeof parsed.summary === 'string' && parsed.summary.trim().length > 0
        ? sanitizeSocialPublishingVisualSummary(parsed.summary)
        : sanitizeSocialPublishingVisualSummary(res.text);

    const analysis: SocialPublishingVisualAnalysis = {
      summary: truncateText(summaryText, MAX_SOCIAL_PUBLISHING_VISUAL_SUMMARY_CHARS),
      highlights: parseHighlights(parsed.highlights),
    };

    const beforeAfterCount = pairs.filter((p) => p.previousAddon !== null).length;

    void ErrorSystem.logInfo('Social publishing visuals analyzed', {
      service: 'social-publishing.posts.visual-analysis',
