...
const {
  ...
  fetchSocialPublishingPostsMock,
  ...
  fetchSocialPublishingPostsMock: vi.fn(),
...
vi.mock('@/features/filemaker/social/hooks/useSocialPublishingPosts', () => ({
  fetchSocialPublishingPosts: (...args: unknown[]) => fetchSocialPublishingPostsMock(...args),
...
describe('useSocialPostCrud', () => {
  beforeEach(() => {
    ...
    fetchSocialPublishingPostsMock.mockResolvedValue([]);
  });
  ...
  it('keeps a published post in published status when saving local draft edits', async () => {
    const deps = createDeps();
    deps.activePost = {
      ...activePost,
      status: 'draft' as const,
      publishedAt: '2026-03-27T12:31:00.000Z',
      publishedPostId: 'urn:li:share:still-published',
      publishedUrl: 'https://www.linkedin.com/feed/update/urn:li:share:still-published/',
    } as never;
  ...
  it('publishes after saving the prepared post payload and syncs the published post into cache', async () => {
    ...
    const publishedPost = {
      ...
      publishedUrl: 'https://www.linkedin.com/feed/update/urn:li:share:published/',
      publishError: null,
    };
    ...
    expect(toastMock).toHaveBeenCalledWith('Published.', { variant: 'success' });
    expect(trackSocialPublishingClientEventMock).toHaveBeenCalledWith(
  ...
  it('skips duplicate publish attempts when the active post is already published', async () => {
    const deps = createDeps();
    deps.activePost = {
      ...activePost,
      status: 'draft' as const,
      publishedAt: '2026-03-27T12:31:00.000Z',
      publishedPostId: 'urn:li:share:already-published',
      publishedUrl: 'https://www.linkedin.com/feed/update/urn:li:share:already-published/',
    } as never;
  ...
  it('skips scheduling attempts when the active post is already published', async () => {
    const deps = createDeps();
    deps.activePost = {
      ...activePost,
      status: 'draft' as const,
  ...
  it('shows the saved publishError after LinkedIn publish fails and skips noisy client exception reporting', async () => {
    ...
    fetchSocialPublishingPostsMock.mockResolvedValueOnce([publishedFailurePost]);
    ...
    expect(fetchSocialPublishingPostsMock).toHaveBeenCalledWith({ scope: 'admin' });
    expect(toastMock).toHaveBeenCalledWith(
  ...
  it('recovers publish success when the client times out but the refreshed post is already published', async () => {
    ...
    fetchSocialPublishingPostsMock.mockResolvedValueOnce([publishedPost]);
  ...
  it('syncs the published post into cache after quick publish succeeds', async () => {
    ...
    const publishedPost = {
      ...
      status: 'published' as const,
      publishedAt: '2026-03-27T12:31:00.000Z',
      publishedPostId: 'urn:li:share:quick-published',
      publishedUrl: 'https://www.linkedin.com/feed/update/urn:li:share:quick-published/',
      publishError: null,
    };
    queryClient.setQueryData(queryKey, [activePost]);
    publishMutateAsyncMock.mockResolvedValueOnce(publishedPost);
  ...
  it('recovers quick publish success when the client times out but the refreshed post is already published', async () => {
    ...
    const publishedPost = {
      ...
      status: 'published' as const,
      publishedAt: '2026-03-27T12:31:00.000Z',
      publishedPostId: 'urn:li:share:456',
      publishedUrl: 'https://www.linkedin.com/feed/update/urn:li:share:456/',
      publishError: null,
    };
    queryClient.setQueryData(queryKey, [activePost]);
    publishMutateAsyncMock.mockRejectedValueOnce(new Error('Request timeout after 15000ms'));
  ...
  it('recovers quick publish success when publication metadata exists even if the refreshed post drifted back to local draft status', async () => {
    const deps = createDeps();
    const { queryClient, Wrapper } = createWrapper();
    const queryKey = QUERY_KEYS.socialPublishing.posts({ scope: 'admin', limit: null });
    const publishedDriftPost = {
      ...
      status: 'draft' as const,
      publishedAt: '2026-03-27T12:31:00.000Z',
      publishedPostId: 'urn:li:share:789',
      publishedUrl: 'https://www.linkedin.com/feed/update/urn:li:share:789/',
      publishError: null,
    };
    queryClient.setQueryData(queryKey, [activePost]);
    publishMutateAsyncMock.mockRejectedValueOnce(new Error('Request timeout after 15000ms'));
    fetchSocialPublishingPostsMock.mockResolvedValueOnce([publishedDriftPost]);
  ...
  it('unpublishes a post and reports a success toast', async () => {
    ...
    expect(toastMock).toHaveBeenCalledWith(
      'Unpublished. Post kept as draft.',
      { variant: 'success' }
    );
  });
