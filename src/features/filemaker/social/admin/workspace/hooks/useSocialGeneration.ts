...
const hasUsableGeneratedContent = <
  T extends
    | Pick<SocialPublishingGeneratedDraft, 'titlePl' | 'titleEn' | 'bodyPl' | 'bodyEn'>
    | Pick<SocialPublishingPost, 'titlePl' | 'titleEn' | 'bodyPl' | 'bodyEn'>,
>(
...
const normalizeGeneratedContent = <
  T extends Pick<SocialPublishingGeneratedDraft, 'titlePl' | 'titleEn' | 'bodyPl' | 'bodyEn'>,
>(
  draftLike: T | null | undefined
): T | null => {
...
export function useSocialGeneration(deps: SocialGenerationDeps) {
  ...
  const handleGenerateInternal = async (
    ...
    trackSocialPublishingClientEvent(
      'social_publishing_post_generate_attempt',
      generationContext
    );
    ...
    try {
      ...
      trackSocialPublishingClientEvent(
        'social_publishing_post_generate_success',
        generationContext
      );
      return true;
    } catch (error) {
      ...
      trackSocialPublishingClientEvent(
        'social_publishing_post_generate_failed',
        {
          ...generationContext,
          error: true,
        }
