 ...
 *
 * This compiler walks the graph in topological order to determine the sequence
 * of processing, then calculates the propagated text through edges.
 *
 * Node roles supported:
 *  - 'text_note'   – Content node; its text is the primary output.
 *  - 'explanatory' – Appends own text to incoming text and contributes
 *                    to the wysiwygContent output channel.
 *  - 'ai_prompt'   – Treated like text_note for compilation.
 *
 * Output channels per node:
 *  - wysiwygText        – Rich HTML text (used in the editor preview).
 *  - plaintextContent   – Stripped plain text for the final AI prompt.
 *  - plainText          – Alias for explanatory nodes.
 *  - wysiwygContent     – Explanatory-only rich content channel.
 *
 * The final prompt is assembled from leaf nodes' plaintextContent, joined
...
const wrapByQuoteMode = (value: string, meta: CaseResolverNodeMeta): string => {
  ...
  if (wrappedValue.length === 0) return wrappedValue;
  ...
  const normalizedColor =
    ...
    /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(meta.textColor.trim())
      ? meta.textColor.trim()
      : '';
  if (normalizedColor.length === 0) return wrappedValue;
...
const wrapByQuoteModeWithoutColor = (value: string, meta: CaseResolverNodeMeta): string => {
  ...
  if (meta.quoteMode === 'double') {
    quotedValue = `"${value}"`;
  } else if (meta.quoteMode === 'single') {
    quotedValue = `'${value}'`;
  }
    ...
    visitOrder.forEach(({ nodeId }) => {
      ...
      const collectIncoming = (
        ...
        incomingEdges.forEach((edge: CaseResolverEdge): void => {
          let acceptsEdge = false;
          if (type === 'wysiwygText') {
            acceptsEdge = isWysiwygTextInputPort(edge.targetHandle);
          } else if (type === 'plaintextContent') {
            acceptsEdge = isPlaintextContentInputPort(edge.targetHandle);
          } else if (type === 'plainText') {
            acceptsEdge = isPlainTextInputPort(edge.targetHandle);
          } else {
            acceptsEdge = isWysiwygContentInputPort(edge.targetHandle);
          }
      ...
      let incomingText = '';
      if (hasIncomingWysiwygText) {
        incomingText = incomingWysiwygText.value;
      } else if (hasIncomingPlainText) {
        incomingText = incomingPlainText.value;
      } else if (meta.role === 'explanatory' && hasIncomingPlaintextContent) {
        incomingText = incomingPlaintextContent.value;
      }

      let incomingTextJoinMode = DEFAULT_CASE_RESOLVER_EDGE_META.joinMode;
      if (hasIncomingWysiwygText) {
        incomingTextJoinMode = incomingWysiwygText.firstJoinMode ?? incomingTextJoinMode;
      } else if (hasIncomingPlainText) {
        incomingTextJoinMode = incomingPlainText.firstJoinMode ?? incomingTextJoinMode;
      } else if (meta.role === 'explanatory' && hasIncomingPlaintextContent) {
        incomingTextJoinMode = incomingPlaintextContent.firstJoinMode ?? incomingTextJoinMode;
      }
      let resolvedWysiwygText = nodeText;
      if (meta.role === 'explanatory' && incomingText.trim().length > 0 && nodeText.trim().length > 0) {
        resolvedWysiwygText = appendWithJoin(incomingText, nodeText, incomingTextJoinMode as CaseResolverJoinMode);
      } else if (incomingText.trim().length > 0) {
        resolvedWysiwygText = incomingText;
      }
      ...
      let secondaryOutputSeed = resolvedWysiwygText;
      if (isExplanatoryPlainTextInputFlow || hasWysiwygTextOnlyIncoming) {
        secondaryOutputSeed = nodeText;
      }
      ...
      let plainTextOutput = stripHtml(wrappedSecondaryPlainTextSeed);
      if (options.transformPlainTextOutput) {
        plainTextOutput = wrappedSecondaryPlainTextSeed;
      }
      ...
      let plaintextContentOutput = incomingPlaintextContent.value;
      if (isExplanatoryPlainTextInputFlow) {
        plaintextContentOutput = incomingPlainText.value;
      }

      if (meta.includeInOutput && wrappedSecondaryText.trim().length > 0) {
        let joinMode = DEFAULT_CASE_RESOLVER_EDGE_META.joinMode ?? 'newline';
        if (isExplanatoryPlainTextInputFlow) {
          joinMode = incomingPlainText.firstJoinMode ?? joinMode;
        } else {
          joinMode = incomingPlaintextContent.firstJoinMode ?? joinMode;
        }
        plaintextContentOutput = appendWithJoin(
          plaintextContentOutput,
          wrappedSecondaryText,
          joinMode as CaseResolverJoinMode
        );
      }
      if (meta.role === 'explanatory' && !options.transformPlainTextOutput) {
        plaintextContentOutput = stripHtml(plaintextContentOutput);
      }
      if (options.transformPlainTextOutput != null) {
      ...
      let wysiwygContentOutput = '';
      if (meta.role === 'explanatory') {
        wysiwygContentOutput = incomingWysiwygContent.value;
      }
      if (
