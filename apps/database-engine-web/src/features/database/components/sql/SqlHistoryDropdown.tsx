...
type HistoryListPanelProps = {
  history: string[];
  onSelectQuery: (query: string) => void;
  onClearHistory: (event: React.MouseEvent<HTMLButtonElement>) => void;
  setShowHistory: (show: boolean) => void;
};
...
function HistoryToggleButton({
  ...
  showHistory,
  onToggle,
}: HistoryToggleButtonProps): React.JSX.Element {
  return (
    <Button
      variant='outline'
      size='sm'
...
function HistoryListPanel({
  ...
  onClearHistory,
  setShowHistory,
}: HistoryListPanelProps): React.JSX.Element {
  return (
    <Card
      variant='glass'
      padding='none'
...
export function SqlHistoryDropdown(props: SqlHistoryDropdownProps): React.JSX.Element {
  ...
  const handleClearHistoryClick = (event: React.MouseEvent<HTMLButtonElement>): void => {
  ...
  return (
    <div className='relative'>
      <HistoryToggleButton history={history} showHistory={showHistory} onToggle={toggleHistory} />
