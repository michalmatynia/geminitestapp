...
export const fallbackCurrencies = defaultCurrencies.map((currency: CurrencyType) => ({
  id: currency.code,
  code: currency.code,
  name: currency.name,
...
export const fallbackCountries = defaultCountries.map((country: CountryType) => {
...
export const fallbackLanguages = defaultLanguages.map((language: LanguageType) => {
  const matchingMappings = countryMappings.filter((mapping: CountryMappingType) =>
