...
export function PreviewSection({
  ...
  const wrapInspector = (node: React.ReactNode): React.ReactNode => (
    <BlockContextProvider value={{ sectionId: section.id, sectionType: section.type, sectionZone: section.zone, mediaStyles }}>
      <InspectorHover nodeId={section.id} fallbackNodeId={null} content={inspectorContent} className='w-full'>
  ...
  const selectedRingBase = isInspecting
    ? isSectionSelected ? 'ring-4 ring-blue-500/65' : isSectionHovered ? 'ring-4 ring-blue-500/45' : 'hover:ring-1 hover:ring-inset hover:ring-border/40'
    : showEditorChrome
      ? isSectionSelected ? (isInspecting ? 'ring-2 ring-inset ring-blue-500/60' : 'ring-2 ring-inset ring-blue-500/40') : 'hover:ring-1 hover:ring-inset hover:ring-border/40'
      : '';
  ...
  if (section.type === 'AnnouncementBar' || section.type === 'Block') {
    ...
    const justifyContent = resolveJustifyContent(justifySetting === 'inherit' ? alignment : justifySetting) ?? (alignment === 'center' ? 'center' : alignment === 'right' ? 'flex-end' : 'flex-start');
    ...
    return wrapInspector(
      ...
      >
        ...
        <div className={getSectionContainerClass({ fullWidth: layout?.fullWidth, maxWidthClass: 'max-w-6xl' })}>
          <div
            className={section.type === 'Block' ? `flex ${flexDirClass} ${wrapClass}` : `flex flex-wrap items-center gap-3 ${alignmentClasses}`}
          ...
          >
            {section.blocks.length === 0 ? (
              ...
              <BlockContextProvider value={{ contained: true }}>
                {section.blocks.map((block: BlockInstance) => <PreviewBlockItem key={block.id} block={block} />)}
              </BlockContextProvider>
            )}
          </div>
        </div>
...
function PreviewBlockItem({ block }: PreviewBlockItemProps): React.ReactNode {
  ...
  const canvasFrameClass = isSelected ? (isInspecting ? 'ring-4 ring-blue-500/45' : 'ring-2 ring-blue-500/35') : (isHovered ? 'ring-4 ring-blue-500/45' : '');
  ...
  const inspectorContent = (
    <InspectorTooltip title={block.type} sections={[{ title: 'Meta', entries: [{ label: 'Type', value: block.type }] }]} />
  );
  
  const wrapBlock = (node: React.ReactNode): React.ReactNode => (
    <BlockContextProvider value={{ parentBlockId: block.id }}>
      <InspectorHover nodeId={block.id} fallbackNodeId={parentBlockId ?? columnId ?? sectionId} content={inspectorContent} className={stretch ? 'h-full' : ''}>
        <GsapAnimationWrapper config={animConfig}>
          <CssAnimationWrapper config={cssAnimConfig}>
            <EventEffectsWrapper settings={block.settings} disableClick nodeId={allowInlineCustomCss ? block.id : ''} customCss={allowInlineCustomCss ? block.settings['customCss'] : undefined}>
              {node}
            </EventEffectsWrapper>
          </CssAnimationWrapper>
        </GsapAnimationWrapper>
    ...
    return wrapBlock(
      <div className='relative group'>
        <div
          ...
          className={buildContainerClass(`w-full text-left text-sm transition ${contained ? 'max-w-full' : ''} ${showEditorChrome ? 'overflow-hidden' : ''}`, `rounded ${isSelected ? 'ring-2 ring-inset ring-blue-500/40 bg-blue-500/15' : 'ring-1 ring-inset ring-border/30 bg-gray-800/30 hover:ring-border/50'}`)}
        >
  ...
  if (block.type === 'Heading') {
    ...
    return wrapBlock(
      <div role='button' tabIndex={0} onClick={handleSelect} className={buildContainerClass(`w-full text-left transition relative group ${contained ? 'max-w-full' : ''}`, '')}>
        <h2 className='text-2xl font-bold leading-tight tracking-tight md:text-3xl text-gray-200' style={typoStyles}>{text}</h2>
      </div>,
    );
  }
  ...
  if (block.type === 'Text') {
    ...
    if (!text.trim() && !showEditorChrome) return null;
    return wrapBlock(
      <div role='button' tabIndex={0} onClick={handleSelect} className={buildContainerClass(`w-full text-left transition ${contained ? 'max-w-full' : ''}`, `rounded ${isSelected ? 'ring-2 ring-inset ring-blue-500/40 bg-blue-500/15' : 'ring-1 ring-inset ring-border/30 bg-gray-800/20 hover:ring-border/50'}`)}>
        {text ? <p className='text-base leading-relaxed text-gray-300 md:text-lg' style={typoStyles}>{text}</p> : <p className='text-sm italic text-gray-500'>Add text content...</p>}
      </div>,
    );
  }

  return wrapBlock(<div role='button' tabIndex={0} onClick={handleSelect} className={buildContainerClass('w-full', '')}><span className='text-gray-500'>{block.type}</span></div>);
}
