...
const GEMINI_WRAPPER = process.env.GEMINI_WRAPPER || 'gemini-preview-iso';
// The corresponding isolated HOME directory (used to check for existing sessions).
const ISO_HOME = process.env.GEMINI_ISO_HOME || path.join(os.homedir(), '.gemini-preview-home');
...
function spawnGemini() {
  ...
  ptyProcess.onData((data) => {
    ...
    if (
      data.includes('Authentication succeeded') &&
      data.includes('Press') &&
      data.includes("'r'")
    ) {
    ...
    if (
      data.includes('We are currently experiencing high demand') ||
      data.includes('No capacity available')
    ) {
      const now = Date.now();
      ...
      if (hasKeep) {
        ...
        scheduleContinueRetry(
          data.includes('No capacity available') ? 'no capacity' : 'high demand'
        );
      }
      return;
    }
  ...
  ptyProcess.onExit(({ exitCode, signal }) => {
    ...
    if (sawCapacity) {
      sawCapacity = false;
      console.error(
        `[gemini] Exited during capacity event — restarting in ${CAPACITY_RETRY_MS}ms...\n`
      );
