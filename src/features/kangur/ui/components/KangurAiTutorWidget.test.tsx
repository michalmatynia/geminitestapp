...
describe('KangurAiTutorWidget', () => {
  ...
  it('accepts the anonymous login prompt by starting guided login help', async () => {
    useOptionalKangurAuthMock.mockReturnValue({
      isAuthenticated: false,
      isLoadingAuth: false,
      navigateToLogin: navigateToLoginMock,
    ...
    const renderOptions = { showCreateAccountAnchor: true, showLoginAnchor: true };
    openChatMock.mockImplementation(() => {
      tutorState = {
        ...tutorState,
        isOpen: true,
  ...
  it('keeps the reopened anonymous onboarding modal free of the removed guest guidance controls', async () => {
    ...
    expect(guestIntro).toHaveAttribute('data-modal-surface', 'canonical-onboarding');
    expect(screen.queryByTestId('kangur-ai-tutor-panel')).not.toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: 'Wpisz pytanie' })).toBeInTheDocument();
  ...
  it('keeps stale tutor history hidden while the selected-text explanation is loading and rebinds the panel to the new fragment', async () => {
    ...
    expect(screen.getAllByText('To jest wyjaśnienie fragmentu.').length).toBeGreaterThan(0);
    expect(
      screen.queryByText('W poprzednim wątku omawialiśmy zupełnie inne zadanie.')
    ).not.toBeInTheDocument();
    expect(screen.queryByText('„Stary fragment”')).not.toBeInTheDocument();
  ...
  it('keeps the original home-progress excerpt bound while the live selection reflows before the reply arrives', async () => {
    ...
    const renderOptions = { homeAnchorKinds: ['progress'] as const, useCoverageAnchorIds: true };
    const { rerender } = renderWithTutorAnchors(renderOptions);
    rerenderWidget = () => {
      rerender(buildTutorAnchorsTree(renderOptions));
    };
  ...
  it('keeps the home-progress excerpt thread alive when the browser selection clears before the docked tutor opens', async () => {
    ...
    useKangurTextHighlightMock.mockImplementation(() => ({
      activateSelectionGlow: activateSelectionGlowMock,
      selectedText: liveSelectedText,
      selectionLineRects: liveSelectedText ? [new DOMRect(680, 1040, 260, 52)] : [],
      selectionRect: liveSelectedText ? new DOMRect(680, 1040, 260, 52) : null,
      selectionContainerRect: liveSelectedText ? new DOMRect(640, 980, 360, 220) : null,
      clearSelection: clearSelectionMock,
      clearSelectionGlow: clearSelectionGlowMock,
      selectionGlowSupported: false,
    }));
  ...
  it('renders the open tutor as a draggable freeform panel while preserving lesson context', () => {
    ...
    expect(screen.getByTestId('kangur-ai-tutor-panel-surface')).toHaveClass('kangur-panel-soft');
  ...
  it('attaches the anchored tutor launcher to the open panel instead of rendering it as a separate floater', () => {
    ...
    expect(screen.getByTestId('kangur-ai-tutor-pointer-marker')).toHaveClass(
      'kangur-chat-pointer-marker'
    );
    expect(screen.getByTestId('kangur-ai-tutor-pointer-glow')).toHaveClass(
      'kangur-chat-pointer-glow'
    );
    expect(screen.getByTestId('kangur-ai-tutor-pointer-line')).toHaveClass(
      'kangur-chat-pointer-line'
    );
  });
  ...
  it('uses a bottom-sheet layout on narrow mobile viewports', () => {
    ...
    expect(screen.getByTestId('kangur-ai-tutor-panel-surface')).toHaveClass(
      'kangur-chat-panel-shell-sheet'
    );
    expect(screen.getByTestId('kangur-ai-tutor-sheet-handle')).toHaveClass(
      'kangur-chat-sheet-handle'
    );
    expect(screen.getByTestId('kangur-ai-tutor-header')).toHaveClass(
      'kangur-chat-header-padding-md'
    );
