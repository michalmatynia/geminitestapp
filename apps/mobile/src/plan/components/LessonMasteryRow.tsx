...
function MasteryHeader({ insight, title, copy }: { insight: KangurMobileDailyPlanLessonMasteryItem; title: string; copy: ReturnType<typeof useKangurMobileI18n>['copy'] }): React.JSX.Element {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
      ...
      <Pill label={`${insight.masteryPercent}%`} tone={getLessonMasteryTone(insight.masteryPercent)} />
    </View>
  );
}
    ...
    <InsetPanel gap={10}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
        ...
        {insight.practiceHref !== null && (
          <LinkButton href={insight.practiceHref} label={copy({ de: 'Danach trainieren', en: 'Practice after', pl: 'Potem trenuj' })} />
        )}
      </View>
    </InsetPanel>
  );
