...
describe('subject-focus remote API client integration', () => {
  ...
  it('ignores invalid runtime subject values without calling the API', async () => {
    ...
    await expect(
      persistRemoteSubjectFocus('learner-1', undefined as unknown as 'maths')
    ).resolves.toBeNull();
    expect(fetchMock).not.toHaveBeenCalled();
    expect(trackWriteFailureMock).not.toHaveBeenCalled();
  });

  it('deduplicates concurrent remote subject focus updates for the same learner scope', async () => {
    ...
    const first = persistRemoteSubjectFocus('learner-1', 'english');
    const second = persistRemoteSubjectFocus('learner-1', 'english');
    ...
    await expect(Promise.all([first, second])).resolves.toEqual(['english', 'english']);
  });
});
