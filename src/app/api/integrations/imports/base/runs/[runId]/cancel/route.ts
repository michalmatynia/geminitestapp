...
export const POST = apiHandlerWithParams<{ runId: string }>(
  (req, ctx, params) => {
    recordLegacyCompatCounter('compat_route_hit', {
      source: 'api.compat.integrations.imports.base.runs.[runId].cancel.POST',
      context: { route: '/api/integrations/imports/base/runs/[runId]/cancel' },
    });
    assertLegacyCompatRouteEnabled({
      route: '/api/integrations/imports/base/runs/[runId]/cancel',
      method: 'POST',
      source: 'api.compat.integrations.imports.base.runs.[runId].cancel.POST',
    });
