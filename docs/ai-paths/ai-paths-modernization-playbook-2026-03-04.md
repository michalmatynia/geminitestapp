...
121. Pruned starter-workflow factory node-shape cast in `core/utils/factory.ts` by typing raw seed nodes as `Omit<AiNode, 'createdAt' | 'updatedAt' | 'data'>` and removing the cast adapter in the legacy description starter-path factory.
