import type { AiNode, DbQueryConfig, Edge, NodeType } from '@/shared/contracts/ai-paths';
...
export const API_ADVANCED_INPUT_PORTS = [
  'context',
  'bundle',
  'prompt',
  'result',
  ...
  rawHistoryRetentionOptionsMax >= AI_PATHS_HISTORY_RETENTION_MIN
    ? Math.min(
      ...
      Math.max(
        ...
        Math.trunc(rawHistoryRetentionOptionsMax)
      )
    )
    : AI_PATHS_HISTORY_RETENTION_MAX;
...
export const NODE_TYPE_COMPATIBILITY: Record<NodeType, NodeType[]> = {
  ...
  math: ['template', 'viewer', 'bundle', 'compare', 'router', 'delay', 'poll', 'http', 'api_advanced', 'database', 'audio_oscillator', 'audio_speaker'],
  ...
  http: ['viewer', 'bundle', 'template', 'prompt', 'math', 'compare', 'poll', 'api_advanced', 'database'],
  api_advanced: ['viewer', 'bundle', 'template', 'prompt', 'math', 'compare', 'poll', 'http', 'database'],
  ...
  bounds_normalizer: ['viewer', 'canvas_output', 'bundle', 'mapper', 'validator', 'template', 'database', 'prompt'],
  canvas_output: [],
};
