      ...
      <div style="max-width:640px;margin:0 auto;padding:40px 24px;">
        ...
        <div style="border:1px solid #2b2b2b;background:#111;padding:24px;margin-bottom:24px;">
          ...
          <div style="padding-top:18px;">
            ...
            ${
              order.discount > 0
                ? `<div style="display:flex;justify-content:space-between;color:#8db281;font-size:14px;margin-bottom:8px;"><span>Discount${order.promoCode !== undefined && order.promoCode.length > 0 ? ` (${escapeHtml(order.promoCode)})` : ''}</span><span>-${formatMoney(order.discount)}</span></div>`
                : ''
            }
          ...
          <p style="font-size:14px;color:#c8c1b5;line-height:1.8;margin:0;">
            ...
            ${
              order.inpostPoint !== undefined
                ? `<br><br><strong style="color:#f5f1e8;">InPost:</strong> ${escapeHtml(order.inpostPoint.name)}${order.inpostPoint.addressLine1 !== undefined && order.inpostPoint.addressLine1.length > 0 ? `<br>${escapeHtml(order.inpostPoint.addressLine1)}` : ''}`
                : ''
            }
            ${
              order.inpostShipment?.trackingNumber === undefined
                ? ''
                : `<br><br>Tracking: ${escapeHtml(order.inpostShipment.trackingNumber)}`
            }
          </p>
        </div>
