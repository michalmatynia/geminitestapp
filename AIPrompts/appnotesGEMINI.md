...
FOCUSED SCAN (2026-02-12): REDUNDANT TYPES + DTO UNIFICATION + TANSTACK QUERY
...
- `src/features/products/hooks/useCategoryQueries.ts`
- `src/features/products/hooks/useProductSettingsQueries.ts`
- `src/shared/dtos/products.ts`
- `src/shared/types/domain/products.ts`
- `src/features/products/types/index.ts`
- `src/shared/lib/query-keys.ts`
- `src/shared/providers/QueryProvider.tsx`

Confirmed redundancy clusters (highest priority)
- Product metadata hooks are duplicated across `useMetadata.ts`, `useProductMetadata.ts`, and `useCatalogQueries.ts` (same endpoints, mixed key patterns, overlapping return types).
- Product DTO vs domain aliases overlap across `src/shared/dtos/products.ts`, `src/shared/types/domain/products.ts`, and `src/features/products/types/index.ts`.
- Same concepts are typed in multiple places (`ProductCategoryWithChildren`, draft DTO aliases, provider/domain enums), increasing drift risk.
- Query keys are partly standardized (`QUERY_KEYS`) but many hooks still use literal arrays (products, integrations, ai-paths, shared hooks), making invalidation fragile.

Additional duplicate types found in wider scan (next wave after products)
- `UserPreferences`: `src/shared/types/domain/products.ts` and `src/shared/types/domain/user-preferences.ts`.
- `ProductAiJobType`: `src/shared/types/domain/products.ts` and `src/shared/types/domain/jobs.ts`.
- Import/export template contracts duplicated in integrations/import-export modules.

Target architecture (DTO and type ownership)
- API DTO source of truth: `src/shared/contracts/**` with Zod schemas + inferred DTO types.
- Domain types: `src/shared/types/domain/**` for runtime/business enrichments only (computed fields, joins, internal-only metadata).
- Feature-local aliases: allowed only as re-exports from shared contracts/domain (no local re-definition).
- API routes and client hooks parse responses with shared schemas at boundaries.

Execution plan (incremental, low-risk)

Phase 1: Products contract baseline
- Create `src/shared/contracts/products.ts`:
  - `productSchema`
  - `productCategorySchema`
  - `productTagSchema`
  - `catalogSchema`
  - `priceGroupSchema`
  - request payload schemas for create/update operations
- Export DTO types via `z.infer`.
- Keep `src/shared/dtos/products.ts` as compatibility layer that re-exports from contracts (temporary).

Phase 2: Products hook convergence
- Introduce one canonical metadata query module:
  - `src/features/products/hooks/useProductMetadataQueries.ts`
  - Owns `useCatalogs/useLanguages/usePriceGroups/useCategories/useTags/useParameters/useProducers`.
- Convert `useMetadata.ts` and `useCatalogQueries.ts` to deprecated re-export wrappers (or remove after call-site migration).
- Use only `QUERY_KEYS.products.metadata.*` and `QUERY_KEYS.products.settings.*` in this domain.

Phase 3: QueryClient centralization and policy hardening
- Move QueryClient construction into `src/shared/lib/query-client.ts`.
- Add `QueryCache` and `MutationCache` callbacks that:
  - forward errors to centralized logger with `queryKey` or `mutationKey`
  - never throw
  - attach stable context (`source`, `key`, `attempt`, `statusCode`)
- Keep retry policy centralized:
  - no retries for 4xx
  - bounded retries for 5xx/network/timeouts
  - short retry windows for SSR-sensitive queries

Phase 4: Query key migration sweep
- Replace literal `queryKey: ['...']` usage with key factories in:
  - products hooks/components first
  - integrations hooks next
  - ai-paths/shared hooks after that
- Add lint rule or codemod guard to block new literal keys outside key factory files.

Phase 5: Invalidation map and test harness
- Define invalidation helpers per domain (`invalidateProductMetadata`, `invalidateCatalogScopedData`, etc.).
- Add tests that assert:
  - mutation handlers invalidate expected keys
  - QueryCache/MutationCache callbacks log normalized error payloads
  - product metadata hooks keep previous behavior while using unified keys/contracts

Quick wins to execute first
1. Merge product metadata hooks into one module and switch all product metadata queries to `QUERY_KEYS.products.metadata.*`.
2. Introduce `src/shared/lib/query-client.ts` with QueryCache/MutationCache callbacks and wire `QueryProvider.tsx` to it.
3. Deduplicate `UserPreferences` and `ProductAiJobType` by selecting one canonical domain file and replacing duplicates with re-exports.

Success criteria
- No new literal query keys added in product/integration domains.
- Product metadata types come from one DTO contract path.
- Query-level and mutation-level errors are logged centrally with stable metadata.
- Existing UI behavior remains unchanged (only architecture + reliability improvements).
