  ...
  async mongoToPrisma(): Promise<SyncResult> {
    ...
    if (!model) {
      return {
        ...
        targetDeleted: 0,
        targetInserted: 0,
        warnings: [`Prisma model "${prismaModelName}" not found`],
      };
    }
...
export function getSupportedCollections(): string[] {
  return [...Object.keys(SPECIALIZED_HANDLERS), ...Object.keys(COLLECTION_MODEL_MAP)];
}
...
export async function copyCollection(
  ...
  if (!handler) {
    return {
      ...
      targetInserted: 0,
      warnings: [
        `No handler found for collection "${collectionName}". Copy is not supported for this collection.`,
      ],
    };
  }
