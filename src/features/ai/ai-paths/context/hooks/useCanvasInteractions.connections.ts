...
export interface UseCanvasInteractionsConnectionsValue {
  ...
  handleStartConnection: (event: React.PointerEvent<Element>, node: AiNode, port: string) => Promise<void>;
  handleCompleteConnection: (event: React.PointerEvent<Element>, node: AiNode, port: string) => void;
  handleReconnectInput: (event: React.PointerEvent<Element>, nodeId: string, port: string) => Promise<void>;
  getPortPosition: (node: AiNode, portName: string | undefined, side: 'input' | 'output') => { x: number; y: number };
}
