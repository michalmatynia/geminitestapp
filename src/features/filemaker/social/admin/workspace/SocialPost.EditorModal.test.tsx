...
describe('SocialPostEditorModal', () => {
  it('uses product-style header actions, removes the draft pill, and switches between tabs', () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Product Weekly Update' })).toBeInTheDocument();
  ...
  it('shows the Publication subtitle even when the local draft status has drifted from published', () => {
    useSocialPostContextMock.mockReturnValue({
      activePost: {
        ...buildPost(),
        status: 'draft',
    ...
    expect(screen.getByRole('dialog', { name: 'Product Weekly Update' })).toHaveTextContent(
      /Published/i
    );
    expect(screen.getByRole('button', { name: 'Published' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Published' })).toHaveAttribute(
      'title',
      'This post is already published. Unpublish it from the posts list before publishing again.'
    );
    ...
    expect(screen.getByLabelText('Scheduled publish date and time')).toHaveAttribute(
      'title',
      'This post is already published. Unpublish it from the posts list before scheduling again.'
    );
    expect(screen.getByRole('button', { name: 'Schedule' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Schedule' })).toHaveAttribute(
      'title',
