...
export function VectorCanvas(props: VectorCanvasProps): React.JSX.Element {
  ...
  const syncCanvasSize = useCallback((): void => {
    ...
    scheduleDraw();
  }, [resolvedBaseCanvasHeight, resolvedBaseCanvasWidth, scheduleDraw, src, canvasRef, containerRef, imgRef]);
  ...
  }, [canvasRenderSize.height, canvasRenderSize.width, resolvedImageOffset.x, resolvedImageOffset.y, src, viewTransform, containerRef, imgRef]);
  ...
  }, [canvasRenderSize.height, canvasRenderSize.width, resolvedImageOffset.x, resolvedImageOffset.y, src, imgRef]);

  useEffect(() => {
    if (!onViewCropRectChange) return;
    onViewCropRectChange(resolveVisibleViewCropRect());
  }, [onViewCropRectChange, resolveVisibleViewCropRect, src, viewTransform, canvasRenderSize.width, canvasRenderSize.height]);

  useEffect(() => {
    if (!onImageContentFrameChange) return;
    onImageContentFrameChange(resolveImageContentFrame());
  }, [canvasRenderSize.height, canvasRenderSize.width, onImageContentFrameChange, resolveImageContentFrame, src, viewTransform]);
  ...
  const handleMouseDown = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>): void => {
      ...
      if (tool === 'polygon') {
        ...
        if (rect && activeShape.points.length >= 3) {
          ...
          const dist = Math.hypot((first.x - nextPoint.x) * rect.width, (first.y - nextPoint.y) * rect.height);
          if (dist < 10) {
            onChange(shapes.map((s) => s.id === activeShape.id ? { ...s, closed: true } : s));
            return;
          }
        }
        onChange(shapes.map((s) => s.id === activeShape.id ? { ...s, points: [...s.points, nextPoint] } : s));
    ...
    [activeShapeId, beginPan, imageMoveEnabled, onChange, onSelectPoint, onSelectShape, resolvedImageOffset, selectionEnabled, shapes, src, syncCanvasSize, toPoint, tool, hitTestPoint, hitTestSegment, hitTestShape, canvasRef, spaceDownRef, imageDragRef, dragRef, dragShapeRef, drawingRef, setIsDraggingImage, setIsDraggingEditablePoint]
  );
  ...
  const handleMouseMove = useCallback(
        ...
        onChange(
          shapes.map((shape: VectorShape) => {
            ...
            if (shape.type === 'rect' && (event.shiftKey || event.altKey)) {
              ...
              const nextRectPoints = resolveRectResizePoints(shape.points, currentDrag.pointIndex, nextPoint, {
      ...
      if (drawingRef.current) {
        ...
        onChange(
          shapes.map((shape: VectorShape) => {
            ...
            if (shape.type === 'rect') {
              const rect = getInteractionRect();
              const nextPoints = resolveRectDragPoints(drawState.anchor ?? shape.points[0]!, nextPoint, {
                scaleFromCenter: event.altKey,
                lockSquare: event.shiftKey,
                viewportWidth: rect?.width,
                viewportHeight: rect?.height,
    ...
    [panningRef, updatePanFromPointer, isDraggingImage, imageDragRef, onImageOffsetChange, dragRef, toPoint, onChange, shapes, getInteractionRect, dragShapeRef, drawingRef]
  );
  ...
  const handleMouseUp = useCallback((): void => {
    ...
    if (drawingRef.current) {
      onChange(shapes.map((s) => s.id === drawingRef.current?.shapeId ? { ...s, closed: true } : s));
    }
    ...
    >
      ...
      >
        {backgroundLayerEnabled && (
          <div className='pointer-events-none absolute left-1/2 top-0 z-[1] -translate-x-1/2 border border-slate-700/70' style={{ width: `${canvasRenderSize.width}px`, height: `${canvasRenderSize.height}px`, backgroundColor: resolvedBackgroundColor }} />
        )}
        {showCanvasGrid && (
        ...
        {src && (
          ...
          <img
            ...
            style={{ width: `${canvasRenderSize.width}px`, height: `${canvasRenderSize.height}px`, left: `calc(50% + ${resolvedImageOffset.x}px)`, top: `${resolvedImageOffset.y}px`, transform: 'translateX(-50%)' }}
        ...
        <canvas
          ref={canvasRef}
          className={cn('absolute left-1/2 top-0 z-20 -translate-x-1/2', isPanning || isDraggingImage || isDraggingEditablePoint ? 'cursor-grabbing' : isHoveringEditablePoint ? 'cursor-pointer' : isHoveringMovableShape ? 'cursor-move' : 'cursor-crosshair')}
        ...
        />
        <VectorShapeOverlay shapes={shapes} activeShapeId={activeShapeId} selectedPointIndex={selectedPointIndex} viewboxSize={SHAPE_VIEWBOX_SIZE} />
      </div>
