...
const stateFile = path.join(runtimeDir, 'dev-studiq-web.json');
...
const terminateDevProcess = async ({ force }) => {
  ...
  if (!state) {
    console.log('No tracked Studiq web dev process found.');
    return;
  }

  if (state.devPid && isPidRunning(state.devPid)) {
    const firstSignal = force ? 'SIGKILL' : 'SIGTERM';
    console.log(`Stopping Studiq web dev process group ${state.devPid} with ${firstSignal}.`);
    killProcessGroup(state.devPid, firstSignal);
    if (!force) {
      await sleep(stopWaitMs);
      if (isPidRunning(state.devPid)) {
        console.log(`Studiq web dev process ${state.devPid} is still running; forcing SIGKILL.`);
        killProcessGroup(state.devPid, 'SIGKILL');
      }
    }
  } else {
    console.log('Tracked Studiq web dev process is not running.');
  }
  ...
  if (
    ...
    console.log(`Stopping Studiq web dev supervisor ${state.supervisorPid} with ${signal}.`);
...
const stopMongo = () => {
  runNpmScript('mongo:studiq:down');
};

const startMongo = () => {
  runNpmScript('mongo:studiq:up');
};
...
const printStatus = async () => {
  ...
  runNpmScript('mongo:studiq:status');
};
...
const shutdownSupervisor = async (child, reason, exitCode = 0) => {
  ...
  shuttingDown = true;
  console.log(`\n[dev:studiq-web] Shutting down (${reason}).`);
...
const startDev = async () => {
  const state = await readState();
  if (state?.devPid && isPidRunning(state.devPid)) {
    console.log(`Studiq web dev app is already running (pid ${state.devPid}).`);
    startMongo();
    return;
  }
  await clearState();
  ...
  const child = spawn(npmCommand, ['run', 'dev', '-w', '@app/studiq-web'], {
    cwd: repoRoot,
    detached: true,
    env: process.env,
    stdio: 'inherit',
  ...
  await writeState({
    command: 'npm run dev -w @app/studiq-web',
    devPid: child.pid,
    supervisorPid: process.pid,
    startedAt: new Date().toISOString(),
  });

  console.log(`\n[dev:studiq-web] Studiq web dev app started (pid ${child.pid}).`);
  console.log('[dev:studiq-web] Press Ctrl+C or run npm run dev:studiq-web:kill to stop app + MongoDB.');
  ...
  child.on('exit', (code, signal) => {
    ...
    void shutdownSupervisor(child, `studiq-web dev exited${signal ? ` by ${signal}` : ''}`, exitCode);
  });
};
