...
function LessonCard({
  ...
  saveLessonCheckpoint: any;
  setSavedCheckpoint: (val: any) => void;
  setDismissedFocusToken: (val: string | null) => void;
  setActiveSectionIdx: (idx: number) => void;
}): JSX.Element | null {
  const router = useRouter();

  if (isPreparing) {
    if (selectedLesson !== null || focusToken !== null) {
      return (
        <>
          <LessonsLoadingDetailCard />
          <LessonsLoadingCatalogCard />
    ...
    if (focusToken !== null) {
      return (
        <Card>
          ...
          <View style={{ gap: 10 }}>
            <LinkButton href='/lessons' label={copy({ de: 'Otwórz pełny katalog' })} stretch tone='primary' />
            <ActionButton
