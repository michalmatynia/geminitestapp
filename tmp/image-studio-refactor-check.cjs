...
function ensureFixtureImage() {
  ...
  if (!fs.existsSync(file)) {
    const b64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wn6QnYAAAAASUVORK5CYII=';
...
(async () => {
  ...
  try {
    ...
    await page.locator('input[name="email"], input[type="email"]').first().fill('e2e.admin@example.com');
    await page.locator('input[name="password"], input[type="password"]').first().fill('E2eAdmin!123');
    await Promise.all([
    ...
    await page.goto(base + '/admin/image-studio', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1400);

    out.extractButtonVisible = (await page.getByRole('button', { name: /extract functions and selectors from prompt|extract/i }).count()) > 0;
    ...
    if (await slotNode.count()) {
      ...
      out.slotSelectedVisibleInPreview = (await page.getByRole('button', { name: /empty slot/i }).count()) > 0;
    }

    // click tree background to deselect
    await page.locator('.h-full.overflow-auto.rounded').first().click({ position: { x: 8, y: 8 } }).catch(()=>{});
    await page.waitForTimeout(500);
    out.deselectToEmptySlotWorks = (await page.getByRole('button', { name: /empty slot/i }).count()) === 0;
    ...
    const createFolder = async (name) => {
      ...
      await dialog.getByRole('button', { name: /create folder/i }).first().click();
    ...
    const slotForMove = page.locator('button[draggable="true"]').filter({ hasText: /slot/i }).first();
    const folderBNode = page.locator('button[draggable="true"]').filter({ hasText: folderB }).first();
    ...
    // extract modal opens
    const extractBtn = page.getByRole('button', { name: /extract functions and selectors from prompt|extract/i }).first();
    if (await extractBtn.count()) {
      await extractBtn.click();
      await page.waitForTimeout(350);
      out.extractModalOpens = (await page.locator('text=/extract prompt params|programmatic extract/i').count()) > 0;
    }
  } catch (error) {
