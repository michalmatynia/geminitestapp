...
function angleTo5MinStep(angleDeg: number): number {
...
function DraggableClock({ onSubmit }: DraggableClockProps): React.JSX.Element {
  ...
  const displayHour = angleToHour(hourAngle);
...
export default function ClockTrainingGame({
  ...
  const [tasks] = useState<ClockTask[]>(() => shuffle(TASKS).slice(0, 5));
  ...
  const handleDone = (finalScore: number): void => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 w-full'>
      ...
      {!feedback && <DraggableClock onSubmit={handleSubmit} />}
    </div>
  );
}
