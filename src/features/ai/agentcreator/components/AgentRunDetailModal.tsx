      ...
      <Tabs defaultValue='summary' className='w-full'>
        ...
        <TabsContent value='summary' className='mt-4 space-y-3'>
          <div className='rounded-md border border-border bg-gray-900 p-3 text-xs text-gray-300'>
            ...
            <p className='mt-1 text-sm text-white'>{selectedAgentRun.prompt}</p>
            <div className='mt-2 text-xs text-gray-400'>
              Status: {selectedAgentRun.status.replace('_', ' ')} · Model: {selectedAgentRun.model || 'Default'}
              {selectedAgentRun.requiresHumanIntervention ? ' · needs input' : ''}
            </div>
            {selectedAgentRun.checkpointedAt ? (
              <div className='mt-2 rounded-md border border-emerald-500/20 bg-emerald-500/5 p-2 text-[11px] text-emerald-200'>
                Checkpoint saved {new Date(selectedAgentRun.checkpointedAt).toLocaleString()}
                {selectedAgentRun.activeStepId ? <span className='text-emerald-300'> · Active step {selectedAgentRun.activeStepId}</span> : null}
              </div>
            ) : (
              <div className='mt-2 text-[11px] text-gray-500'>No checkpoint saved yet.</div>
            )}
        ...
        <TabsContent value='preview' className='mt-4'>
          <div className='rounded-md border border-border bg-card p-3'>
            <div className='flex items-center justify-between text-xs text-gray-400'>
              <span>Preview</span>
              <span>
                {agentStreamStatus === 'live' ? 'Live' : agentStreamStatus === 'connecting' ? 'Connecting...' : agentStreamStatus === 'error' ? 'Fallback' : 'Idle'}
              </span>
            </div>
            <div className='mt-3 overflow-hidden rounded-md border border-border bg-gray-900'>
              {agentSnapshot?.screenshotPath || agentSnapshot?.screenshotData ? (
                <div className='relative'>
                  <Image
                    src={agentSnapshot.screenshotPath ? `/api/agentcreator/agent/${selectedAgentRun.id}/assets/${agentSnapshot.screenshotPath}` : (agentSnapshot.screenshotData ?? '')}
                    alt='Agent preview'
                    ...
                    unoptimized
                  />
                  {agentSnapshot.mouseX !== null && agentSnapshot.mouseY !== null && agentSnapshot.viewportWidth && agentSnapshot.viewportHeight ? (
                    <div
                      ...
                      style={{
                        left: `${(agentSnapshot.mouseX / agentSnapshot.viewportWidth) * 100}%`,
                        top: `${(agentSnapshot.mouseY / agentSnapshot.viewportHeight) * 100}%`,
                      }}
                    />
                  ) : null}
                </div>
              ) : (
                <div className='flex min-h-[200px] items-center justify-center text-xs text-gray-500'>No preview yet.</div>
              )}
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value='steps' className='mt-4'>
          <div className='rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            <div className='mb-3 flex items-center justify-between text-[11px] text-gray-400'>
              ...
              {selectedAgentRun.checkpointedAt ? <span className='text-emerald-300'>{new Date(selectedAgentRun.checkpointedAt).toLocaleString()}</span> : <span className='text-gray-500'>None</span>}
            </div>
            {selectedAgentRun.checkpointedAt ? (
              <div className='mb-3 rounded-md border border-emerald-500/20 bg-emerald-500/5 p-2 text-[11px] text-emerald-200'>
                {selectedAgentRun.activeStepId ? `Active step: ${selectedAgentRun.activeStepId}` : 'Active step: unknown'}
              </div>
            ) : null}
                          ...
                          <ul className='mt-2 space-y-1 pl-3 text-[10px] text-gray-300'>
                            {subgoal.steps?.map((step: any, stepIndex: number) => (
                              <li key={`${goalIndex}-${subIndex}-${stepIndex}`}>
                                ...
                                {step.successCriteria ? <div className='mt-1 text-[9px] text-gray-400'>Success: {step.successCriteria}</div> : null}
                                {step.expectedObservation ? <div className='mt-1 text-[9px] text-gray-500'>Expected: {step.expectedObservation}</div> : null}
                              </li>
                            ))}
                          </ul>
                        </div>
                      )) ?? <p className='text-[10px] text-gray-500'>No subgoals captured.</p>}
                    </div>
                  </div>
                ))}
              </div>
            ) : <p className='mt-2 text-gray-500'>No hierarchy captured yet.</p>}
          </div>
            ...
            {latestPlanSteps.length ? (
              <div className='mt-2 max-h-56 space-y-2 overflow-y-auto rounded-md border border-border bg-gray-900 p-2 text-[11px] text-gray-200'>
                {latestPlanSteps.map((step, index) => {
                  ...
                  return (
                    <div key={stepId} className='rounded-md border border-border bg-card p-2'>
                      <div className='flex items-center justify-between text-[10px] text-gray-500'>
                        <span>{step.status ?? 'pending'}</span>
                        {step.phase ? <span className='text-amber-200'>{step.phase}</span> : null}
                      </div>
                      <p className='mt-1 text-[11px] text-gray-200'>{step.title || `Step ${index + 1}`}</p>
                      {step.successCriteria ? <p className='mt-1 text-[10px] text-gray-400'>Success: {step.successCriteria}</p> : null}
                      {step.expectedObservation ? <p className='mt-1 text-[10px] text-gray-500'>Expected: {step.expectedObservation}</p> : null}
                      {events.length > 0 ? (
                        <div className='mt-2 rounded-md border border-border bg-gray-900 p-2'>
                          <p className='text-[10px] uppercase tracking-wide text-gray-500'>Planning events</p>
                          <ul className='mt-1 space-y-1 text-[10px] text-gray-300'>
                            {events.map((event: AgentAuditLog) => <li key={event.id}>{event.message}</li>)}
                          </ul>
                        </div>
                      ) : null}
                    </div>
          ...
          <div className='mt-4 rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            <div className='flex items-center gap-2'>
              <span className='rounded-full border border-amber-400/40 bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-200'>Branch</span>
                  ...
                  return (
                    <div key={audit.id ?? `branch-${index}`} className='rounded-md border border-border bg-card p-2'>
                      <div className='flex items-center justify-between text-[10px] text-gray-500'>
                        ...
                        {meta?.reason ? <span className='text-amber-200'>{meta.reason}</span> : null}
                      </div>
                      <p className='mt-1 text-[11px] text-gray-200'>{audit.message}</p>
                      ...
                      ) : <p className='mt-2 text-[10px] text-gray-500'>No branch steps captured.</p>}
                    </div>
                  );
                })}
              </div>
            ) : <p className='mt-2 text-gray-500'>No branches recorded yet.</p>}
          </div>

          <div className='mt-4 rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            <p className='text-[11px] text-gray-500'>Agent steps</p>
            <div className='mt-2 max-h-48 space-y-2 overflow-y-auto rounded-md border border-border bg-gray-900 p-2 text-[11px] text-gray-200'>
              {agentAuditLogs.length === 0 ? <p className='text-gray-500'>No steps yet.</p> : (
                agentAuditLogs.map((step: AgentAuditLog) => (
                  <div key={step.id} className='rounded-md border border-border px-2 py-1'>
                    <div className='flex items-center justify-between text-[10px] text-gray-500'>
                      <span className='uppercase tracking-wide'>{step.level}</span>
                    ...
                    {step.metadata ? (
                      <div className='mt-2'>
                        <Button
                          ...
                          onClick={() => setExpandedAuditIds(prev => ({ ...prev, [step.id]: !prev[step.id] }))}
                        >
                          {expandedAuditIds[step.id] ? 'Hide metadata' : 'Show metadata'}
                        </Button>
        ...
        <TabsContent value='logs' className='mt-4'>
          <div className='rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            ...
            <div className='mt-2 max-h-48 space-y-1 overflow-y-auto rounded-md border border-border bg-gray-900 p-2 text-[11px] text-gray-200'>
              {agentBrowserLogs.length === 0 ? <p className='text-gray-500'>No logs yet.</p> : (
        ...
        <TabsContent value='context' className='mt-4'>
          <div className='rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            ...
            {latestPlannerContext ? (
              <div className='mt-2 max-h-48 overflow-y-auto rounded-md border border-border bg-gray-900 p-2 text-[11px] text-gray-200'>
                <pre className='whitespace-pre-wrap break-words'>{JSON.stringify(latestPlannerContext, null, 2)}</pre>
              </div>
            ) : <p className='mt-2 text-gray-500'>No planner context captured yet.</p>}
          </div>
          <div className='mt-4 rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            ...
            ) : <p className='mt-2 text-gray-500'>No session context captured yet.</p>}
          </div>
        </TabsContent>
        ...
        <TabsContent value='elements' className='mt-4'>
          <div className='rounded-md border border-border bg-card p-3 text-xs text-gray-300'>
            ...
            ) : <p className='mt-2 text-gray-500'>No candidate elements captured yet.</p>}
          </div>
        </TabsContent>
      </Tabs>
    </AppModal>
