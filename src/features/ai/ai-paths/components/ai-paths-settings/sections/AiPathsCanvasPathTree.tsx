...
type AiPathsTreeNodeProps = FolderTreeViewportRenderNodeInput & {
  ...
  handleDuplicatePath: (pathId?: string, options?: { folderPath?: string | null }) => void;
  ...
  toast: (message: string, options?: { variant?: 'error' | 'info' | 'success' | 'warning' }) => void;
};
...
export function AiPathsCanvasPathTree(): React.JSX.Element {
  ...
  return (
    <>
      <FolderTreePanel
        className='h-full border-r border-border/60 bg-card/35'
            ...
            <div className='mb-2 flex items-center justify-between gap-2'>
              <div>
                ...
                <p className='text-[11px] text-gray-500'>
                  Group and switch AI paths from the canvas.
                </p>
              </div>
            </div>
            <div className='mb-2'>
              <Input
                ...
                placeholder='Search groups or paths'
                className='h-8 text-xs'
                aria-label='Search path groups'
              />
            </div>
