...
import { useBenchmarkActions, useBenchmarkState } from '../../context/BenchmarkContext';
import { useSettingsState } from '../../context/SettingsContext';
import { promptExploderClampNumber } from '../../helpers/formatting';
import { BENCHMARK_SUITE_OPTIONS } from './PromptExploder.Constants';
  ...
  const suiteOptionsWithCounts = React.useMemo(() => {
    return BENCHMARK_SUITE_OPTIONS.map(option => {
      if (option.value === 'default') {
        return { ...option, label: `Default (${DEFAULT_PROMPT_EXPLODER_BENCHMARK_CASES.length} cases)` };
      }
      if (option.value === 'extended') {
        return { ...option, label: `Extended (${EXTENDED_PROMPT_EXPLODER_BENCHMARK_CASES.length} cases)` };
      }
      return option;
    });
  }, []);
