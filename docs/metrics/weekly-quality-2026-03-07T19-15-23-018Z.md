...
Generated at: 2026-03-07T19:15:23.018Z
Node: v24.3.0
...
| --- | --- | ---: | ---: | --- |
| Build | PASS | 2.9m | 0 | `npm run build` |
...
| Typecheck | PASS | 49.1s | 0 | `npm run typecheck` |
| Critical Flow Gate | PASS | 29.7s | 0 | `npm run test:critical-flows:strict -- --ci --no-history` |
| Security Smoke Gate | PASS | 10.3s | 0 | `npm run test:security-smoke:strict -- --ci --no-history` |
| Unit Domain Gate | PASS | 5.4m | 0 | `npm run test:unit:domains:strict -- --ci --no-history` |
| Full Unit Tests | SKIPPED | 0ms | - | `npm run test:unit` |
| E2E Tests | SKIPPED | 0ms | - | `npm run test:e2e` |
| Architecture Guardrails | PASS | 5.1s | 0 | `node scripts/architecture/check-guardrails.mjs` |
| UI Consolidation Guardrail | PASS | 3.6s | 0 | `node scripts/architecture/check-ui-consolidation.mjs` |
| Observability Check | PASS | 888ms | 0 | `npm run observability:check` |
