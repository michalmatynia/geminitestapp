...
describe('Product AI Job Queue Worker', () => {
  ...
  it('processes a job successfully', async () => {
    ...
    await mocks.productAiProcessor({ jobId: 'job-1', productId: 'p1', type: 'description_generation', payload: {} });

    expect(mockJobRepo.updateJob).toHaveBeenCalledWith('job-1', expect.objectContaining({ 
      status: 'running',
  ...
  it('handles errors and captures exception', async () => {
    ...
    await expect(mocks.productAiProcessor({ 
      jobId: 'job-1', 
      productId: 'p1', 
      type: 'description_generation', 
      payload: {} 
