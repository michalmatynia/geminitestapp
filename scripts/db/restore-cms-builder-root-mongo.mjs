...
import { buildMongoOptions, parseDbNameFromUri } from './cms-builder-mongo-selection.mjs';

const args = new Set(process.argv.slice(2));
const apply = args.has('--apply');
const expectedConfirm = 'restore-cms-builder-root';
...
try {
  ...
  for (const { name, docs } of backup.collections) {
    ...
    existing[name] =
      ids.length > 0 ? await db.collection(name).countDocuments({ _id: { $in: ids } }) : 0;
  }
