import { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { CheckCircle, XCircle, RefreshCw } from 'lucide-react';
import { addXp, XP_REWARDS, loadProgress } from '@/components/progress/progressSystem';

const MONTHS = [
  'Styczeń',
  'Luty',
  'Marzec',
  'Kwiecień',
  'Maj',
  'Czerwiec',
  'Lipiec',
  'Sierpień',
  'Wrzesień',
  'Październik',
  'Listopad',
  'Grudzień',
];
const DAYS = ['Poniedziałek', 'Wtorek', 'Środa', 'Czwartek', 'Piątek', 'Sobota', 'Niedziela'];
const MONTHS_DAYS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

function shuffle(arr) {
  return [...arr].sort(() => Math.random() - 0.5);
}

function randInt(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}

function generateQuestion() {
  const type = randInt(0, 4);

  if (type === 0) {
    // Which month is number X?
    const idx = randInt(0, 11);
    const answer = MONTHS[idx];
    const wrongs = shuffle(MONTHS.filter((_, i) => i !== idx)).slice(0, 3);
    return {
      question: `Który miesiąc jest ${idx + 1}. w roku?`,
      answer,
      choices: shuffle([answer, ...wrongs]),
    };
  }

  if (type === 1) {
    // What number is month X?
    const idx = randInt(0, 11);
    const answer = String(idx + 1);
    const wrongs = shuffle(
      Array.from({ length: 12 }, (_, i) => String(i + 1)).filter((n) => n !== answer)
    ).slice(0, 3);
    return {
      question: `Który numer kolejny ma miesiąc ${MONTHS[idx]}?`,
      answer,
      choices: shuffle([answer, ...wrongs]),
    };
  }

  if (type === 2) {
    // How many days in month X?
    const idx = randInt(0, 11);
    const answer = String(MONTHS_DAYS[idx]);
    const allCounts = ['28', '29', '30', '31'];
    const wrongs = shuffle(allCounts.filter((d) => d !== answer)).slice(0, 3);
    return {
      question: `Ile dni ma miesiąc ${MONTHS[idx]}?`,
      answer,
      choices: shuffle([answer, ...wrongs]),
    };
  }

  if (type === 3) {
    // Which day comes after day X?
    const idx = randInt(0, 5);
    const answer = DAYS[idx + 1];
    const wrongs = shuffle(DAYS.filter((d) => d !== answer)).slice(0, 3);
    return {
      question: `Jaki dzień tygodnia następuje po ${DAYS[idx]}?`,
      answer,
      choices: shuffle([answer, ...wrongs]),
    };
  }

  // type === 4: How many days in a week / months in a year?
  const isWeek = Math.random() > 0.5;
  const answer = isWeek ? '7' : '12';
  const wrong1 = isWeek ? ['5', '6', '8'] : ['10', '11', '13'];
  return {
    question: isWeek ? 'Ile dni ma tydzień?' : 'Ile miesięcy ma rok?',
    answer,
    choices: shuffle([answer, ...wrong1]),
  };
}

const TOTAL = 6;

export default function CalendarTrainingGame({ onFinish }) {
  const [questions] = useState(() => Array.from({ length: TOTAL }, generateQuestion));
  const [current, setCurrent] = useState(0);
  const [score, setScore] = useState(0);
  const [selected, setSelected] = useState(null);
  const [done, setDone] = useState(false);
  const [xpEarned, setXpEarned] = useState(0);

  const question = questions[current];

  const handleSelect = (choice) => {
    if (selected !== null) return;
    setSelected(choice);
    const correct = choice === question.answer;
    const newScore = correct ? score + 1 : score;
    if (correct) setScore(newScore);

    setTimeout(() => {
      setSelected(null);
      if (current + 1 >= TOTAL) {
        handleDone(newScore);
      } else {
        setCurrent((c) => c + 1);
      }
    }, 1200);
  };

  const handleDone = (finalScore) => {
    const isPerfect = finalScore === TOTAL;
    const isGood = finalScore >= 4;
    const xp = isPerfect ? 60 : isGood ? 30 : 10;
    const prog = loadProgress();
    addXp(xp, {
      calendarPerfect: isPerfect ? (prog.calendarPerfect || 0) + 1 : prog.calendarPerfect || 0,
    });
    setXpEarned(xp);
    setDone(true);
  };

  const handleRestart = () => {
    setCurrent(0);
    setScore(0);
    setSelected(null);
    setDone(false);
    setXpEarned(0);
  };

  if (done) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className='flex flex-col items-center gap-5 py-4'
      >
        <div className='text-6xl'>{score >= 5 ? '🏆' : score >= 3 ? '😊' : '💪'}</div>
        <h3 className='text-2xl font-extrabold text-green-700'>
          Wynik: {score}/{TOTAL}
        </h3>
        {xpEarned > 0 && (
          <div className='bg-indigo-100 text-indigo-700 font-bold px-4 py-2 rounded-full text-sm'>
            +{xpEarned} XP ✨
          </div>
        )}
        <p className='text-gray-500 text-center max-w-xs'>
          {score === TOTAL
            ? 'Idealnie! Świetnie znasz kalendarz!'
            : 'Ćwicz dalej, a zostaniesz mistrzem kalendarza!'}
        </p>
        <div className='flex gap-3'>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.97 }}
            onClick={handleRestart}
            className='flex items-center gap-2 bg-green-100 text-green-700 font-bold px-5 py-2.5 rounded-2xl hover:bg-green-200 transition'
          >
            <RefreshCw className='w-4 h-4' /> Jeszcze raz
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.97 }}
            onClick={onFinish}
            className='bg-gradient-to-r from-green-500 to-teal-500 text-white font-bold px-5 py-2.5 rounded-2xl shadow'
          >
            Zakończ lekcję ✅
          </motion.button>
        </div>
      </motion.div>
    );
  }

  return (
    <div className='flex flex-col items-center gap-5 w-full'>
      {/* Progress dots */}
      <div className='flex gap-2'>
        {questions.map((_, i) => (
          <div
            key={i}
            className={`w-3 h-3 rounded-full transition-all ${
              i < current
                ? 'bg-green-400'
                : i === current
                  ? 'bg-green-600 scale-125'
                  : 'bg-gray-200'
            }`}
          />
        ))}
      </div>

      <div className='bg-green-50 border border-green-200 rounded-2xl px-6 py-4 text-center w-full'>
        <p className='text-lg font-extrabold text-green-800'>{question.question}</p>
      </div>

      <div className='grid grid-cols-2 gap-3 w-full'>
        {question.choices.map((choice) => {
          let bg = 'bg-white hover:bg-green-50 border-green-200';
          if (selected !== null) {
            if (choice === question.answer) bg = 'bg-green-100 border-green-500';
            else if (choice === selected) bg = 'bg-red-100 border-red-400';
            else bg = 'bg-gray-50 border-gray-200';
          }
          return (
            <motion.button
              key={choice}
              whileHover={selected === null ? { scale: 1.04 } : {}}
              whileTap={selected === null ? { scale: 0.97 } : {}}
              onClick={() => handleSelect(choice)}
              className={`border-2 rounded-2xl px-4 py-3 font-bold text-gray-700 text-base transition-all ${bg}`}
            >
              {choice}
            </motion.button>
          );
        })}
      </div>

      <AnimatePresence>
        {selected !== null && (
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0 }}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-bold text-lg ${
              selected === question.answer
                ? 'bg-green-100 text-green-700'
                : 'bg-red-100 text-red-700'
            }`}
          >
            {selected === question.answer ? (
              <>
                <CheckCircle className='w-5 h-5' /> Brawo! Dobrze!
              </>
            ) : (
              <>
                <XCircle className='w-5 h-5' /> Poprawna odpowiedź: {question.answer}
              </>
            )}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
}
