...
const {
  enqueueSocialPublishingPipelineJobMock,
  recoverSocialPublishingPipelineQueueMock,
  resolveSocialPublishingActorMock,
  startSocialPublishingPipelineQueueMock,
...
vi.mock('@/features/filemaker/social/workers/socialPublishingPipelineQueue', () => ({
  enqueueSocialPublishingPipelineJob: (...args: unknown[]) =>
    enqueueSocialPublishingPipelineJobMock(...args),
  recoverSocialPublishingPipelineQueue: (...args: unknown[]) =>
    recoverSocialPublishingPipelineQueueMock(...args),
  startSocialPublishingPipelineQueue: (...args: unknown[]) =>
    startSocialPublishingPipelineQueueMock(...args),
}));
...
describe('social pipeline trigger handler', () => {
  beforeEach(() => {
    ...
    recoverSocialPublishingPipelineQueueMock.mockResolvedValue([]);
    enqueueSocialPublishingPipelineJobMock.mockResolvedValue('job-123');
  ...
  it('recovers stale jobs, starts the worker, and enqueues the scheduled tick job by default', async () => {
    const response = await postHandler(
      new NextRequest('http://localhost/api/filemaker/social-pipeline/trigger', {
        method: 'POST',
      }),
      createContext()
    );

    expect(recoverSocialPublishingPipelineQueueMock).toHaveBeenCalledTimes(1);
    expect(startSocialPublishingPipelineQueueMock).toHaveBeenCalledTimes(1);
    expect(enqueueSocialPublishingPipelineJobMock).toHaveBeenCalledWith({
      type: 'pipeline-tick',
    });
  ...
  it('fails fast when Redis is configured but unreachable', async () => {
    ...
    expect(recoverSocialPublishingPipelineQueueMock).not.toHaveBeenCalled();
    expect(startSocialPublishingPipelineQueueMock).not.toHaveBeenCalled();
    expect(enqueueSocialPublishingPipelineJobMock).not.toHaveBeenCalled();
  });
