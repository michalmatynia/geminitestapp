...
type KangurLearnerProfileRuntimeSnapshot = ReturnType<typeof useKangurLearnerProfileRuntime>['snapshot'];
type KangurLearnerProfileOperationPerformance =
  KangurLearnerProfileRuntimeSnapshot['operationPerformance'][number];
  ...
  return (
    <div className='flex h-32 items-end gap-2'>
      {snapshot.weeklyActivity.map((point) => {
        const heightPercent =
          point.games === 0 ? 6 : Math.max(14, Math.round((point.games / maxWeeklyGames) * 100));
        return (
    ...
    >
      <div className='flex flex-col gap-3'>
        <div className='flex flex-wrap items-start justify-between gap-3'>
          ...
          >
            <Link
              href={buildKangurOperationPracticeHref(basePath, item.operation, item.averageAccuracy)}
              targetPageKey='Game'
              transitionAcknowledgeMs={LEARNER_PROFILE_PERFORMANCE_ROUTE_ACKNOWLEDGE_MS}
