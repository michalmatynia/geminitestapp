...
import { ENGLISH_ADVERBS_ACTION_STUDIO_ROUNDS } from './EnglishAdverbsActionStudioGame.data';
import {
  buildRoundState,
  countRoundCorrect,
...
export function useEnglishAdverbsActionStudioGameState() {
  ...
  const [roundState, setRoundState] = useState<RoundState>(() =>
    buildRoundState(ENGLISH_ADVERBS_ACTION_STUDIO_ROUNDS[0])
  );
  ...
  const round =
    ENGLISH_ADVERBS_ACTION_STUDIO_ROUNDS[roundIndex] ?? ENGLISH_ADVERBS_ACTION_STUDIO_ROUNDS[0];
  ...
  const handleCheck = (): void => {
    ...
    setFeedback({
      ...
      text: isPerfect
        ? translations('englishAdverbs.inRound.studio.feedback.perfect')
        : translations('englishAdverbs.inRound.studio.feedback.retry'),
    });
    setSelectedTokenId(null);
    setChecked(true);
  };
  ...
  const handleRestart = (): void => {
    setRoundIndex(0);
    setRoundState(buildRoundState(ENGLISH_ADVERBS_ACTION_STUDIO_ROUNDS[0]));
    setChecked(false);
    setRoundCorrect(0);
    setTotalCorrect(0);
    setFeedback(null);
