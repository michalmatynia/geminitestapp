...
import { Button, StatusBadge, DocumentationSection, Hint, FormSection, Card } from '@/shared/ui';
      ...
      {insightsQuery.isLoading ? (
        ...
        <div className='mt-1 space-y-3'>
          {insightsQuery.data?.insights.map((insight: AiInsightRecord) => (
            <Card key={insight.id} variant='subtle-compact' padding='sm' className='border-border/60 bg-card/40 text-xs text-gray-300'>
              ...
              {insight.warnings.length > 0 ? (
                <Card variant='warning' padding='sm' className='mt-3 border-amber-500/20'>
                  <div className='mb-1.5 text-[10px] font-bold uppercase tracking-wider text-amber-200/70'>Warnings</div>
