...
const CASE_RESOLVER_ADDRESSER_LABEL_PATTERN =
  '(?:from|od|nadawca|sender|addresser|wnioskodawca)';
const CASE_RESOLVER_ADDRESSEE_LABEL_PATTERN =
  '(?:to|do|adresat|recipient|addressee|odbiorca|organ)';
const CASE_RESOLVER_INLINE_ADDRESSER_LABEL_PATTERN = `^\\s*${CASE_RESOLVER_ADDRESSER_LABEL_PATTERN}\\s*:\\s*(.+?)\\s*$`;
const CASE_RESOLVER_INLINE_ADDRESSEE_LABEL_PATTERN = `^\\s*${CASE_RESOLVER_ADDRESSEE_LABEL_PATTERN}\\s*:\\s*(.+?)\\s*$`;
...
export const PROMPT_EXPLODER_PATTERN_PACK: PromptValidationRule[] = [
  ...
  createRegexRule({
    ...
    pattern: CASE_RESOLVER_INLINE_ADDRESSER_LABEL_PATTERN.replace(
      '(.+?)',
      '([\\p{Lu}][\\p{L}\'’.-]+)(?:\\s+([\\p{Lu}][\\p{L}\'’.-]+(?:\\s+[\\p{Lu}][\\p{L}\'’.-]+){0,2}))?\\s+([\\p{Lu}][\\p{L}\'’.-]+)'
    ),
    flags: 'imu',
