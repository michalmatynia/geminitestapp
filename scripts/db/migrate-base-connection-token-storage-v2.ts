...
import { migrateBaseTokenStorage } from './lib/integrations/base-token-storage-migration';
...
type ProviderSummary = {
  ...
  connectionsWithBaseApiToken: number;
...
const scanConnections = (input: {
  connections: Array<{ id: string; baseApiToken: unknown; password: unknown }>;
  warnings: string[];
}): ConnectionScanStats => {
  const stats: ConnectionScanStats = {
    connectionsScanned: 0,
    connectionsWithBaseApiToken: 0,
  ...
  input.connections.forEach((connection) => {
    ...
    if (migrated.hadBaseApiTokenBefore) {
      stats.connectionsWithBaseApiToken += 1;
    }
    if (!migrated.hadBaseApiTokenBefore && migrated.hadLegacyPasswordBefore) {
      stats.connectionsWithLegacyPasswordOnly += 1;
    }
    if (migrated.backfilled) {
      stats.connectionsBackfilled += 1;
    }
    if (!migrated.hasBaseApiTokenAfter) {
...
const migratePrisma = async (options: CliOptions): Promise<ProviderSummary> => {
  ...
  try {
    ...
    return {
      ...
      connectionsWithBaseApiToken: stats.connectionsWithBaseApiToken,
...
const main = async (): Promise<void> => {
  ...
  console.log('[migrate-base-connection-token-storage-v2] Summary');
