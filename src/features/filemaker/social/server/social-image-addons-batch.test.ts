...
vi.mock('@/features/playwright/server/instances', () => ({
  createSocialCaptureBatchPlaywrightInstance: (input: Record<string, unknown> = {}) => ({
    ...
    tags: ['social-publishing', 'capture', 'batch'],
    ...input,
  }),
}));
...
vi.mock('@/features/filemaker/social/shared/social-capture-presets', () => ({
...
describe('createSocialPublishingImageAddonsBatch', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('throws when baseUrl is empty', async () => {
    await expect(
      createSocialPublishingImageAddonsBatch({ baseUrl: '' })
  ...
  it('forwards request cookies into Playwright storage state', async () => {
    ...
    await createSocialPublishingImageAddonsBatch({
      baseUrl: 'https://kangur.app',
      presetIds: ['game'],
  ...
  it('throws when no presets match the provided ids', async () => {
    await expect(
      createSocialPublishingImageAddonsBatch({
        baseUrl: 'https://kangur.app',
  ...
  it('throws when Playwright run fails', async () => {
    ...
    await expect(
      createSocialPublishingImageAddonsBatch({ baseUrl: 'https://kangur.app' })
