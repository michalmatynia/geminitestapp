...
const HEADERLESS_PROFILE_FIELDS = [
  FIELDS.createdBy,
...
type LegacyOrganizationProfileRowsFormat = 'CSV/TSV' | 'XLSX';

export type LegacyOrganizationProfileRow = Record<string, string>;

export type ParsedLegacyOrganizationProfile = {
  createdAt?: string;
  createdBy?: string;
...
const findHeaderRowIndex = (rows: string[][]): number =>
  rows.findIndex((row: string[], index: number): boolean => {
    ...
    return hasProfileHeader(row.map((field: string): string => normalizeString(field)));
  });
...
const rowsToLegacyOrganizationProfileRows = (
  matrix: unknown[][],
  input: { format: LegacyOrganizationProfileRowsFormat }
): LegacyOrganizationProfileRow[] => {
  const rows = matrix
  ...
  if (!hasProfileHeader(header)) {
    if (input.format === 'CSV/TSV' && looksLikeHeaderlessProfileRows(rows)) {
      return rows.map((row: string[]): LegacyOrganizationProfileRow =>
        rowToObject(HEADERLESS_PROFILE_FIELDS, row)
      );
    }
    throw buildMissingHeaderError(input.format);
  }

  return rows.slice(headerRowIndex + 1).map((row: string[]): LegacyOrganizationProfileRow =>
    rowToObject(header, row)
  );
};

export const parseFilemakerLegacyOrganizationProfileRows = (
  text: string
): LegacyOrganizationProfileRow[] => {
...
export const parseFilemakerLegacyOrganizationProfileWorkbookRows = async (
  ...
  return rowsToLegacyOrganizationProfileRows(matrix, { format: 'XLSX' });
};
