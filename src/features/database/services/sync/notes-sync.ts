...
export const syncNotes: SyncHandler = async ({ mongo, prisma }) => {
  ...
  const data = (docs as unknown as MongoNoteDoc[]).map((doc): Prisma.NoteCreateManyInput & { tags: any[], categories: any[], relationsFrom: any[] } => {
    ...
    return {
      ...
      notebookId: doc.notebookId && availableNotebookIds.has(doc.notebookId) ? doc.notebookId : null,
  ...
  const created = noteData.length ? await prisma.note.createMany({ data: noteData as Prisma.NoteCreateManyInput[] }) : { count: 0 };
