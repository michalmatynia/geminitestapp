export const AMAZON_REVERSE_IMAGE_SCAN_RUNTIME_PART_2 = String.raw`    };
  };
  ...
  const buildAmazonCandidateOutcome = (input = {}) => ({
    ...
    candidateResults: Array.isArray(input.candidateResults)
      ...
      : [],
    candidatePreviews: Array.isArray(input.candidatePreviews)
      ? input.candidatePreviews
          .map((candidate, index) => ({
            id: toText(candidate?.id),
            matchedImageId: toText(candidate?.matchedImageId),
            url: toText(candidate?.url),
            asin: toText(candidate?.asin),
            marketplaceDomain: toText(candidate?.marketplaceDomain),
            title: toText(candidate?.title),
            snippet: toText(candidate?.snippet),
            heroImageUrl: toText(candidate?.heroImageUrl),
            heroImageAlt: toText(candidate?.heroImageAlt),
            heroImageArtifactName: toText(candidate?.heroImageArtifactName),
            artifactKey: toText(candidate?.artifactKey),
            rank:
              typeof candidate?.rank === 'number' && Number.isFinite(candidate.rank)
                ? Math.trunc(candidate.rank)
                : typeof index === 'number'
  ...
  const collectAmazonCandidatePreviewData = async ({
    candidateUrl,
    matchedImageId,
    amazonCandidateAttempt,
    candidateRank = null,
    ...
    if (!probeResult.success || !probeResult.amazonProbe) {
      return buildAmazonCandidateOutcome({
        status: 'failed',
        matchedImageId: toText(matchedImageId),
        currentUrl: page.url(),
        url: candidateUrl,
        message: 'Failed to collect Amazon candidate preview.',
        stage: 'amazon_probe',
      });
    }

    const amazonProbe = probeResult.amazonProbe;
    ...
    return buildAmazonCandidateOutcome({
      ...
      price: null,
      url: amazonProbe.canonicalUrl || amazonProbe.candidateUrl || candidateUrl,
      description: amazonProbe.descriptionSnippet || candidateResult?.snippet || null,
      amazonDetails: null,
      amazonProbe,
      candidateResults: candidateResult ? [candidateResult] : [],
      candidatePreviews: [
        {
          id: candidateResult?.asin || amazonProbe.asin || matchedImageId || candidateUrl,
          matchedImageId: toText(matchedImageId),
          url: amazonProbe.canonicalUrl || amazonProbe.candidateUrl || candidateUrl,
          asin: amazonProbe.asin || candidateResult?.asin || null,
          marketplaceDomain: amazonProbe.marketplaceDomain || candidateResult?.marketplaceDomain || null,
          title: amazonProbe.pageTitle || candidateResult?.title || null,
  ...
  try {
    ...
    for (const candidate of imageCandidates) {
      ...
      if (shouldCollectAmazonCandidatePreviews === true) {
        const candidatePreviews = [];
        for (const [candidateRankIndex, amazonCandidateUrl] of amazonCandidates.slice(0, 5).entries()) {
          const amazonCandidateAttempt = attemptedAmazonCandidateCount + 1;
          attemptedAmazonCandidateCount += 1;
          const candidateResult =
            amazonCandidateResults.find((entry) => entry?.url === amazonCandidateUrl) ?? null;
          const previewResult = await collectAmazonCandidatePreviewData({
            candidateUrl: amazonCandidateUrl,
            matchedImageId: candidateId,
            amazonCandidateAttempt,
            candidateRank: candidateRankIndex + 1,
            candidateResult,
          }).catch((error) =>
          ...
          if (previewResult.stage !== 'amazon_open_failed') {
            openedAmazonCandidateCount += 1;
          }

          if (previewResult.status === 'failed') {
            lastAmazonCandidateError = previewResult.message;
            log('amazon.scan.amazon_candidate_preview_failed', {
              candidateId,
              amazonCandidateUrl,
          ...
          if (Array.isArray(previewResult.candidatePreviews)) {
            candidatePreviews.push(...previewResult.candidatePreviews);
          }
        }
        ...
        if (candidatePreviews.length > 0) {
          await emitResult({
            ...
            title: null,
            price: null,
            url: candidatePreviews[0]?.url || amazonCandidates[0] || page.url(),
            description: null,
            amazonDetails: null,
            amazonProbe: null,
            ...
            matchedImageId: candidateId,
            currentUrl: page.url(),
            message: 'Collected Amazon candidate previews for manual selection.',
            stage: 'amazon_probe',
          });
          return;
        }
