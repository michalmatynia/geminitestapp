...
function SuppressionManagementPanel(): React.JSX.Element {
  const {
    suppressionEntries,
  ...
  return (
    <div className='space-y-4 rounded-md border border-border/60 bg-card/25 p-4'>
      ...
      <Button
        ...
        disabled={suppressionEmailDraft.trim() === '' || isUpdatePending}
      >
        Add Suppression
      </Button>
      <SuppressionEntriesList
        entries={suppressionEntries}
        isUpdatePending={isUpdatePending}
        onRemove={handleRemoveSuppressionEntry}
      />
    </div>
  );
}

function SuppressionEntriesList({
  entries,
  isUpdatePending,
  onRemove,
}: {
  entries: ReturnType<typeof useCampaignEditContext>['suppressionEntries'];
  isUpdatePending: boolean;
  onRemove: (emailAddress: string) => Promise<void>;
}): React.JSX.Element {
  return (
    <div className='space-y-2 pt-2'>
      <div className='text-[11px] uppercase tracking-[0.22em] text-gray-500'>
        Current Suppressions ({entries.length})
      </div>
      {entries.length === 0 ? (
              ...
              >
                <div className='min-w-0 flex-1 truncate'>
                  ...
                  <div className='text-[10px] text-gray-500'>
                    {entry.reason} • {notes !== '' ? notes : 'No notes'}
                  </div>
                </div>
                <Button
                  type='button'
                  ...
                  onClick={() => { void onRemove(entry.emailAddress); }}
...
function PreferenceLinkTokensPanel(): React.JSX.Element {
...
export const DeliveryGovernanceSection = (): React.JSX.Element => {
  return (
    <FormSection title='Delivery Governance' className='space-y-4 p-4'>
      <div className='grid gap-4 md:grid-cols-2'>
        <SuppressionManagementPanel />
        <PreferenceLinkTokensPanel />
      </div>
    </FormSection>
  );
};
