...
- Client local runtime native subset now includes:
  - `audio_oscillator`, `audio_speaker`, `bundle`, `compare`, `constant`, `context`, `delay`, `fetcher`, `gate`, `iterator`, `mapper`, `math`, `mutator`, `notification`, `parser`, `prompt`, `regex`, `router`, `simulation`, `string_mutator`, `template`, `trigger`, `validation_pattern`, `validator`, `viewer`
- Remaining server-only native node families are:
  - `agent`, `ai_description`, `api_advanced`, `database`, `db_schema`, `description_updater`, `http`, `learner_agent`, `model`, `playwright`, `poll`
