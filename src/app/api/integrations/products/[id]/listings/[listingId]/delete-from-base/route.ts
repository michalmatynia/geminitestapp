...
export const POST = apiHandlerWithParams<{ id: string; listingId: string }>(
  (req, ctx, params) => {
    recordLegacyCompatCounter('compat_route_hit', {
      source: 'api.compat.integrations.products.[id].listings.[listingId].delete-from-base.POST',
      context: { route: '/api/integrations/products/[id]/listings/[listingId]/delete-from-base' },
    });
    assertLegacyCompatRouteEnabled({
      route: '/api/integrations/products/[id]/listings/[listingId]/delete-from-base',
      method: 'POST',
      source: 'api.compat.integrations.products.[id].listings.[listingId].delete-from-base.POST',
    });
  ...
  {
    source: 'integrations.products.[id].listings.[listingId].delete-from-base.POST',
    requireCsrf: false,
  }
);
