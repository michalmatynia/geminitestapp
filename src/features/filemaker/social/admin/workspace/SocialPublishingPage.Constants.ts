...
import type {
  ...
  SocialPublishingPostEditorStateDto,
} from '@/shared/contracts/social-publishing-posts';
import type { ImageFileSelection } from '@/shared/contracts/files';

export type EditorState = SocialPublishingPostEditorStateDto;
...
export const statusLabel: Record<SocialPublishingPost['status'], string> = {
  draft: 'Draft',
  scheduled: 'Scheduled',
  published: 'Published',
  failed: 'Failed',
...
const TEMP_ADDON_PREFIX = '/var/tmp/libapp-uploads/filemaker/social-addons/';

const toServeUrl = (absPath: string): string => {
  const filename = absPath.slice(TEMP_ADDON_PREFIX.length);
  return `/api/filemaker/social-image-addons/serve?filename=${encodeURIComponent(filename)}`;
};
