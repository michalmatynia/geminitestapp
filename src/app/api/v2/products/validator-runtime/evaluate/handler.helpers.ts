...
import type { ProductValidationPattern, ProductValidationSeverity, ProductValidationPostAcceptBehavior } from '@/shared/contracts/products/validation';
...
export const MAX_RUNTIME_VALUE_FIELDS = 120;
export const MAX_RUNTIME_PATTERN_IDS = 120;
export const MAX_RUNTIME_STRING_LENGTH = 20_000;
export const DEFAULT_AI_RUNTIME_TIMEOUT_MS = 12_000;

export type RuntimeFieldIssue = {
  patternId: string;
  message: string;
...
export const assertRuntimePayloadBounds = (values: Record<string, unknown>, patternIds: string[] | undefined): void => {
  ...
  if (patternIds !== undefined && patternIds.length > MAX_RUNTIME_PATTERN_IDS) {
    throw badRequestError(
...
export const buildRuntimeFieldEntries = (values: Record<string, unknown>): RuntimeFieldEntry[] =>
...
const evaluateDatabaseRuntime = async ({
  ...
  const payloadSource =
    config['payload'] !== null && typeof config['payload'] === 'object' && !Array.isArray(config['payload'])
  ...
  if (operation === 'action') {
    ...
    const rawAction = String(renderedPayload['action'] ?? 'find').trim();
    const actionName = rawAction !== '' ? rawAction : 'find';
    ...
    if (response.ok === false) {
      throw badRequestError(response.error !== '' ? response.error : 'Runtime DB action failed.');
    }
    const resultData = response.data ?? {};
    const rawResultPath = typeof config['resultPath'] === 'string' ? config['resultPath'].trim() : '';
    const resultPath = rawResultPath !== '' ? rawResultPath : '';
  ...
  const payload: DbQueryPayload = {
    ...
    collection: String(renderedPayload['collection'] ?? ''),
    filter:
      renderedPayload['filter'] !== null && typeof renderedPayload['filter'] === 'object'
        ? renderedPayload['filter']
        : {},
    ...(renderedPayload['projection'] !== null && typeof renderedPayload['projection'] === 'object'
      ? { projection: renderedPayload['projection'] }
      : {}),
    ...(renderedPayload['sort'] !== null && typeof renderedPayload['sort'] === 'object'
  ...
  if (response.ok === false) {
    throw badRequestError(response.error !== '' ? response.error : 'Runtime DB query failed.');
  }
  const resultData = response.data ?? {};
  const defaultResultPath = payload.single === true ? 'item' : 'count';
  const rawResultPath =
    typeof config['resultPath'] === 'string' ? config['resultPath'].trim() : defaultResultPath;
  const resultPath = rawResultPath !== '' ? rawResultPath : defaultResultPath;
...
const evaluateAiRuntime = async ({
  ...
  const completion = await withTimeout(
    runBrainChatCompletion({
      ...
      messages: [
        {
          role: 'system',
          content: [(brainConfig.systemPrompt !== undefined && brainConfig.systemPrompt !== '' ? brainConfig.systemPrompt : systemPrompt), settingsCache.contextRegistryPrompt]
  ...
  if (raw !== '') {
    ...
    if (parsedMatch !== null) {
      ...
      const resultPath = rawResultPath !== '' ? rawResultPath : 'raw';
  ...
  return {
    matched: false,
    resultData: { raw, brainApplied: brainConfig.brainApplied },
    message: null,
...
export const buildRuntimeIssue = ({
  pattern,
  fieldName,
  fieldValue,
  replacementValue,
  ...
  return {
    ...
    replacementScope,
    replacementActive: effectiveReplacementValue !== null,
...
export const evaluatePatternsForField = async ({
  pattern,
  entry,
  values,
  latestProductValues,
  ...
  pattern: ProductValidationPattern;
  entry: RuntimeFieldEntry;
  values: Record<string, unknown>;
  latestProductValues: Record<string, unknown> | null;
  validationScope: 'draft_template' | 'product_create' | 'product_edit';
  runtimeConfig: Record<string, unknown>;
  runtimeSettingsCache: RuntimeSettingsCache;
}): Promise<RuntimeFieldIssue | null> => {
  ...
  try {
    ...
    return buildRuntimeIssue({
      ...
      message: runtimeMessage ?? (messageFromTemplate !== '' ? messageFromTemplate : pattern.message),
