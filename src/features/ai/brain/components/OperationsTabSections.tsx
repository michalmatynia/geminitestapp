...
export function DomainCard({
  ...
  const runtimeRiskMetric = domainKey === 'ai_paths'
    ? domain.metrics.find((m) => m.key === 'runtime_kernel_risk')
    : undefined;
  ...
  const showRuntimeRiskBadge = domainKey === 'ai_paths' &&
  ...
  return (
    <Card variant='subtle' padding='md' className='border-border/60 bg-card/35 space-y-3'>
      ...
      <div className='flex flex-wrap items-center gap-2'>
        {domain.links.map((link: BrainOperationsLink) => (
      ...
      {isExpanded && (
        <div className='space-y-2 rounded-md border border-border/60 bg-background/30 p-2'>
          ...
          {domain.recentEvents.length === 0 ? (
            ...
            <div className='space-y-1'>
              {domain.recentEvents.map((event: BrainOperationsRecentEvent) => (
