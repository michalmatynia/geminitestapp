...
// Helper to avoid unsafe call/assignment
function resolveOpponents(data: any, opponentsLimit: number) {
  const rawEntries = (data?.entries ?? []) as KangurDuelLeaderboardEntry[];
  return [...rawEntries]
