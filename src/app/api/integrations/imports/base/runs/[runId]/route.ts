...
export const GET = apiHandlerWithParams<{ runId: string }>(
  (req, ctx, params) => {
    recordLegacyCompatCounter('compat_route_hit', {
      source: 'api.compat.integrations.imports.base.runs.[runId].GET',
      context: { route: '/api/integrations/imports/base/runs/[runId]' },
    });
    assertLegacyCompatRouteEnabled({
      route: '/api/integrations/imports/base/runs/[runId]',
      method: 'GET',
      source: 'api.compat.integrations.imports.base.runs.[runId].GET',
    });
