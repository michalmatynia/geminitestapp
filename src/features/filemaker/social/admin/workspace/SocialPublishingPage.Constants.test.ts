...
describe('SocialPublishingPage.Constants', () => {
  ...
  it('builds, merges, and matches image selections', () => {
    ...
    expect(
      resolveImagePreview({
        url: '/var/tmp/libapp-uploads/filemaker/social-addons/shot.png',
      })
    ).toBe('/api/filemaker/social-image-addons/serve?filename=shot.png');

    const merged = mergeImageAssets([first], [firstAlias, first, second]);
    expect(merged).toEqual([first, second]);
    expect(matchesImageAsset(first, { id: '/uploads/first.png' })).toBe(true);
    expect(matchesImageAsset(first, firstAlias)).toBe(true);
