...
async function syncMongoToPrisma(
  ...
  await syncCollection('auth_login_challenges', async () => {
    ...
    const created = data.length
      ? await prisma.authLoginChallenge.createMany({ data })
      : { count: 0 };
    return {
  ...
  await syncCollection('auth_security_attempts', async () => {
    ...
    const created = data.length
      ? await prisma.authSecurityAttempt.createMany({ data })
      : { count: 0 };
    return {
      sourceCount: data.length,
      targetDeleted: deleted.count,
...
async function syncPrismaToMongo(
  ...
  await syncCollection('auth_security_profiles', () =>
    syncAuthSecurityProfilesPrismaToMongo(context)
  );
  ...
  await syncCollection('integration_connections', () =>
    syncIntegrationConnectionsPrismaToMongo(context)
  );
